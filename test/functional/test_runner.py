...
BASE_SCRIPTS = [
    ...
    # Longest test should go first, to favor running tests in parallel
    # Kerrigan-specific tests (multi-algo PoW + Sapling shielded transactions)
    'feature_multialgo_stress.py',
    'feature_sapling_reorg.py',
    'feature_invalidateblock.py',
    'feature_sapling.py',
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, attempts=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, skipunit=False):
    ...
    try:
        ...
        if subprocess.run(["pgrep", "-x", "kerrigand"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a kerrigand process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
