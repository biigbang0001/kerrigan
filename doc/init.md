...
All three Linux startup configurations assume the existence of a "kerrigan" user
...
All three configurations assume several paths that might need to be adjusted.
    ...
    Configuration file:  /etc/kerrigan/kerrigan.conf
...
kerrigan user and group. It is advised for security reasons to make the
configuration file and data directory only readable by the kerrigan user and
group. Access to kerrigan-cli and other kerrigand rpc clients can then be
controlled by group membership.

NOTE: When using the systemd .service file, the creation of the aforementioned
directories and the setting of their permissions is automatically handled by
systemd. Directories are given a permission of 710, giving the kerrigan user and group
access to files under it _if_ the files themselves give permission to the
kerrigan user and group to do so. This does not allow
...
options specified in the init files (e.g. setting `KERRIGAND_DATADIR` for
OpenRC).

### macOS

    Binary:              /usr/local/bin/kerrigand
    Configuration file:  ~/Library/Application Support/Kerrigan/kerrigan.conf
    Data directory:      ~/Library/Application Support/Kerrigan
    Lock file:           ~/Library/Application Support/Kerrigan/.lock
...
Launch Daemon with a dedicated kerrigan user.
