...
installer (on Windows) or just copy over `/Applications/Kerrigan-Qt` (on macOS) or
...
Kerrigan is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 14+, and Windows 10+. Kerrigan
should also work on most other Unix-like systems but is not as
frequently tested on them. It is not recommended to use Kerrigan on
unsupported systems.
