...
Currently, it is possible to create a multisig wallet using Kerrigan only.
...
At the time of writing, there is no way to extract a specific path from wallets in Kerrigan. For this, an external signer/xpub can be used.
...
for ((n=1;n<=3;n++))
do
 ./src/kerrigan-cli -testnet -named createwallet wallet_name="participant_${n}" descriptors=true
done
```

Extract the xpub of each wallet. To do this, the `listdescriptors` RPC is used. By default, Kerrigan single-sig wallets are created using path `m/44'/1'/0'/` for PKH based accounts. Each of them uses the chain 0 for external addresses and chain 1 for internal ones, as shown in the example below.
...
pkh([1004658e/84'/1'/0']tpubDCBEcmVKbfC9KfdydyLbJ2gfNL88grZu1XcWSW9ytTM6fitvaRmVyr8Ddf7SjZ2ZfMx9RicjYAXhuh3fmLiVLPodPEqnQQURUfrBKiiVZc8/1/*)#en65rxc5
```
...
All RPCs in Kerrigan will include the checksum in their output.
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./src/kerrigan-cli -testnet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("pkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./src/kerrigan-cli -testnet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("pkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```

`jq` is used to extract the xpub from the `pkh` descriptor.
...
external_desc_sum=$(./src/kerrigan-cli -testnet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./src/kerrigan-cli -testnet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./src/kerrigan-cli help importdescriptors`.
...
```bash
./src/kerrigan-cli -testnet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true descriptors=true

./src/kerrigan-cli  -testnet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./src/kerrigan-cli  -testnet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./src/kerrigan-cli -testnet loadwallet "participant_${n}"; done
```

### 1.4 Fund the wallet

The wallet can receive testnet coins by generating a new address and passing it as parameters to `getcoins.py` script.
...
The url used by the script can also be accessed directly. At time of writing, the url is [`https://testnetfaucet.com`](https://testnetfaucet.com).
...
```bash
receiving_address=$(./src/kerrigan-cli -testnet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/testnet/getcoins.py -c ./src/kerrigan-cli -a $receiving_address
```
...
```bash
./src/kerrigan-cli -testnet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./src/kerrigan-cli -testnet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./src/kerrigan-cli -testnet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./src/kerrigan-cli -testnet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./src/kerrigan-cli -testnet decodepsbt $funded_psbt

./src/kerrigan-cli -testnet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./src/kerrigan-cli -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./src/kerrigan-cli -testnet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./src/kerrigan-cli -testnet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./src/kerrigan-cli -testnet finalizepsbt $combined_psbt | jq -r '.hex')

./src/kerrigan-cli -testnet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./src/kerrigan-cli -testnet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./src/kerrigan-cli -testnet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./src/kerrigan-cli -testnet finalizepsbt $psbt_2 | jq -r '.hex')
