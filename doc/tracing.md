# User-space, Statically Defined Tracing (USDT) for Kerrigan

Kerrigan includes statically defined tracepoints to allow for more
...
**Note**: Kerrigan uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Kerrigan
...
Multiple tools can list the available tracepoints in a `kerrigand` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Kerrigan, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/kerrigand
stap net        outbound_message 0x0000000000107c05 /src/kerrigand
stap validation block_connected  0x00000000002fb10c /src/kerrigand
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Kerrigan.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/kerrigand | grep NT_STAPSDT -A 4 -B 2
