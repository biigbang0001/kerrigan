...
* [ ] Update kerrigan.conf and commit, see [gen-dash-conf.sh](https://github.com/dashpay/dash/blob/develop/contrib/devtools/README.md#gen-dash-confsh).
...
### Kerrigan maintainers/release engineers, suggestion for writing release notes
...
Checkout the Kerrigan version you'd like to build:
...
After successfully notarizing the .dmg file, extract `Kerrigan-Qt.app` from the .dmg.
To verify that the notarization process was successful, run the following command:

```sh
spctl -a -vv -t install Kerrigan-Qt.app
```

Replace `Kerrigan-Qt.app` with the path to your .app file. This command checks whether your .app file passes Gatekeeper’s
