# CJDNS support in Kerrigan

It is possible to run Kerrigan over CJDNS, an encrypted IPv6 network that
...
done outside of Kerrigan, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Kerrigan network and individual nodes.
...
before it will work with your Kerrigan node. This is described in steps
...
## Run Kerrigan with CJDNS

Once you are connected to the CJDNS network, the following Kerrigan
...
When enabled, this option tells Kerrigan that it is running in an
...
IPv6 local network. This helps Kerrigan perform better address management:
...
CJDNS support was added to Kerrigan in version 21.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `kerrigan-cli -addrinfo` to see the
...
## CJDNS-related information in Kerrigan

There are several ways to see your CJDNS address in Kerrigan:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `kerrigan-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `kerrigan-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `kerrigan-cli help getnodeaddresses` for details.
