...
- [Development guidelines](#development-guidelines)
    - [General Kerrigan](#general-dash-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Kerrigan uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while kerrigand is running with the `logging`
RPC.  For instance, launching kerrigand with `-debug` or `-debug=1` will turn on
...
Kerrigan is a multi-threaded application, and deadlocks or other
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run kerrigand.

You can then use the `-debug=lock` configuration option at kerrigand startup or
`kerrigan-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `kerrigan-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_kerrigan
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_kerrigan --log_level=test_suite
$ valgrind -v --leak-check=full src/kerrigand -printtoconsole
...
# A coverage report will now be accessible at `./test_kerrigan.coverage/index.html`.
```
...
To profile a running kerrigand process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep kerrigand` -- sleep 60
```
...
Kerrigan can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`kerrigand`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Net threads:
  ...
  - [ThreadSocketHandler (`d-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 7120.
...
However, in open source software such as Kerrigan, where everyone uses
...
pay attention to for reviewers of Kerrigan code.
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: Kerrigan uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Kerrigan, in which case
...
a Kerrigan subtree, so that they can be integrated quickly. Cosmetic changes
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `kerrigan-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `kerrigan-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Kerrigan, the wallet was always
