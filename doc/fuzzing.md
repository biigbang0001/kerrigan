# Fuzzing Kerrigan using libFuzzer

## Quickstart guide

To quickly get started fuzzing Kerrigan using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
It is possible to specify `kerrigand` arguments to the `fuzz` executable.
...
Every single pull request submitted against the Kerrigan repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Kerrigan more robust.
...
# Fuzzing Kerrigan using afl++

## Quickstart guide

To quickly get started fuzzing Kerrigan using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Kerrigan using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Kerrigan using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Kerrigan P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Dash
Core without having to write any custom fuzzing harness. The `kerrigand` server
