# Multiprocess Kerrigan

On unix systems, the `--enable-multiprocess` build option can be passed to `./configure` to build new `kerrigan-node`, `kerrigan-wallet`, and `kerrigan-gui` executables alongside existing `kerrigand` and `kerrigan-qt` executables.

`kerrigan-node` is a drop-in replacement for `kerrigand`, and `kerrigan-gui` is a drop-in replacement for `kerrigan-qt`, and there are no differences in use or external behavior between the new and old executables. But internally (after backporting [bitcoin#10102](https://github.com/bitcoin/bitcoin/pull/10102)), `kerrigan-gui` will spawn a `kerrigan-node` process to run P2P and RPC code, communicating with it across a socket pair, and `kerrigan-node` will spawn `kerrigan-wallet` to run wallet code, also communicating over a socket pair. This will let node, wallet, and GUI code run in separate address spaces for better isolation, and allow future improvements like being able to start and stop components independently on different machines and environments.

## Next steps

Specific next steps after backporting [bitcoin#10102](https://github.com/bitcoin/bitcoin/pull/10102) will be:

- [ ] Adding `-ipcbind` and `-ipcconnect` options to `kerrigan-node`, `kerrigan-wallet`, and `kerrigan-gui` executables so they can listen and connect to TCP ports and unix socket paths. This will allow separate processes to be started and stopped any time and connect to each other.
- [ ] Adding `-server` and `-rpcbind` options to the `kerrigan-wallet` executable so wallet processes can handle RPC requests directly without going through the node.
...
make
src/kerrigan-node -regtest -printtoconsole -debug=ipc
DASHD=kerrigan-node test/functional/test_runner.py
```
