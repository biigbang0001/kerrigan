            ...
            CallOneOf(
                ...
                [&] {
                    ...
                    have_tx = orphanage.HaveTx(tx->GetHash());
                    {
                        ...
                        unsigned int nMaxSize = (tx->nType == TRANSACTION_SAPLING)
                            ? MAX_SAPLING_TX_EXTRA_PAYLOAD : MAX_STANDARD_TX_SIZE;
                        Assert(!have_tx == (::GetSerializeSize(*tx, PROTOCOL_VERSION) > nMaxSize));
                        Assert(!have_tx || !add_tx);
                    }
                },
                [&] {
