...
BOOST_AUTO_TEST_CASE(BinaryOperatorTest)
{
    ...
    BOOST_CHECK(b >= a);
    BOOST_CHECK(b >= b);
    // a should be 0.00000002 KRGN/kB now
    a += a;
    BOOST_CHECK(a == b);
}
...
BOOST_AUTO_TEST_CASE(ToStringTest)
{
    CFeeRate feeRate;
    feeRate = CFeeRate(1);
    BOOST_CHECK_EQUAL(feeRate.ToString(), "0.00000001 KRGN/kB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeEstimateMode::KRGN_KB), "0.00000001 KRGN/kB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeEstimateMode::KSAT_B), "0.001 ksat/B");
}
