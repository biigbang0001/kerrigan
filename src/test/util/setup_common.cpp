...
using node::KerriganChainstateSetup;
...
{
    return PeerManager::make(chainparams, connman, *node.addrman, banman, *node.chainman, *node.mempool, *node.mn_metaman,
                             *node.mn_sync, *node.govman, *node.sporkman, *node.chainlocks, *node.clhandler, node.active_ctx, node.dmnman,
...
{
    KerriganChainstateSetup(chainman, *Assert(node.govman.get()), *Assert(node.mn_metaman.get()), *Assert(node.mn_sync.get()),
...
void KerriganChainstateSetupClose(NodeContext& node)
{
...
TestingSetup::~TestingSetup()
{
    ...
    // KerriganChainstateSetup() is called by LoadChainstate() internally but
    // winding them down is our responsibility
    KerriganChainstateSetupClose(m_node);
}
