...
void AddrManImpl::Unserialize(Stream& s_)
{
    ...
    if (nUBuckets > ADDRMAN_NEW_BUCKET_COUNT + 10) { // allow slight format overhead
        throw std::ios_base::failure("AddrMan nUBuckets exceeds safety limit");
    }
    ...
    for (int bucket = 0; bucket < nUBuckets; ++bucket) {
        int num_entries{0};
        s >> num_entries;
        if (num_entries > ADDRMAN_BUCKET_SIZE) {
            throw std::ios_base::failure("AddrMan bucket entry count exceeds safety limit");
        }
