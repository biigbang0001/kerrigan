...
void CNode::CopyStats(CNodeStats& stats) const
{
...
const std::array<std::string, 33> V2_BITCOIN_IDS = {
    ...
    "", /* FEEFILTER is not implemented in Kerrigan */
...
/** List of short messages allocated in Kerrigan's reserved namespace, in order.
 *
 ...
 */
const std::array<std::string, 32> V2_KERRIGAN_IDS = {
    NetMsgType::SPORK,
 ...
 /** Explicit version requirements for Kerrigan v2 short IDs added after baseline.
 *
 ...
 * Messages not in this map are baseline messages available at BIP324_KERRIGAN_BASELINE_VERSION.
 *
 * IMPORTANT:
...
int GetMessageMinVersion(const std::string& message_type)
{
    ...
    return BIP324_KERRIGAN_BASELINE_VERSION;
}

/** A complete set of short IDs
 *
 * Bitcoin takes up short IDs up to 128 (lower half) while Kerrigan can take
...
bool IsValidV2ShortID(uint8_t first_byte) {
    ...
    // and Kerrigan's *used* slots. We do this by checking if the byte is within
    // the range where a valid message is expected to reside.
    return first_byte < std::size(V2_BITCOIN_IDS) ||
           (first_byte >= 128 && static_cast<uint8_t>(first_byte - 128) < std::size(V2_KERRIGAN_IDS));
}
...
std::vector<NodeEvictionCandidate> CConnman::GetEvictionCandidates() const
{
    ...
    for (const CNode* node : m_nodes) {
        ...
        // Always protect verified masternode connections from eviction,
        // not just when this node is itself a masternode. Ensures ChainLock/IS
        // reliability on regular full nodes.
...
{
    std::vector<NodeEvictionCandidate> vEvictionCandidates = GetEvictionCandidates();
...
bool CConnman::AddConnection(const std::string& address, ConnectionType conn_type, bool use_v2transport = false)
{
    ...
    int existing_connections = WITH_READ_LOCK(m_nodes_mutex,
...
{
    size_t nodes_size = WITH_READ_LOCK(m_nodes_mutex, return m_nodes.size(););
...
std::vector<AddedNodeInfo> CConnman::GetAddedNodeInfo(bool include_connected) const
{
    ...
    for (const auto& addr : lAddresses) {
        CService service{MaybeFlipIPv6toCJDNS(LookupNumeric(addr.m_added_node, Params().GetDefaultPort(addr.m_added_node)))};
                                               ...
                                               CMasternodeSync& mn_sync)
{
    ...
    while (!interruptNet)
    {
        ...
        // Hard cap on masternode outbound connections to prevent FD exhaustion
        // during quorum formation. Count current MN outbound connections.
        {
            static constexpr size_t MAX_MN_OUTBOUND = 64;
            size_t nMNOutbound = 0;
            ForEachNode([&](const CNode* pnode) {
                if (pnode->m_masternode_connection && !pnode->IsInboundConn() && !pnode->fDisconnect) {
                    ++nMNOutbound;
                }
            });
            if (nMNOutbound >= MAX_MN_OUTBOUND) {
                LogPrint(BCLog::NET_NETCONN, "CConnman::%s -- at MN outbound cap (%zu), skipping connection to %s\n",
                         _func_, MAX_MN_OUTBOUND, connectToDmn->proTxHash.ToString());
                continue;
            }
        }
...
std::optional<std::pair<CNetMessage, bool>> CNode::PollMessage()
{
    ...
    // Both queues make forward progress while quorum messages get priority
...
{
    {
        READ_LOCK(connman.m_nodes_mutex);
