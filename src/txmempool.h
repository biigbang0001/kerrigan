...
private:
    ...
    /** Sapling nullifiers currently in the mempool. Prevents in-pool double-spend. */
    std::set<uint256> m_sapling_nullifiers GUARDED_BY(cs);
    /** Reverse index: nullifier -> txid of the mempool tx that spends it. */
    std::map<uint256, uint256> m_sapling_nullifier_txids GUARDED_BY(cs);
...
public:
    ...
    void removeSaplingNullifierConflicts(const CTransaction &tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
    void removeForBlock(const std::vector<CTransactionRef>& vtx, unsigned int nBlockHeight) EXCLUSIVE_LOCKS_REQUIRED(cs);
    void removeExpiredAssetUnlock(int nBlockHeight) EXCLUSIVE_LOCKS_REQUIRED(cs);

    /**
     * Sapling nullifier tracking for in-mempool double-spend prevention.
     * All three methods require cs to be held.
     */
    bool CheckSaplingNullifiers(const CTransaction& tx) const EXCLUSIVE_LOCKS_REQUIRED(cs);
    void AddSaplingNullifiers(const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
    void RemoveSaplingNullifiers(const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(cs);
    /** Look up the txid of the mempool tx that spends a given Sapling nullifier, or null hash if none. */
    uint256 GetSaplingNullifierTxid(const uint256& nullifier) const EXCLUSIVE_LOCKS_REQUIRED(cs);
...
private:
    ...
    /**
     * addUnchecked extension for Kerrigan-specific transactions (ProTx).
     */
