...
const char * const BITCOIN_CONF_FILENAME = "kerrigan.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //kerrigan-tx using stdin
...
{
    LOCK(cs_args);
    const auto& ov = m_settings.forced_settings.find(SettingName(strArg));
    if (ov != m_settings.forced_settings.end()) {
        m_settings.forced_settings.erase(ov);
    }

    // Cleanse and erase command-line options so seed phrases don't linger
    // in memory after ForceRemoveArg.
    auto it = m_settings.command_line_options.find(SettingName(strArg));
    if (it != m_settings.command_line_options.end()) {
        for (auto& val : it->second) {
            if (val.isStr()) {
                std::string& s = const_cast<std::string&>(val.getValStr());
                memory_cleanse(s.data(), s.size());
            }
        }
        m_settings.command_line_options.erase(it);
    }

    // Also cleanse config file settings so seed phrases don't linger.
    for (const auto& network : { CBaseChainParams::MAIN, CBaseChainParams::TESTNET, CBaseChainParams::REGTEST, CBaseChainParams::DEVNET }) {
        if (auto* section = util::FindKey(m_settings.ro_config, network)) {
            auto cfg_it = section->find(SettingName(strArg));
            if (cfg_it != section->end()) {
                for (auto& val : cfg_it->second) {
                    if (val.isStr()) {
                        std::string& s = const_cast<std::string&>(val.getValStr());
                        memory_cleanse(s.data(), s.size());
                    }
                }
                section->erase(cfg_it);
            }
        }
    }
}
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Kerrigan
    // macOS: ~/Library/Application Support/Kerrigan
    // Unix-like: ~/.kerrigan
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Kerrigan";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Kerrigan";
#else
    // Unix-like
    return pathRet / ".kerrigan";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty kerrigan.conf if it does not exist
