...
void CDKGSessionManager::UpdatedBlockTip(const CBlockIndex* pindexNew, bool fInitialDownload)
{
    ...
    for (auto& [key, dkgType] : dkgSessionHandlers) {
        // At the start of each new DKG cycle, clear stale pending messages and
        // seen-message hashes from the previous session to prevent unbounded growth.
        const auto& llmq_params_opt = Params().GetLLMQ(key.llmq_type);
        if (llmq_params_opt.has_value() && (pindexNew->nHeight % llmq_params_opt->dkgInterval == 0)) {
            Assert(dkgType)->ClearPendingMessages();
        }
                                                           ...
                                                           CDataStream& vRecv)
{
    ...
    if (msg_type == NetMsgType::QWATCH) {
        ...
        if (pfrom.IsInboundConn() && pfrom.GetVerifiedProRegTxHash().IsNull()) {
            LogPrint(BCLog::LLMQ, "CDKGSessionManager -- qwatch from unverified inbound peer=%d; ignoring\n", pfrom.GetId());
            return MisbehavingError{10, "qwatch from unverified peer"};
        }
        pfrom.qwatch = true;
        return {};
    }
    ...
    for (const auto i : irange::range(members.size())) {
        if (validMembers[i]) {
            ...
            if (it == contributionsCache.end()) {
                ...
                if (vvec_size > 400) { // max quorum size (LLMQ_400_60)
                    throw std::ios_base::failure("vvec_size exceeds max quorum size");
                }
