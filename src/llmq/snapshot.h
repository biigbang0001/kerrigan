...
public:
    ...
    void Unserialize(Stream& s)
    {
        ...
        static constexpr size_t MAX_QUORUM_SNAPSHOT_SIZE = 5000;
        size_t cnt = ReadCompactSize(s);
        if (cnt > MAX_QUORUM_SNAPSHOT_SIZE) throw std::ios_base::failure("activeQuorumMembers too large");
        ReadFixedBitSet(s, activeQuorumMembers, cnt);
        cnt = ReadCompactSize(s);
        if (cnt > MAX_QUORUM_SNAPSHOT_SIZE) throw std::ios_base::failure("mnSkipList too large");
        for ([[maybe_unused]] const auto _ : irange::range(cnt)) {
            int obj;
            s >> obj;
            if (obj < 0) throw std::ios_base::failure("negative mnSkipList entry");
...
public:
    ...
    {
        READWRITE(LIMITED_VECTOR(obj.baseBlockHashes, 32), obj.blockRequestHash, obj.extraShare);
    }
};
...
public:
    ...
    void Unserialize(Stream& s)
    {
        ...
        static constexpr size_t MAX_ROTATION_LIST_SIZE = 100;

        size_t cnt = ReadCompactSize(s);
        if (cnt > MAX_ROTATION_LIST_SIZE) throw std::ios_base::failure("lastCommitmentPerIndex too large");
        for ([[maybe_unused]] const auto _ : irange::range(cnt)) {
            CFinalCommitment qc;
            ::Unserialize(s, qc);
        ...
        cnt = ReadCompactSize(s);
        if (cnt > MAX_ROTATION_LIST_SIZE) throw std::ios_base::failure("quorumSnapshotList too large");
        for ([[maybe_unused]] const auto _ : irange::range(cnt)) {
        ...
        cnt = ReadCompactSize(s);
        if (cnt > MAX_ROTATION_LIST_SIZE) throw std::ios_base::failure("mnListDiffList too large");
        for ([[maybe_unused]] const auto _ : irange::range(cnt)) {
