...
{
    if (quorumMember >= inv.size()) {
        LogPrintf("CSigSharesInv::Set -- quorumMember %u out of range (size %u), ignoring\n",
                  quorumMember, (unsigned)inv.size());
        return;
    }
    inv[quorumMember] = v;
}
...
{
    CSigSharesInv inv;
    // Determine the required size from the maximum member index in the batch
    uint16_t maxMember = 0;
    for (const auto& sigShare : sigShares) {
        maxMember = std::max(maxMember, sigShare.first);
    }
    inv.Init(sigShares.empty() ? 0 : static_cast<size_t>(maxMember) + 1);
    for (const auto& sigShare : sigShares) {
...
static void InitSession(CSigSharesNodeState::Session& s, const llmq::SignHash& signHash, CSigBase from)
{
    ...
    if (!llmq_params_opt.has_value()) {
        throw std::runtime_error(strprintf("InitSession: unknown LLMQ type %d", ToUnderlying(from.getLlmqType())));
    }
...
bool CSigSharesManager::ProcessMessageSigSesAnn(const CNode& pfrom, const CSigSesAnn& ann)
{
    ...
    if (!quorum) {
        // Fail-closed: reject remaining announcements from this batch.
        // A peer referencing unknown quorums is either ahead (benign, rare) or
        // spamming (malicious). Returning false stops batch processing.
    ...
    // Reject new sessions if this node already has too many (DoS protection)
    auto signHash = ann.buildSignHash();
    if (nodeState.sessions.find(signHash.Get()) == nodeState.sessions.end() &&
        nodeState.sessions.size() >= CSigSharesNodeState::MAX_SESSIONS_PER_NODE) {
        LogPrint(BCLog::LLMQ_SIGS, "CSigSharesManager::%s -- too many sessions for node=%d, dropping\n",
                 __func__, pfrom.GetId());
        return true; // drop silently, try other announcements
    }
...
std::shared_ptr<CRecoveredSig> CSigSharesManager::ProcessSigShare(const CSigShare& sigShare, const CQuorumCPtr& quorum)
{
    ...
    bool canTryRecovery = false;
    {
        ...
        for (auto otherNodeId : quorumNodes) {
            auto it = nodeStates.find(otherNodeId);
                                                                ...
                                                                const uint256& msgHash)
{
    ...
    // Verify every recovered signature. On a fresh network with low volume, sporadic verification
    // (e.g. 1-in-100) would effectively skip all checks.
    {
...
{
    if (attempt >= quorum.params.recoveryMembers) {
        throw std::out_of_range(strprintf("SelectMemberForRecovery: attempt %d >= recoveryMembers %d",
                                          attempt, quorum.params.recoveryMembers));
    }
...
void CSigSharesManager::CollectSigSharesToAnnounce(std::unordered_map<NodeId, Uint256HashMap<CSigSharesInv>>& sigSharesToAnnounce)
{
    ...
    sigSharesQueuedToAnnounce.ForEach([this, &quorumNodesMap, &sigSharesToAnnounce](const SigShareKey& sigShareKey,
        ...
        for (const auto& nodeId : quorumNodes) {
            ...
            if (quorumMember >= session.knows.inv.size()) continue;
            ...
            if (quorumMember >= inv.inv.size()) continue;
...
bool CSigSharesManager::GetSessionInfoByRecvId(NodeId nodeId, uint32_t sessionId, CSigSharesNodeState::SessionInfo& retInfo)
{
    ...
    return it->second.GetSessionInfoByRecvId(sessionId, retInfo);
}
