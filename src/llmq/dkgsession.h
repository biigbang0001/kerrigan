...
public:
    ...
    inline void Unserialize(Stream& s)
    {
        ...
        // Cap vvec before allocation to prevent OOM from crafted messages
        size_t nVvecSize = ReadCompactSize(s);
        if (nVvecSize > 400) throw std::ios_base::failure("CDKGContribution vvec too large");
        std::vector<CBLSPublicKey> tmp1(nVvecSize);
        for (size_t i = 0; i < nVvecSize; i++) {
            s >> tmp1[i];
        }

        s >> tmp2;
        if (tmp2.blobs.size() > 400) throw std::ios_base::failure("CDKGContribution contributions too large");
        s >> sig;
...
public:
    ...
    SERIALIZE_METHODS(CDKGComplaint, obj)
    {
        READWRITE(
                ...
                obj.quorumHash,
                obj.proTxHash,
                LIMITED_DYNBITSET(obj.badMembers, 400),
                LIMITED_DYNBITSET(obj.complainForMembers, 400),
                obj.sig
                );
    }
...
public:
    ...
    {
        READWRITE(obj.llmqType, obj.quorumHash, obj.proTxHash, LIMITED_VECTOR(obj.contributions, 400), obj.sig);
    }
...
public:
    SERIALIZE_METHODS(CDKGPrematureCommitment, obj)
    {
        READWRITE(
                ...
                obj.quorumHash,
                obj.proTxHash,
                LIMITED_DYNBITSET(obj.validMembers, 400),
                obj.quorumPublicKey,
                obj.quorumVvecHash,
                obj.quorumSig,
                obj.sig
