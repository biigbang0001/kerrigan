...
{
    if (protx_hash.IsNull() || !secretKeyShare.IsValid()) {
        return false;
    }
    const int memberIdx = GetMemberIndex(protx_hash);
    if (memberIdx < 0) {
        return false;
    }
    LOCK(cs_vvec_shShare);
    if (!HasVerificationVectorInternal() || static_cast<size_t>(memberIdx) >= members.size() || !qc->validMembers[memberIdx]) {
        return false;
    }
    // Compute expected pubkey inline to avoid calling GetPubKeyShare, which
    // would deadlock on cs_vvec_shShare already held above.
    const auto& m = members[memberIdx];
    CBLSPublicKey expectedPubKey = blsCache.BuildPubKeyShare(m->proTxHash, quorumVvec, CBLSId(m->proTxHash));
    if (secretKeyShare.GetPublicKey() != expectedPubKey) {
        return false;
    }
