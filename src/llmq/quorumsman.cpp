...
// For qwatch peers, GetVerifiedProRegTxHash() returns null.  Using a null hash
// as the dedup key causes cross-peer collisions.  This helper derives a
// deterministic, collision-free uint256 from the peer's node id when needed.
static uint256 GetPeerDedupHash(const CNode& peer)
{
    uint256 proRegTx = peer.GetVerifiedProRegTxHash();
    if (proRegTx.IsNull()) {
        auto nodeId = static_cast<int64_t>(peer.GetId());
        std::memcpy(proRegTx.begin(), &nodeId, sizeof(nodeId));
        // Fill remaining bytes with 0xFF so the synthetic key can never collide
        // with a real 256-bit proRegTx hash (which would require the top 24
        // bytes to all be 0xFF, which is astronomically unlikely).
        std::memset(proRegTx.begin() + sizeof(nodeId), 0xFF, uint256::size() - sizeof(nodeId));
    }
    return proRegTx;
}
                                       ...
                                       const uint256& proTxHash) const
{
    ...
    const CQuorumDataRequestKey key(GetPeerDedupHash(*pfrom), true, pindex->GetBlockHash(), llmqType);
...
MessageProcessingResult CQuorumManager::ProcessMessage(CNode& pfrom, CConnman& connman, std::string_view msg_type, CDataStream& vRecv)
{
    if (msg_type == NetMsgType::QGETDATA) {
        ...
        bool request_limit_exceeded(false);
        {
            ...
            const CQuorumDataRequestKey key(GetPeerDedupHash(pfrom), false, request.GetQuorumHash(), request.GetLLMQType());
    ...
    if (msg_type == NetMsgType::QDATA) {
        ...
        {
            LOCK2(::cs_main, cs_data_requests);
            const CQuorumDataRequestKey key(GetPeerDedupHash(pfrom), true, request.GetQuorumHash(), request.GetLLMQType());
            auto it = mapQuorumDataRequests.find(key);
            if (it == mapQuorumDataRequests.end()) {
        ...
        if (request.GetDataMask() & CQuorumDataRequest::QUORUM_VERIFICATION_VECTOR) {
            ...
            // Pre-check vector size before allocating to prevent unbounded deserialization
            uint64_t nVecSize = ReadCompactSize(vRecv);
            if (nVecSize > 400) {
                return MisbehavingError{10, "oversized quorum verification vector"};
            }
            std::vector<CBLSPublicKey> verificationVector(nVecSize);
            for (auto& pk : verificationVector) {
                vRecv >> pk;
            }
...
void CQuorumManager::MigrateOldQuorumDB(CEvoDB& evoDb) const
{
    ...
    if (!evoDb.CommitRootTransaction()) {
        LogPrintf("CQuorumManager::%s -- Failed to commit evoDb cleanup\n", __func__);
    }
