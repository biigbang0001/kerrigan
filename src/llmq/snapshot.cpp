...
{
    // Copy into a local vector to avoid mutating the caller's data through the Span
    std::vector<const CBlockIndex*> sorted(baseBlockIndexes.begin(), baseBlockIndexes.end());
    if (!use_legacy_construction) {
    ...
    for (const auto baseBlock : sorted) {
