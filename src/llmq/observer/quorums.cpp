...
void QuorumObserver::StartVvecSyncThread(gsl::not_null<const CBlockIndex*> block_index, CQuorumCPtr pQuorum) const
{
    ...
    // Keep a reference before moving into the lambda so we can reset the flag
    // on enqueue failure.
    auto pQuorumRef = pQuorum;
    auto fut = workerPool.push([pQuorum = std::move(pQuorum), block_index, this](int threadId) mutable {
    ...
    if (!fut.valid()) {
        // Worker pool queue was full; reset the flag so a future attempt can retry.
        pQuorumRef->fQuorumDataRecoveryThreadRunning = false;
        LogPrint(BCLog::LLMQ, "QuorumObserver::%s -- worker pool saturated, vvec sync task dropped\n", __func__);
    }
}
...
void QuorumObserver::StartCleanupOldQuorumDataThread(gsl::not_null<const CBlockIndex*> pIndex) const
{
    ...
    //
    // Allow a retry window of 10 blocks after the trigger height so that
    // transient worker-pool saturation doesn't skip an entire cycle.
    const int offset = pIndex->nHeight % 576;
    if (offset < 58 || offset > 67) {
        return;
    }
    // Only the trigger block (58) or a retry block if the previous attempt was missed
    if (offset > 58 && !fCleanupMissed) {
        return;
    }
    ...
    auto fut = workerPool.push([pIndex, t, this](int threadId) {
    ...
    if (!fut.valid()) {
        fCleanupMissed = true;
        LogPrint(BCLog::LLMQ, "QuorumObserver::%s -- worker pool saturated, will retry in next blocks\n", __func__);
    } else {
        fCleanupMissed = false;
    }
}
} // namespace llmq
