...
bool CDKGSession::PreVerifyMessage(const CDKGComplaint& qc, bool& retBan) const
{
    ...
    for (const auto i : irange::range(members.size(), size_t(params.size))) {
        if (qc.badMembers[i] || qc.complainForMembers[i]) {
            retBan = true;
            logger.Batch("invalid complaint bitset. bit %d should not be set", i);
            return false;
        }
    }
...
std::optional<CInv> CDKGSession::ReceiveMessage(const CDKGJustification& qj)
{
    ...
    for (const auto& [index, skContribution] : qj.contributions) {
        ...
        if (!member2) {
            logger.Batch("GetMemberAtIndex(%u) returned null in justification handling", (unsigned)index);
            continue;
        }
    ...
    // If we never received a contribution from this member (observer nodes
    // don't mark non-contributors as bad), the vvec is null. Skip verification
    // and treat the justification as invalid to avoid a null dereference.
    if (!receivedVvecs[member->idx]) {
        logger.Batch("no vvec for %s, skipping justification verification", member->dmn->proTxHash.ToString());
        MarkBadMember(member->idx);
        return inv;
    }
    ...
    for (const auto& [index, skContribution] : qj.contributions) {
        const auto* member2 = GetMemberAtIndex(index);
        if (!member2) {
            logger.Batch("GetMemberAtIndex(%u) returned null during async verify setup", (unsigned)index);
            continue;
        }
    ...
    for (const auto& [index, skContribution] : qj.contributions) {
        const auto* member2 = GetMemberAtIndex(index);
        if (!member2) {
            logger.Batch("GetMemberAtIndex(%u) returned null during verify result processing", (unsigned)index);
            continue;
        }
        ...
        if (fut.valid()) {
            try {
                ...
                // Worker pool saturation, treat as verification failure
                LogPrint(BCLog::LLMQ, "CDKGSession::%s -- future.get() failed for %s: %s\n",
                         __func__, member2->dmn->proTxHash.ToString(), e.what());
            }
        } else {
            // Enqueue failed (pool saturated), treat as unverified
            LogPrint(BCLog::LLMQ, "CDKGSession::%s -- async verify enqueue failed for %s\n",
                     __func__, member2->dmn->proTxHash.ToString());
        }
        if (!result) {
