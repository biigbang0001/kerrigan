...
std::string GetCrashInfoStrFromSerializedStr(const std::string& ciStr)
{
    ...
    if (hdr.magic != "KerriganCrashInfo") {
...
#if !defined(WIN32)
// Uses non-async-signal-safe calls (backtrace, logging, heap) for best-effort
// crash diagnostics. Emits a minimal write(2) preamble first so the signal
// number reaches stderr even if the process deadlocks in the unsafe portion.
static void HandlePosixSignal(int s)
{
    ...
    // Async-signal-safe preamble: write signal number to stderr using only
    // write(2) and stack-local buffers only, no heap, no mutex, no stdio.
    {
        const char prefix[] = "\n\n*** Fatal signal ";
        const char suffix[] = " received ***\n";
        char numbuf[12];
        int num = s;
        // Convert signal number to decimal string (async-signal-safe)
        int pos = sizeof(numbuf);
        bool neg = (num < 0);
        if (neg) num = -num;
        do {
            numbuf[--pos] = '0' + (num % 10);
        ...
        if (neg) numbuf[--pos] = '-';
        // Ignore write() return; nothing we can do on failure from a signal handler
        (void)write(STDERR_FILENO, prefix, sizeof(prefix) - 1);
        (void)write(STDERR_FILENO, numbuf + pos, sizeof(numbuf) - pos);
        (void)write(STDERR_FILENO, suffix, sizeof(suffix) - 1);
    }

    // Best-effort diagnostics below. These calls are NOT async-signal-safe
    // and may deadlock.  The write(2) above ensures we at least logged the
    // signal number before we attempt anything risky.
