...
static bool HTTPReq_JSONRPC(const CoreContext& context, HTTPRequest* req)
{
    ...
    try {
        ...
        // Set the URI and local port (for per-algo RPC routing)
        jreq.URI = req->GetURI();
        jreq.localPort = req->GetLocalPort();
        ...
        // array of requests
            ...
            if (valRequest.size() > 1000) {
                throw JSONRPCError(RPC_INVALID_REQUEST, "Batch too large (max 1000 calls)");
            }
