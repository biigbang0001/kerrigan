...
#include <primitives/block.h> // GetAlgoByName, NUM_ALGOS
...
/** Per-algo RPC port mapping (populated from -rpcalgoport) */
std::map<uint16_t, int> g_rpc_algo_ports;
...
static bool HTTPBindAddresses(struct evhttp* http)
{
    ...
    // Bind per-algo RPC ports (-rpcalgoport=algo:port)
    // Allows pool software to get algo-specific block templates without passing
    // the "algo" parameter; just connect to the right port.
    g_rpc_algo_ports.clear();
    for (const std::string& strAlgoPort : gArgs.GetArgs("-rpcalgoport")) {
        auto pos = strAlgoPort.find(':');
        if (pos == std::string::npos) {
            LogPrintf("Invalid -rpcalgoport format (expected algo:port): %s\n", strAlgoPort);
            continue;
        }
        std::string algoName = strAlgoPort.substr(0, pos);
        uint16_t port = static_cast<uint16_t>(LocaleIndependentAtoi<int64_t>(strAlgoPort.substr(pos + 1)));
        int algo = GetAlgoByName(algoName, -1);
        if (algo < 0 || algo >= NUM_ALGOS) {
            LogPrintf("Invalid algorithm in -rpcalgoport: %s (use: x11, kawpow, equihash200, equihash192)\n", algoName);
            continue;
        }
        if (port == 0) {
            LogPrintf("Invalid port in -rpcalgoport: %s\n", strAlgoPort);
            continue;
        }
        // Bind algo ports to the same addresses as the main RPC port,
        // so Docker and remote pool setups work correctly.
        bool algoBound = false;
        for (const auto& ep : endpoints) {
            evhttp_bound_socket* bind_handle = evhttp_bind_socket_with_handle(http,
                ep.first.empty() ? nullptr : ep.first.c_str(), port);
            if (bind_handle) {
                boundSockets.push_back(bind_handle);
                algoBound = true;
                LogPrintf("Binding RPC algo %s on %s:%i\n", algoName,
                          ep.first.empty() ? "0.0.0.0" : ep.first, port);
            }
        }
        if (algoBound) {
            g_rpc_algo_ports[port] = algo;
        }
    }
...
uint16_t HTTPRequest::GetLocalPort() const
{
    evhttp_connection* con = evhttp_request_get_connection(req);
    if (!con) return 0;
