...
[[maybe_unused]] static constexpr std::array<CSporkDef, 8> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_19_CHAINLOCKS_ENABLED,            4070908800ULL), // OFF, needs LLMQ
...
public:
    ...
    {
        READWRITE(obj.nSporkID, obj.nValue, obj.nTimeSigned, LIMITED_VECTOR(obj.vchSig, 128));
    }
...
public:
    ...
    void Serialize(Stream &s) const EXCLUSIVE_LOCKS_REQUIRED(!cs)
    {
        ...
        // not reuse pubkeys from previous kerrigand run.
    ...
    void Unserialize(Stream &s) EXCLUSIVE_LOCKS_REQUIRED(!cs)
    {
        ...
        // Reject corrupt cache: there are ~25 defined sporks and a small
        // number of authorized signers.
        static constexpr size_t MAX_SPORK_ENTRIES = 100;
        static constexpr size_t MAX_SIGNERS_PER_SPORK = 100;
        if (mapSporksByHash.size() > MAX_SPORK_ENTRIES) {
            throw std::ios_base::failure("SporkStore hash map exceeds safety limit");
        }
        if (mapSporksActive.size() > MAX_SPORK_ENTRIES) {
            throw std::ios_base::failure("SporkStore active map exceeds safety limit");
        }
        for (const auto& [id, signerMap] : mapSporksActive) {
            if (signerMap.size() > MAX_SIGNERS_PER_SPORK) {
                throw std::ios_base::failure("SporkStore per-spork signer count exceeds safety limit");
            }
        }
    }
...
/** Global read-only pointer to the spork manager, set during init.
 *  Used by subsystems (e.g. HMP in validation) that lack a direct reference. */
extern const CSporkManager* g_sporkman;
