...
public:
    ...
    //! algo-specific header fields (Equihash / KawPoW)
    uint256 hashReserved;                   //!< Equihash: 32 zero bytes (Zcash compat: Sapling root position)
    uint256 nNonce256;                      //!< Equihash: 32-byte mining nonce
    std::vector<unsigned char> nSolution;   //!< Equihash solution
    uint32_t nKawPowHeight{0};              //!< KawPoW DAG epoch height (distinct from chain nHeight)
    uint64_t nNonce64{0};                   //!< KawPoW 64-bit nonce
    uint256 mix_hash;                       //!< KawPoW mix hash

    //! (memory only) Miner's HMP identity pubkey from CCbTx v4 (48 bytes, zero if pre-HMP)
    CBLSPublicKey hmpMinerPubKey;

    //! (memory only) Seal quality score for this block (basis points, 10000 = 1.0x, max 19500 = 1.95x)
    uint64_t nSealWeight{10000};

    //! (memory only) Cumulative PoW * seal_multiplier up to and including this block
    arith_uint256 nChainSealWork{};
    ...
    //! Get mining algorithm for this block from version bits
    int GetAlgo() const
    {
        ...
        // Return ALGO_INVALID (consistent with CBlockHeader::GetAlgo).
        // Silently defaulting to X11 masks corrupted block data.
        LogPrintf("WARNING: CBlockIndex::GetAlgo(): unmapped algo bits 0x%03x in nVersion=0x%08x at height %d\n",
                  nVersion & BLOCK_VERSION_ALGO, nVersion, nHeight);
        return ALGO_INVALID;
    }
...
public:
    ...
    SERIALIZE_METHODS(CDiskBlockIndex, obj)
    {
        ...
        // algo-specific header fields (requires reindex on upgrade)
        int algo = obj.GetAlgo();
        if (IsEquihash(algo)) {
            READWRITE(obj.hashReserved, obj.nNonce256);
            READWRITE(LIMITED_VECTOR(obj.nSolution, 1400));
        } else if (algo == ALGO_KAWPOW) {
            READWRITE(obj.nKawPowHeight, obj.nNonce64, obj.mix_hash);
        }

        // HMP seal weight (persisted so nChainSealWork can be reconstructed during IBD)
        READWRITE(VARINT(obj.nSealWeight));
    }
