...
MAKE_MSG(SEALSHARE, "sealshare");
MAKE_MSG(SEALASM, "sealasm");
MAKE_MSG(PUBKEYCOMMIT, "pkcommit");
}; // namespace NetMsgType
...
const static std::string allNetMessageTypes[] = {
    ...
    // Hivemind HMP
    NetMsgType::SEALSHARE,
    NetMsgType::SEALASM,
    NetMsgType::PUBKEYCOMMIT,
};
const static std::vector<std::string> allNetMessageTypesVec(std::begin(allNetMessageTypes), std::end(allNetMessageTypes));

/** Message types that are not allowed by blocks-relay-only policy.
 *  We do not want DKG or LLMQ signing messages to be relayed
