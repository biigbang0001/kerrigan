                                   ...
                                   const chainlock::Chainlocks& chainlocks, BlockValidationState& state)
{
    ...
    if (cbTx.bestCLSignature.IsValid()) {
        int curBlockCoinbaseCLHeight = static_cast<int>(pindex->nHeight) - static_cast<int>(cbTx.bestCLHeightDiff) - 1;
        if (curBlockCoinbaseCLHeight < 0) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cbtx-clheight");
        }
    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // Sapling transactions have their own activation gate (SaplingHeight),
    // so they bypass the DIP0003 requirement. All other special txs need DIP0003.
    if (tx.nType == TRANSACTION_SAPLING) {
        int nHeight = pindexPrev ? pindexPrev->nHeight + 1 : 0;
        return sapling::CheckSaplingTx(tx, chainman.GetConsensus(), nHeight, state, check_sigs);
    }
                                                ...
                                                CDeterministicMNList& mnListRet)
{
    ...
    if (auto dmn = payee ? newList.GetMN(payee->proTxHash) : nullptr) {
        ...
        // Starting from v19 and until MNRewardReallocation, BroodNodes will be paid 4 blocks in a row
        // No need to check if v19 is active, since BroodNode ProRegTxes are allowed only after v19 activation
        ...
        if (dmn->nType == MnType::Evo && !isMNRewardReallocation) {
            ...
            if (debugLogs) {
                LogPrint(BCLog::MNPAYMENTS, "%s -- MN %s is a BroodNode, bumping nConsecutivePayments to %d\n", __func__,
    ...
    // reset nConsecutivePayments on non-paid BroodNodes
        ...
        if (!fVerifyOnly) {
            ...
            if (sapling::IsSaplingActive(m_consensus_params, pindex->nHeight)) {
                if (!m_sapling_state.VerifyBestBlock(pindex->GetBlockHash())) {
                    int saplingHeight = m_sapling_state.GetBestBlockHeight();
                    if (saplingHeight <= pindex->nHeight) {
                        LogPrintf("ERROR: Sapling state inconsistency detected at height %d, reindex required\n", pindex->nHeight);
                        return false;
                    }
                }
            }
...
static bool CheckService(const ProTx& proTx, TxValidationState& state)
{
    ...
    case NetInfoStatus::Duplicate:
        return state.Invalid(TxValidationResult::TX_BAD_SPECIAL, "bad-protx-netinfo-duplicate");
    case NetInfoStatus::MaxLimit:
        return state.Invalid(TxValidationResult::TX_BAD_SPECIAL, "bad-protx-netinfo-max-limit");
...
// Enforce version upgrade rules after v24 activation
                                 ...
                                 const ChainstateManager& chainman, TxValidationState& state)
{
    ...
    if (state_version == ProTxVersion::LegacyBLS && tx_version > ProTxVersion::BasicBLS) {
        // Must upgrade to BasicBLS before ExtAddr
    ...
    if (tx_type != TRANSACTION_PROVIDER_UPDATE_SERVICE && tx_version == ProTxVersion::ExtAddr) {
        // Only ProRegTx and ProUpServTx can use ExtAddr
                   ...
                   TxValidationState& state, bool check_sigs)
{
    ...
    if (pindexPrev) {
        ...
        // never allow duplicate platformNodeIds for BroodNodes
        if (opt_ptx->nType == MnType::Evo) {
                      ...
                      const ChainstateManager& chainman, TxValidationState& state, bool check_sigs)
{
    ...
    if (opt_ptx->nType != dmn->nType) {
        return state.Invalid(TxValidationResult::TX_BAD_SPECIAL, "bad-protx-type-mismatch");
    }
    ...
    // don't allow updating to platformNodeIds already used by other BroodNodes
    if (opt_ptx->nType == MnType::Evo) {
        if (mnList.HasUniqueProperty(opt_ptx->platformNodeID) &&
