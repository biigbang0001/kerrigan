...
const std::map<std::string, RPCResult> RPCRESULT_MAP{{
    ...
    RESULT_MAP_ENTRY("collateralAddress", RPCResult::Type::STR, "Kerrigan address used for collateral"),
    ...
    RESULT_MAP_ENTRY("operatorPayoutAddress", RPCResult::Type::STR, "Kerrigan address used for operator reward payments"),
    ...
    RESULT_MAP_ENTRY("ownerAddress", RPCResult::Type::STR, "Kerrigan address used for payee updates and proposal voting"),
    RESULT_MAP_ENTRY("payoutAddress", RPCResult::Type::STR, "Kerrigan address used for masternode reward payments"),
    ...
    RESULT_MAP_ENTRY("votingAddress", RPCResult::Type::STR, "Kerrigan address used for voting"),
}};
...
RPCResult CAssetLockPayload::GetJsonHelp(const std::string& key, bool optional)
{
    return {RPCResult::Type::OBJ, key, optional, key.empty() ? "" : "The asset lock special transaction",
    {
        ...
        {RPCResult::Type::ARR, "creditOutputs", "", {
            {RPCResult::Type::OBJ, "", "", {
                {RPCResult::Type::NUM, "value", "The value in KRGN"},
                {RPCResult::Type::NUM, "valueSat", "The value in satoshis"},
...
RPCResult CAssetUnlockPayload::GetJsonHelp(const std::string& key, bool optional)
{
    return {RPCResult::Type::OBJ, key, optional, key.empty() ? "" : "The asset unlock special transaction",
    {
        ...
        {RPCResult::Type::NUM, "fee", "Transaction fee in satoshis awarded to the miner"},
...
RPCResult CCbTx::GetJsonHelp(const std::string& key, bool optional)
{
    return {RPCResult::Type::OBJ, key, optional, key.empty() ? "" : "The coinbase special transaction",
    {
        ...
        {RPCResult::Type::STR_HEX, "minerIdentity", /*optional=*/true, "BLS public key of the block's miner (HMP)"},
        {RPCResult::Type::STR_HEX, "sealForAncestor", /*optional=*/true, "Assembled HMP seal signature for ancestor block"},
        {RPCResult::Type::STR_HEX, "sealBlockHash", /*optional=*/true, "Hash of the block the seal covers"},
        {RPCResult::Type::ARR, "vSealSigners", /*optional=*/true, "BLS public keys of seal signers",
            {{RPCResult::Type::STR_HEX, "", "BLS public key"}}},
        {RPCResult::Type::ARR, "vSealSignerAlgos", /*optional=*/true, "Mining algorithm domain per signer",
            {{RPCResult::Type::NUM, "", "Algorithm identifier"}}},
        {RPCResult::Type::ARR, "vCommitments", /*optional=*/true, "HMP pubkey commitments (v5+)",
            {{RPCResult::Type::STR_HEX, "", "BLS public key commitment"}}},
    }};
}
...
UniValue CCbTx::ToJson() const
{
    ...
    if (nVersion >= CCbTx::Version::MERKLE_ROOT_QUORUMS) {
        ...
        if (nVersion >= CCbTx::Version::CLSIG_AND_BALANCE) {
            ...
            if (nVersion >= CCbTx::Version::HMP_SEAL) {
                ret.pushKV("minerIdentity", minerIdentity.ToString());
                ret.pushKV("sealForAncestor", sealForAncestor.ToString());
                ret.pushKV("sealBlockHash", sealBlockHash.ToString());
                UniValue signers(UniValue::VARR);
                for (const auto& signer : vSealSigners) {
                    signers.push_back(signer.ToString());
                }
                ret.pushKV("vSealSigners", signers);
                UniValue algos(UniValue::VARR);
                for (const auto& algo : vSealSignerAlgos) {
                    algos.push_back(static_cast<int>(algo));
                }
                ret.pushKV("vSealSignerAlgos", algos);
                if (nVersion >= CCbTx::Version::HMP_COMMITMENT) {
                    UniValue commitments(UniValue::VARR);
                    for (const auto& commitment : vCommitments) {
                        commitments.push_back(commitment.ToString());
                    }
                    ret.pushKV("vCommitments", commitments);
                }
            }
        }
    }
