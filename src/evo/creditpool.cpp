...
// Avoids circular dependency through masternode/payments.h
...
static bool GetDataFromUnlockTx(const CTransaction& tx, CAmount& toUnlock, uint64_t& index, TxValidationState& state)
{
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        if (!MoneyRange(toUnlock)) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-assetunlocktx-amount-out-of-range");
        }
    }
    return true;
}
...
CCreditPool CCreditPoolManager::ConstructCreditPool(const gsl::not_null<const CBlockIndex*> block_index, CCreditPool prev)
{
    ...
    if (!opt_block_data) {
        // No block data but prev has state -- should not happen
        if (prev.locked != 0) {
    ...
    // Clamp to zero: distantUnlocked can exceed the sum during edge-case reorgs
    const CAmount latelyUnlocked = std::max(CAmount(0), prev.latelyUnlocked + blockData.unlocked - distantUnlocked);
    if (DeploymentActiveAt(*block_index, m_chainman, Consensus::DEPLOYMENT_V24)) {
        ...
        currentLimit = std::min(currentLimit, std::max(CAmount(0), LimitAmountHigh - latelyUnlocked));
    }
