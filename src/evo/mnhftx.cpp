...
bool CMNHFManager::UndoBlock(const CBlock& block, const CBlockIndex* const pindex)
{
    ...
    for (const auto& versionBit : excluded_signals) {
        ...
        // Runtime checks instead of assert (compiled out in release builds)
        if (signals.find(versionBit) == signals.end()) {
            LogPrintf("CMNHFManager::%s: versionBit %d not found in signals for block %s\n",
                      __func__, versionBit, pindex->GetBlockHash().ToString());
            return false;
        }
        if (!Params().IsValidMNActivation(versionBit, pindex->GetMedianTimePast())) {
            LogPrintf("CMNHFManager::%s: versionBit %d is not a valid MN activation for block %s\n",
                      __func__, versionBit, pindex->GetBlockHash().ToString());
            return false;
        }
    }
...
std::optional<CMNHFManager::Signals> CMNHFManager::GetFromCache(const CBlockIndex* const pindex)
{
    ...
    // TODO: remove -- only needed because versionbits_tests lets phashBlock be nullptr
...
bool CMNHFManager::ForceSignalDBUpdate()
{
    ...
    {
        bls::BLSSchemeGuard guard; // restores original scheme on scope exit
        bls::bls_legacy_scheme.store(false);
        GetSignalsStage(m_chainman.ActiveTip());
    } // guard restores original bls_legacy_scheme value
