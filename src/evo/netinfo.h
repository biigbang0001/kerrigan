...
public:
    ...
    {
        // Max FQDN length is 253 per RFC 1035; bounded to prevent OOM on crafted msgs
        READWRITE(LIMITED_STRING(obj.m_addr, 253));
...
private:
    // operator== and operator!= are defined by the parent which then call the child's IsEqual() override
...
public:
    ...
    void Unserialize(Stream& s)
    {
        ...
        if (nSize > MAX_ENTRIES_EXTNETINFO) {
            throw std::ios_base::failure("ExtNetInfo map too large");
        }
        m_data.clear();
        for (size_t i = 0; i < nSize; i++) {
            ...
            NetInfoList value;
            s >> value;
            if (value.size() > MAX_ENTRIES_EXTNETINFO) {
                throw std::ios_base::failure("ExtNetInfo entry list too large");
            }
