...
public:
    ...
    SERIALIZE_METHODS(CDeterministicMN, obj)
    {
        ...
        // Disk uses CLIENT_VERSION; network uses protocol versioning
...
private:
    ...
    // Lazy SML cache, invalidated by AddMN/RemoveMN/UpdateMN
...
public:
    ...
    // Compare lists ignoring non-deterministic members (nTotalRegisteredCount, internalId)
...
public:
    ...
    void Serialize(Stream& s) const
    {
        ...
        std::vector<std::pair<uint64_t, CDeterministicMNStateDiff>> sortedUpdated(
            updatedMNs.begin(), updatedMNs.end());
        std::sort(sortedUpdated.begin(), sortedUpdated.end(),
            [](const auto& a, const auto& b) { return a.first < b.first; });
        WriteCompactSize(s, sortedUpdated.size());
        for (const auto& [internalId, pdmnState] : sortedUpdated) {
...
private:
    ...
    {
        // Cap per-field entries well above any realistic MN list size.
        static constexpr size_t MAX_MN_DIFF_ENTRIES = 4096;
        ...
        size_t nAddedSize = ReadCompactSize(s);
        if (nAddedSize > MAX_MN_DIFF_ENTRIES) {
            throw std::ios_base::failure("CDeterministicMNListDiff::addedMNs size exceeds limit");
        }
        for (size_t to_read = nAddedSize; to_read > 0; --to_read) {
            addedMNs.push_back(std::make_shared<CDeterministicMN>(deserialize, s));
        }

        size_t nUpdatedSize = ReadCompactSize(s);
        if (nUpdatedSize > MAX_MN_DIFF_ENTRIES) {
            throw std::ios_base::failure("CDeterministicMNListDiff::updatedMNs size exceeds limit");
        }
        for (size_t to_read = nUpdatedSize; to_read > 0; --to_read) {
        ...
        size_t nRemovedSize = ReadCompactSize(s);
        if (nRemovedSize > MAX_MN_DIFF_ENTRIES) {
            throw std::ios_base::failure("CDeterministicMNListDiff::removedMns size exceeds limit");
        }
        for (size_t to_read = nRemovedSize; to_read > 0; --to_read) {
            uint64_t internalId = ReadVarInt<Stream, VarIntMode::DEFAULT, uint64_t>(s);
...
private:
    ...
    // Methods for RecalculateAndRepairDiffs
