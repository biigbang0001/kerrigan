...
// Track executed escrow release proposals (key: "E_exec" + hash -> height)
inline std::string EscrowExecKey(const uint256& hash)
{
    std::string key = "E_exec";
    key.append(reinterpret_cast<const char*>(hash.begin()), hash.size());
    return key;
}

inline bool IsEscrowReleaseExecuted(CEvoDB& evodb, const uint256& hash)
{
    int32_t height;
    return evodb.Read(EscrowExecKey(hash), height);
}

inline void MarkEscrowReleaseExecuted(CEvoDB& evodb, const uint256& hash, int height)
{
    evodb.Write(EscrowExecKey(hash), static_cast<int32_t>(height));
}

inline void UnmarkEscrowReleaseExecuted(CEvoDB& evodb, const uint256& hash)
{
    evodb.Erase(EscrowExecKey(hash));
}
