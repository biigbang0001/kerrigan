...
public:
    ...
    //! Get shielded (Sapling) balance.
    virtual CAmount getShieldedBalance() = 0;

    //! Generate a new Sapling shielded payment address.
    virtual std::string getNewSaplingAddress() = 0;

    //! List all Sapling shielded addresses in the wallet.
    virtual std::vector<std::string> listSaplingAddresses() = 0;

    //! Info about a single shielded note, for the Qt history view.
    struct ShieldedNoteInfo {
        uint256 cmu;
        CAmount value;
        std::string address;
        std::string memo;
    ...
    //! Get all shielded notes (spent + unspent) for the history view.
    virtual std::vector<ShieldedNoteInfo> getShieldedNoteHistory() = 0;

    //! Send a Sapling z_sendmany transaction. Returns txid on success, throws on error.
    virtual std::string saplingZSendMany(const std::string& from_address,
        const std::vector<std::pair<std::string, CAmount>>& recipients,
        int minconf = 1,
        std::optional<CAmount> fee = std::nullopt) = 0;
