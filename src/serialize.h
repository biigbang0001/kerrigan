...
/**
 * Fixed-size bit set: packed as ceil(size/8) bytes, LSB-first within each byte.
 * The caller specifies the bit count; no length prefix is written on the wire.
 */
...
/**
 * Dynamic bit set: a CompactSize length prefix (bit count) followed by
 * the FixedBitSet payload: ceil(n/8) packed bytes, LSB-first per byte.
 */
...
struct LimitedDynamicBitSetFormatter
{
    ...
    void Unser(Stream& s, std::vector<bool>& vec)
    {
        ...
        if (size > Limit) {
            throw std::ios_base::failure("Dynamic bitset length limit exceeded");
        }
        ReadFixedBitSet(s, vec, size);
    }
};
...
{
    // Optimized byte-vector specialization
    template<typename Stream>
    void Unser(Stream& s, std::vector<uint8_t>& v)
    {
        size_t size = ReadCompactSize(s);
        if (size > Limit) {
    ...
    template<typename Stream>
    void Ser(Stream& s, const std::vector<uint8_t>& v)
    {
        WriteCompactSize(s, v.size());
        if (!v.empty()) s.write(MakeByteSpan(v));
    }

    // Generic element-by-element serialization for non-byte vectors
    template<typename Stream, typename T, typename = std::enable_if_t<!std::is_same_v<T, uint8_t>>>
    void Unser(Stream& s, std::vector<T>& v)
    {
        size_t size = ReadCompactSize(s);
        if (size > Limit) {
            throw std::ios_base::failure("Vector length limit exceeded");
    ...
    template<typename Stream, typename T, typename = std::enable_if_t<!std::is_same_v<T, uint8_t>>>
    void Ser(Stream& s, const std::vector<T>& v)
    {
        WriteCompactSize(s, v.size());
        for (const auto& elem : v) s << elem;
    }
};

#define LIMITED_VECTOR(obj,n) Using<LimitedVectorFormatter<n>>(obj)

/** Bounded std::list deserialization (analogous to LimitedVectorFormatter).
 *  Checks compact-size prefix before allocating elements.
 */
template<size_t Limit>
struct LimitedListFormatter
{
    template<typename Stream, typename T>
    void Unser(Stream& s, std::list<T>& v)
    {
        size_t size = ReadCompactSize(s);
        if (size > Limit) {
            throw std::ios_base::failure("List length limit exceeded");
        }
        v.clear();
    ...
    template<typename Stream, typename T>
    void Ser(Stream& s, const std::list<T>& v)
    {
        WriteCompactSize(s, v.size());
        for (const auto& elem : v) Serialize(s, elem);
    }
};

#define LIMITED_LIST(obj,n) Using<LimitedListFormatter<n>>(obj)
...
public:
    ...
    /** CXX Rust FFI bridge method */
    void write_u8(const uint8_t* pch, size_t nSize) { write(MakeByteSpan(Span{pch, nSize})); }
};
