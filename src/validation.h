...
// Conservative limit for HEADERS2 (compressed) messages.
// Kerrigan has multi-algo headers; Equihash 200,9 headers are ~1427 bytes each
// (80-byte base + 2-byte varint + 1344-byte solution).  With a 3MB P2P message
// cap, 8000 headers x 1427 bytes = ~10.9 MB, far over the limit.  Use 2000 so
// the worst-case batch stays under 3MB regardless of algo mix.
static const unsigned int MAX_HEADERS_COMPRESSED_RESULT = 2000;
...
/**
 * Kerrigan block subsidy: fixed 25 COIN with Bitcoin-style halvings.
 * GetBlockSubsidyInner() computes subsidy for the block AFTER nPrevHeight.
 * GetSuperblockSubsidyInner() always returns 0 (no superblocks in Kerrigan).
 * GetMasternodePayment() returns 20% of blockValue (masternode share).
 */
