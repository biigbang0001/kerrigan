    ...
    while (!m_shutdown) {
        ...
        if (m_connman) {
            for (const auto& [seal, height] : relays) {
                ...
                LogPrint(BCLog::HMP, "HMP: broadcast assembled seal for block %s height %d\n",
...
bool CSealManager::TryAssemble(CSealSession& session, std::optional<std::pair<CAssembledSeal, int>>& relayOut)
{
    ...
    LogPrint(BCLog::HMP, "HMP: assembled seal for block %s with %zu signers\n",
...
void CSealManager::OnNewBlock(const uint256& blockHash, int height, const uint256& prevSealHash)
{
    ...
    LogPrint(BCLog::HMP, "HMP: started seal collection for block %s at height %d\n",
...
HMPAcceptResult CSealManager::AddSealShare(const CSealShare& share)
{
    ...
    int sessionBlockHeight = 0;
    {
        ...
        if (it == m_sessions.end()) {
            LogPrint(BCLog::HMP, "HMP: ignoring share for unknown block %s\n",
        ...
        if (elapsed > m_signingWindowMs + 2000) { // 2s network tolerance
            LogPrint(BCLog::HMP, "HMP: rejecting late seal share (elapsed=%dms, window=%dms)\n",
        ...
        if (session.shares.size() >= MAX_SEAL_SIGNERS) {
            LogPrint(BCLog::HMP, "HMP: Session at height %d reached max shares (%zu)\n", session.blockHeight, MAX_SEAL_SIGNERS);
            return HMPAcceptResult::REJECTED_BENIGN;
        }
    ...
    if (m_privilege) {
        ...
        if (tier == HMPPrivilegeTier::UNKNOWN) {
            LogPrint(BCLog::HMP, "HMP: rejecting share from unknown-tier signer %s\n",
                     share.signerPubKey.ToString().substr(0, 16));
            return HMPAcceptResult::REJECTED_BENIGN;
        }
    }
    ...
    if (m_commitments && m_commitments->IsEnabled() &&
        ...
        LogPrint(BCLog::HMP, "HMP: rejecting share from uncommitted signer %s at height %d\n",
    ...
    if (!IsVRFSelected(vrfHash, multiplier)) {
        LogPrint(BCLog::HMP, "HMP: rejecting share, signer not VRF-selected for block %s\n",
                 share.blockHash.ToString().substr(0, 16));
        return HMPAcceptResult::REJECTED_BENIGN;
    }
    ...
    // Re-acquire lock, re-validate, insert
    {
        ...
        LogPrint(BCLog::HMP, "HMP: accepted share for block %s from %s (total: %zu)\n",
                 share.blockHash.ToString().substr(0, 16),
                 share.signerPubKey.ToString().substr(0, 16),
                 session.shares.size());
    }
...
void CSealManager::RecordAttempt(const SignAttempt& attempt) const
{
    AssertLockHeld(cs);
...
std::optional<CSealShare> CSealManager::SignBlock(const uint256& blockHash, int algo) const
{
    ...
    if (m_privilege) {
        ...
        LogPrint(BCLog::HMP, "HMP: SignBlock tier=%d for identity=%s algo=%d block=%s\n",
    ...
    if (m_commitments && m_commitments->IsEnabled()) {
        ...
        if (!attempt.committed) {
            LogPrint(BCLog::HMP, "HMP: SignBlock identity not committed at height %d\n", attempt.height);
    ...
    int multiplier = GetVRFTierMultiplier(static_cast<int>(tier));
    if (!IsVRFSelected(vrfHash, multiplier)) {
        LogPrint(BCLog::HMP, "HMP: not VRF-selected for block %s (tier=%d mult=%d)\n",
    ...
    LogPrint(BCLog::HMP, "HMP: VRF SELECTED for block %s (tier=%d) -- producing seal share\n",
    ...
    // This also avoids a scalar round-trip bug in VerifyProof.
...
{
    {
        ...
        // Height-based dedup before expensive Verify() -- if we already have
        // a seal at this height with >= signers, skip BLS verification entirely.
        auto heightIt = m_heightToHash.find(height);
        if (heightIt != m_heightToHash.end() && heightIt->second != seal.blockHash) {
            auto existingSeal = m_assembledSeals.find(heightIt->second);
    ...
    if (uniqueSigners.size() != seal.signers.size()) {
        LogPrint(BCLog::HMP, "HMP: rejecting assembled seal with %zu duplicate signers\n",
...
void CSealManager::BroadcastShare(const CSealShare& share)
{
    ...
    LogPrint(BCLog::HMP, "HMP: broadcast seal share for block %s\n",
             share.blockHash.ToString().substr(0, 16));
}
...
void CSealManager::BlockDisconnected(const uint256& blockHash, int height)
{
    ...
    LogPrint(BCLog::HMP, "HMP: seal manager disconnect block %s height %d\n",
             blockHash.ToString().substr(0, 16), height);
}
