                                             ...
                                             const std::vector<CBLSPublicKey>& explicitCommitments)
{
    ...
    if (minerPubKey.IsValid() && m_commitments.find(minerPubKey) == m_commitments.end()) {
        ...
        LogPrint(BCLog::HMP, "HMP: implicit commitment for miner %s at height %d\n",
    ...
    for (const auto& pk : explicitCommitments) {
        ...
        LogPrint(BCLog::HMP, "HMP: explicit commitment for %s at height %d\n",
...
{
    // Cheap structural checks before expensive BLS verification
    ...
    if (!commit.Verify()) {
        LogPrint(BCLog::HMP, "HMP: rejecting commit with invalid signature from %s\n",
    ...
    for (auto it = m_pending.begin(); it != m_pending.end(); ) {
        if ((nNow - it->second.nTimestamp) > COMMIT_TTL_SECS) {
    ...
    LogPrint(BCLog::HMP, "HMP: added pending commit from %s (pool size: %zu)\n",
...
{
    // Don't add entries with invalid BLS signatures to the pool.
    // Reorg-restored pubkeys only have the raw pubkey (no signature), creating
    // dead entries that block the signer from re-committing via P2P for up to
    // COMMIT_TTL_SECS. Instead, let the signer re-broadcast their signed commit
    // naturally. Log for diagnostics.
    if (!commit.signature.IsValid()) {
        LogPrint(BCLog::HMP, "HMP: skipping unsigned reorg-restored commit for %s (signer should re-broadcast)\n",
                 commit.pubKey.IsValid() ? commit.pubKey.ToString().substr(0, 16) : "invalid");
        return HMPAcceptResult::ACCEPTED;
    }
    ...
    m_pending[commit.pubKey] = commit;
    LogPrint(BCLog::HMP, "HMP: restored commit from reorg for %s (pool size: %zu)\n",
             commit.pubKey.ToString().substr(0, 16), m_pending.size());
    return HMPAcceptResult::ACCEPTED;
}
                                                            ...
                                                            const CHMPCommitmentRegistry& registry) const
{
    ...
    for (const auto& [pk, commit] : m_pending) {
        ...
        // Skip reorg-restored entries with no BLS signature
