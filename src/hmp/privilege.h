...
enum class HMPPrivilegeTier {
    ...
    BROOD,    // ELDER + registered BroodNode (40K collateral) operator
};

/** Interface for querying masternode status from the privilege tracker.
 *  Injected at construction to avoid HMP depending on evo/ internals. */
class IMasternodeLookup {
public:
    virtual ~IMasternodeLookup() = default;
    /** Check if a BLS public key corresponds to an active BroodNode operator.
     *  @param pindex  Block index to query the MN list for. When non-null the
     *                 implementation MUST use the deterministic list at that
     *                 block (consensus-critical). When nullptr, the tip list
     *                 may be used (RPC / P2P paths). */
    virtual bool IsBroodNodeOperator(const CBLSPublicKey& operatorPubKey,
                                     const CBlockIndex* pindex = nullptr) const = 0;
};
...
private:
    ...
    // Equivocation demotion: pubkey -> demotedUntilHeight (per-algo)
    std::map<CBLSPublicKey, int> m_demotions[NUM_ALGOS] GUARDED_BY(cs);
...
public:
    CHMPPrivilegeTracker() : m_windowSize(100), m_warmupBlocks(10), m_minBlocksSolved(1),
                             ...
                             m_broodDemotionDuration(1000) {}

    explicit CHMPPrivilegeTracker(const Consensus::Params& params);

    /** Set the masternode lookup interface for BROOD tier queries.
     *  Called after dmnman is initialized (may be nullptr if no MN support). */
    void SetMasternodeLookup(const IMasternodeLookup* mnLookup) { m_mnLookup = mnLookup; }
     ...
     *  @param pindex  When non-null, the BROOD tier lookup uses the deterministic
     *                 MN list at this block (required on consensus paths).
     *                 When nullptr, the tip list is used (RPC / P2P). */
    HMPPrivilegeTier GetTier(const CBLSPublicKey& pubkey, int algo,
                              const CBlockIndex* pindex = nullptr) const;

    /** Get effective weight with 25% dominance cap. Returns weight in basis points (0-2500).
     *  Test-only API -- not used in production paths. */
    ...
    /** Demote a pubkey across all algos (equivocation punishment).
     *  Demoted keys return UNKNOWN tier for demotionDuration blocks. */
    void DemotePubkey(const CBLSPublicKey& pubkey, int currentHeight);

    /** Remove expired demotion records */
    void ClearExpiredDemotions(int currentHeight);

    /** Clear all privilege data (windows + records + demotions). Called when SPORK_25_HMP_ENABLED
