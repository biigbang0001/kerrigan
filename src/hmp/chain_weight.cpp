                                ...
                                const CBlockIndex* pindex)
{
    ...
    // Cross-algorithm and BroodNode bonuses (PRD Section 4.8).
    // Single pass: classify each signer once to avoid redundant GetTier() calls.
    std::set<uint8_t> elderAlgos;
    std::set<uint8_t> broodAlgos;
    for (size_t i = 0; i < signerPubKeys.size(); i++) {
        HMPPrivilegeTier tier = privilege->GetTier(signerPubKeys[i], signerAlgos[i], pindex);
        if (tier >= HMPPrivilegeTier::ELDER) {
            elderAlgos.insert(signerAlgos[i]);
        }
        if (tier >= HMPPrivilegeTier::BROOD) {
            broodAlgos.insert(signerAlgos[i]);
        }
    }
    elderAlgos.erase(static_cast<uint8_t>(blockAlgo));
    uint64_t crossAlgoBonus = std::min<uint64_t>(elderAlgos.size() * 500, 1500);
    uint64_t broodBonus = std::min<uint64_t>(broodAlgos.size() * 300, 1200);

    return baseMult + crossAlgoBonus + broodBonus;
}
