...
void StartShutdown()
{
    ...
    if (!fRequestShutdown.exchange(true)) {
        ...
        if (res != 0) {
            // This path may be reached from a signal handler, so only
            // use async-signal-safe functions (no LogPrintf, no assert).
            const char msg[] = "Sending shutdown token failed\n";
