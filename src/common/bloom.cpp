...
bool CBloomFilter::CheckSpecialTransactionMatchesAndUpdate(const CTransaction &tx)
{
    ...
    case(TRANSACTION_SAPLING): {
        // Sapling shielded data is encrypted; the only publicly-visible
        // identifiers are nullifiers (spends) and note commitments (outputs).
        // Match nullifiers so SPV wallets can detect when their notes are spent,
        // and commitments so they can detect incoming shielded payments.
        if (const auto opt_payload = GetTxPayload<SaplingTxPayload>(tx)) {
            for (const auto& spend : opt_payload->vSpendDescriptions) {
                if (contains(spend.nullifier)) {
                    if ((nFlags & BLOOM_UPDATE_MASK) == BLOOM_UPDATE_ALL)
                        insert(tx.GetHash());
                    return true;
                }
            }
            for (const auto& output : opt_payload->vOutputDescriptions) {
                if (contains(output.cmu)) {
                    if ((nFlags & BLOOM_UPDATE_MASK) == BLOOM_UPDATE_ALL)
                        insert(tx.GetHash());
                    return true;
                }
