...
#ifndef KERRIGAN_CRYPTO_BLS_H
#define KERRIGAN_CRYPTO_BLS_H
...
// bls library uses relic, which may define DEBUG and ERROR, which leads to many warnings in some build setups
#undef ERROR
#undef DEBUG
...
namespace bls {
    ...
    /** RAII guard that snapshots bls_legacy_scheme on construction and
     *  restores it on destruction unless commit() was called.
     *  Prevents scheme leaks on early-return error paths. */
    struct BLSSchemeGuard {
        bool saved;
        bool committed{false};
        BLSSchemeGuard() : saved(bls_legacy_scheme.load()) {}
        ~BLSSchemeGuard() { if (!committed) bls_legacy_scheme.store(saved); }
        /** Call on success to keep whatever value the guarded code set. */
        void commit() { committed = true; }
        BLSSchemeGuard(const BLSSchemeGuard&) = delete;
        BLSSchemeGuard& operator=(const BLSSchemeGuard&) = delete;
    };
}
...
public:
    ...
    [[nodiscard]] bool VerifyInsecureAggregated(Span<CBLSPublicKey> pubKeys, Span<uint256> hashes, const bool specificLegacyScheme) const;
...
private:
    ...
    /** Internal accessor that does NOT acquire the mutex.
     *  Caller must already hold the lock. */
    const BLSObject& GetInternal() const
    {
...
public:
    ...
    CBLSLazyWrapper& operator=(const CBLSLazyWrapper& r)
    {
        ...
        std::scoped_lock l(mutex, r.mutex);
        bufValid = r.bufValid;
    ...
    bool operator==(const CBLSLazyWrapper& r) const
    {
        ...
        // Fall through: one or both need deserialization. Use the
        // non-locking internal accessor since we already hold both
        // locks. This fixes the TOCTOU between lock release and
        // comparison.
        return GetInternal() == r.GetInternal();
    }
