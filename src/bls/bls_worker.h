...
#ifndef KERRIGAN_CRYPTO_BLS_WORKER_H
#define KERRIGAN_CRYPTO_BLS_WORKER_H
...
private:
    ...
    {
        {
            std::unique_lock<std::mutex> lock(cacheCs);
        ...
        // Build OUTSIDE the lock -- no promise cached yet
        T v = builder();
        {
            std::unique_lock<std::mutex> lock(cacheCs);
            // Another thread may have built it concurrently -- that's OK, just use ours
            std::promise<T> p;
            p.set_value(v);
...
#endif //KERRIGAN_CRYPTO_BLS_WORKER_H
