...
bool CBLSWorker::GenerateContributions(int quorumThreshold, Span<CBLSId> ids, BLSVerificationVectorPtr& vvecRet, std::vector<CBLSSecretKey>& skSharesRet)
{
    ...
    return ranges::all_of(futures, [](auto& f){
        if (!f.valid()) {
            LogPrintf("WARNING: BLS worker task failed to enqueue (pool saturated)\n");
            return false;
        }
        try {
            return f.get();
        } catch (const std::future_error& e) {
            LogPrintf("WARNING: BLS worker task future error: %s\n", e.what());
            return false;
        }
    });
}
...
struct Aggregator : public std::enable_shared_from_this<Aggregator<T>> {
    ...
    {
        auto fut = workerPool.push(f);
        if (!fut.valid()) {
            // Pool saturated -- run inline to ensure completion
            LogPrint(BCLog::LLMQ, "BLS: worker pool saturated, running aggregation inline\n");
            f(0);
        }
    }
};
...
struct ContributionVerifier : public std::enable_shared_from_this<ContributionVerifier> {
    ...
    {
        if (parallel) {
            auto fut = workerPool.push(std::forward<Callable>(f));
            if (!fut.valid()) {
                LogPrint(BCLog::LLMQ, "BLS: worker pool saturated, verification task falling back to inline\n");
                f(0);
            }
        } else {
            f(0);
...
{
    auto fut = workerPool.push([secKey, msgHash, doneCallback](int threadId) {
        doneCallback(secKey.Sign(msgHash, bls::bls_legacy_scheme.load()));
    });
    if (!fut.valid()) {
        LogPrint(BCLog::LLMQ, "BLS: worker pool saturated, signing task falling back to inline\n");
        doneCallback(secKey.Sign(msgHash, bls::bls_legacy_scheme.load()));
    }
}
...
void CBLSWorker::PushSigVerifyBatch()
{
    ...
    sigVerifyBatchesInProgress++;
    auto fut = workerPool.push(f, batch);
    if (!fut.valid()) {
        // Pool saturated -- process inline without going through `f` which
        // reacquires sigVerifyMutex and would violate the caller's unique_lock
        // ownership invariant.
        LogPrint(BCLog::LLMQ, "BLS: worker pool saturated, running sig verify batch inline\n");
        auto& jobs = *batch;
        for (auto& job : jobs) {
            if (!job.cancelCond()) {
                bool valid = job.sig.VerifyInsecure(job.pubKey, job.msgHash);
