...
bool CheckPackage(const Package& txns, PackageValidationState& state)
{
    ...
    // Check for conflicting Sapling nullifiers within the package.
    // Two txs spending the same shielded note must not both enter the mempool.
    std::set<std::array<uint8_t, 32>> nullifiers_seen;
    for (const auto& tx : txns) {
        auto payload = GetTxPayload<SaplingTxPayload>(*tx);
        if (payload) {
            for (const auto& spend : payload->vSpendDescriptions) {
                if (!nullifiers_seen.insert(spend.nullifier).second) {
                    return state.Invalid(PackageValidationResult::PCKG_POLICY, "sapling-nullifier-conflict-in-package");
                }
            }
        }
    }
