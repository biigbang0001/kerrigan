...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    const bool fHMPActive_context{DeploymentActiveAfter(pindexPrev, chainparams.GetConsensus(), Consensus::DEPLOYMENT_HMP)};
    ...
    // nBits param is unused by Kerrigan's height-only subsidy, kept for API compat.
    ...
    if (!fDIP0003Active_context) {
        ...
        if (fHMPActive_context) {
            cbTx.nVersion = CCbTx::Version::HMP_SEAL;
        ...
        // Populate Hivemind HMP fields in CCbTx v4+
        // minerIdentity is consensus-required at Stage 3+ and must be
        // populated regardless of SPORK_25 state.  SPORK_25 only gates seal
        // signing and proof production, not identity declaration.
        if (fHMPActive_context && g_hmp_identity && g_hmp_identity->IsValid()) {
            cbTx.minerIdentity = g_hmp_identity->GetPublicKey();
        }
        // SPORK_25_HMP_ENABLED gates seal/commitment production only.
        if (fHMPActive_context && g_sporkman && g_sporkman->IsSporkActive(SPORK_25_HMP_ENABLED)) {
            if (g_seal_manager) {
                auto seal = g_seal_manager->GetSealForTemplate(nHeight);
                if (seal) {
                    cbTx.sealForAncestor = seal->aggregatedSig;
                    cbTx.sealBlockHash = seal->blockHash;
                    // Embed signer list on-chain so nSealWeight is reproducible
                    // without in-memory seal cache after restart.
                    cbTx.vSealSigners = seal->signers;
                    cbTx.vSealSignerAlgos = seal->signerAlgos;
                    LogPrintf("CreateNewBlock() h[%d] HMP seal for block %s with %zu signers\n",
                              nHeight, seal->blockHash.ToString().substr(0, 16), seal->signers.size());
                }
            }
            // Embed pending pubkey commitments (CCbTx v5)
            if (chainparams.GetConsensus().nHMPCommitmentOffset > 0 &&
                g_hmp_commit_pool && g_hmp_commitments) {
                auto commits = g_hmp_commit_pool->GetForTemplate(
                    Consensus::Params::MAX_COMMITMENTS_PER_BLOCK, *g_hmp_commitments);
                if (!commits.empty()) {
                    cbTx.vCommitments = std::move(commits);
                    cbTx.nVersion = CCbTx::Version::HMP_COMMITMENT;
                    LogPrintf("CreateNewBlock() h[%d] HMP embedding %zu pubkey commitments\n",
                              nHeight, cbTx.vCommitments.size());
                }
            }
        }
...
bool BlockAssembler::TestPackageTransactions(const CTxMemPool::setEntries& package) const
{
    for (CTxMemPool::txiter it : package) {
        ...
        if ((!it->GetTx().vin.empty() || it->GetTx().nType == TRANSACTION_SAPLING) && !m_clhandler.IsTxSafeForMining(txid)) {
            return false;
        }
    }
    return true;
