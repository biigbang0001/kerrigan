                                                     ...
                                                     std::function<void()> coins_error_cb)
{
    ...
    evodb.reset();
    evodb = std::make_unique<CEvoDB>(util::DbWrapperParams{.path = data_dir, .memory = kerrigan_dbs_in_memory, .wipe = fReset || fReindexChainState});
    ...
    KerriganChainstateSetup(chainman, govman, mn_metaman, mn_sync, sporkman, chainlocks, chain_helper,
                        dmnman, *evodb, llmq_ctx, mempool, data_dir, kerrigan_dbs_in_memory,
    ...
    // Check chain data version marker. If the block index has data but no
    // version marker (or an incompatible version), the chain DB was created
    // by an older binary. Trigger a full reindex to rebuild with current
    // serialization format.
    // Version 2: CDiskBlockIndex now serializes nSolution for equihash blocks
    // and KawPoW uses standard Ravencoin constants. Old chain data is incompatible.
    static constexpr int CURRENT_CHAIN_VERSION{2};
    if (!fReset && !fReindex && !chainman.BlockIndex().empty()) {
        int nChainVersion{0};
        if (!pblocktree->ReadChainVersion(nChainVersion) || nChainVersion < CURRENT_CHAIN_VERSION) {
            LogPrintf("Chain data from incompatible version detected (got %d, need %d). Reindexing...\n",
                      nChainVersion, CURRENT_CHAIN_VERSION);
            fReindex = true;
            pblocktree->WriteReindexing(true);
        }
    }
    ...
    // Stamp the chain data version so future builds can detect incompatible DB.
    if (!fReset && !fReindex) {
        pblocktree->WriteChainVersion(CURRENT_CHAIN_VERSION);
    }

    return std::nullopt;
}

void KerriganChainstateSetup(ChainstateManager& chainman,
                         CGovernanceManager& govman,
                         CMasternodeMetaMan& mn_metaman,
                         CMasternodeSync& mn_sync,
                         CSporkManager& sporkman,
                         ...
                         const Consensus::Params& consensus_params)
{
    ...
    chain_helper = std::make_unique<CChainstateHelper>(evodb, *dmnman, govman, *(llmq_ctx->isman), *(llmq_ctx->quorum_block_processor),
                                                       ...
                                                       *(llmq_ctx->qman), data_dir, /*fWipe=*/llmq_dbs_wipe);
}

void KerriganChainstateSetupClose(std::unique_ptr<CChainstateHelper>& chain_helper,
                              std::unique_ptr<CDeterministicMNManager>& dmnman,
                              std::unique_ptr<LLMQContext>& llmq_ctx,
                              CTxMemPool* mempool)
