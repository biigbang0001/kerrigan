...
int SyncManager::RequestGovernanceObjectVotes(const std::vector<CNode*>& vNodesCopy) const
{
    ...
    // CNetAddr key (not CService) prevents port-churn bypass
    static std::map<uint256, std::map<CNetAddr, int64_t>> mapAskedRecently GUARDED_BY(cs_recently);
    LOCK(cs_recently);
    ...
    // Prune expired inner entries and empty outer entries
    for (auto it = mapAskedRecently.begin(); it != mapAskedRecently.end(); ) {
        for (auto jt = it->second.begin(); jt != it->second.end(); ) {
            if (jt->second < nNow) jt = it->second.erase(jt);
            else ++jt;
        }
        if (it->second.empty()) it = mapAskedRecently.erase(it);
        else ++it;
    }
