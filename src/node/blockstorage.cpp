...
{
    // Primary: chain with most seal-weighted work wins (Hivemind HMP)
    if (pa->nChainSealWork > pb->nChainSealWork) return false;
    if (pa->nChainSealWork < pb->nChainSealWork) return true;

    // Fallback: pure PoW (pre-HMP blocks or when sealing is unavailable)
                                           ...
                                           enum BlockStatus nStatus)
{
    ...
    // Initialize seal work from nSealWeight (default 10000 = 1.0x, updated in ConnectBlock when seal data available)
    {
        arith_uint256 blockProof = GetBlockProof(*pindexNew);
        uint64_t clampedWeight = std::min(pindexNew->nSealWeight, uint64_t{20000});
        arith_uint256 weightedProof = blockProof * arith_uint256(clampedWeight) / arith_uint256(10000);
        pindexNew->nChainSealWork = (pindexNew->pprev ? pindexNew->pprev->nChainSealWork : arith_uint256(0)) + weightedProof;
    }
    if (nStatus & BLOCK_VALID_MASK) {
        pindexNew->RaiseValidity(nStatus);
        if (best_header == nullptr || CBlockIndexWorkComparator()(best_header, pindexNew)) {
...
bool BlockManager::LoadBlockIndex(const Consensus::Params& consensus_params)
{
    ...
    for (CBlockIndex* pindex : vSortedByHeight) {
        ...
        // Reconstruct nChainSealWork from persisted nSealWeight values
        {
...
void ThreadImport(ChainstateManager& chainman, std::vector<fs::path> vImportFiles, const ArgsManager& args)
{
...

    {
        ...
        bool fAutoReindex = false; // set if ActivateBestChain detects corrupt block data

auto_reindex_retry:
        // -reindex (or auto-triggered reindex after block corruption)
        if (fReindex) {
            int nFile = 0;
            ...
            // Stamp chain data version so next startup doesn't re-trigger reindex
            // Must match CURRENT_CHAIN_VERSION in chainstate.cpp to prevent re-triggering reindex
            WITH_LOCK(::cs_main, chainman.m_blockman.m_block_tree_db->WriteChainVersion(2));
            // To avoid ending up in a situation without genesis block, re-try initializing (no-op if reindexing worked):
            chainman.ActiveChainstate().LoadGenesisBlock();
        }

        // -loadblock= (skip on auto-reindex retry, files were already processed)
        if (!fAutoReindex) {
        ...
        for (CChainState* chainstate : WITH_LOCK(::cs_main, return chainman.GetAll())) {
            ...
            if (!chainstate->ActivateBestChain(state, nullptr)) {
                ...
                // If this is a system error (corrupt block data, disk I/O) and we
                // haven't already retried, trigger an automatic reindex.
                if (!state.IsInvalid() && !fAutoReindex) {
                    LogPrintf("Block data may be corrupted. Triggering automatic reindex...\n");
                    fReindex = true;
                    fAutoReindex = true;
                    WITH_LOCK(::cs_main, chainman.m_blockman.m_block_tree_db->WriteReindexing(true));
                    goto auto_reindex_retry;
                }
                StartShutdown();
                return;
            }
    ...
    LogPrintf("ThreadImport complete: fImporting=%d fReindex=%d, peer sync gate now open\n",
              fImporting.load(), fReindex.load());
