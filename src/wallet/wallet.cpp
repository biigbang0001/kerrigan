...

    {
        ...
        for (MasterKeyMap::value_type& pMasterKey : mapMasterKeys)
        {
            ...
            if (Unlock(_vMasterKey))
            {
                ...
                int64_t nElapsed = TicksSinceEpoch<std::chrono::milliseconds>(SystemClock::now()) - nStartTime;
                ...
                crypter.SetKeyFromPassphrase(strNewWalletPassphrase, pMasterKey.second.vchSalt, pMasterKey.second.nDeriveIterations, pMasterKey.second.nDerivationMethod);
                nElapsed = TicksSinceEpoch<std::chrono::milliseconds>(SystemClock::now()) - nStartTime;
                if (nElapsed < 1) nElapsed = 1;
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        // Encrypt Sapling spending keys
        if (!m_sapling_key_manager.IsEmpty()) {
            if (!m_sapling_key_manager.EncryptKeys(_vMasterKey, *encrypted_batch)) {
                encrypted_batch->TxnAbort();
                delete encrypted_batch;
                encrypted_batch = nullptr;
                assert(false);
...
bool CWallet::LoadToWallet(const uint256& hash, const UpdateWalletTxFn& fill_wtx)
{
    ...
    // If wallet doesn't have a chain (e.g when using kerrigan-wallet tool),
...
void CWallet::transactionAddedToMempool(const CTransactionRef& tx, int64_t nAcceptTime) {
    ...
    // Scan for Sapling shielded notes arriving via mempool.
    if (!m_sapling_key_manager.IsEmpty() && tx->nType == TRANSACTION_SAPLING) {
        auto payload = GetTxPayload<SaplingTxPayload>(*tx, /*assert_type=*/false);
        if (payload) {
            m_sapling_key_manager.ScanSaplingOutputs(*tx, *payload, -1, Params().NetworkIDString(), Params().GetConsensus().SaplingHeight, &batch);
            m_sapling_key_manager.ScanSaplingSpends(*tx, *payload, &batch);
        }
    }
}
...
void CWallet::ProcessSaplingBlock(const CBlock& block, int height, WalletBatch& batch)
{
    AssertLockHeld(cs_wallet);
    if (m_sapling_key_manager.IsEmpty()) return;

    const std::string& netID = Params().NetworkIDString();
    std::vector<uint256> blockCmus;
    std::vector<std::pair<CTransactionRef, SaplingTxPayload>> saplingTxs;

    for (const CTransactionRef& ptx : block.vtx) {
        if (ptx->nType == TRANSACTION_SAPLING) {
            auto payload = GetTxPayload<SaplingTxPayload>(*ptx, /*assert_type=*/false);
            if (payload) {
                // Detect incoming shielded notes for this wallet
                m_sapling_key_manager.ScanSaplingOutputs(*ptx, *payload, height, netID, Params().GetConsensus().SaplingHeight, &batch);
                // Collect all note commitments for witness tracking
                for (const auto& od : payload->vOutputDescriptions) {
                    uint256 cmu;
                    std::memcpy(cmu.begin(), od.cmu.data(), 32);
                    blockCmus.push_back(cmu);
                }
                saplingTxs.emplace_back(ptx, *payload);
            }
        }
    }
    ...
    for (const auto& [ptx, payload] : saplingTxs) {
        m_sapling_key_manager.ScanSaplingSpends(*ptx, payload, &batch);
    }
}
...
void CWallet::blockConnected(const CBlock& block, int height)
{
    ...
    ProcessSaplingBlock(block, height, batch);
...
CWallet::ScanResult CWallet::ScanForWalletTransactions(const uint256& start_block, int start_height, std::optional<int> max_height, const WalletRescanReserver& reserver, bool fUpdate, const bool save_progress)
{
    ...
    // Disable fast rescan when wallet has Sapling keys; BIP 158 filters
    // exclude shielded data (privacy), so Sapling-only blocks would be skipped.
    if (!IsLegacy() && m_sapling_key_manager.IsEmpty() && chain().hasBlockFilterIndex(BlockFilterType::BASIC_FILTER)) fast_rescan_filter = std::make_unique<FastWalletRescanFilter>(*this);
    ...
    while (!fAbortRescan && !chain().shutdownRequested()) {
        ...
        if (fetch_block) {
            ...
            if (!block.IsNull()) {
                ...
                ProcessSaplingBlock(block, block_height, batch);
...
DBErrors CWallet::ZapSelectTx(std::vector<uint256>& vHashIn, std::vector<uint256>& vHashOut)
{
    ...
    // Clean up orphaned Sapling notes whose creating or spending tx was zapped (persist to DB)
    if (!m_sapling_key_manager.IsEmpty()) {
        WalletBatch zapBatch(GetDatabase());
        m_sapling_key_manager.ZapNotesByTxid(std::set<uint256>(vHashOut.begin(), vHashOut.end()), &zapBatch);
    }
...
std::shared_ptr<CWallet> CWallet::Create(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
...

    {
        ...
        const auto& skm = walletInstance->GetSaplingKeyManager();
        LogPrint(BCLog::SAPLING, "%s Sapling: %u spending keys, %u addresses, %u notes\n",
                 walletInstance->GetDisplayName(), skm.GetSpendableKeyCount(), skm.GetAllAddresses().size(), skm.GetNoteCount());
    }
    ...
    if (!gArgs.GetBoolArg("-walletcrosschain", DEFAULT_WALLETCROSSCHAIN)) {
        ...
        if (batch.ReadBestBlock(locator) && locator.vHave.size() > 0 && chain.getHeight()) {
            ...
            if (chain.getBlockHash(0) != locator.vHave.back()) {
                error = Untranslated("Wallet files should not be reused across chains. Restart kerrigand with -walletcrosschain to override.");
                return false;
            }
        }
    }
...
bool CWallet::AutoBackupWallet(const fs::path& wallet_path, bilingual_str& error_string, std::vector<bilingual_str>& warnings)
{
    ...
    if (wallet_path.empty()) {
        ...
        if (IsLocked()) {
            warnings.push_back(_("Wallet is locked, can't replenish keypool! Automatic backups are disabled, please unlock your wallet to replenish keypool."));
            WalletLogPrintf("%s\n", Join(warnings, Untranslated("\n")).original);
            nWalletBackups = -2;
            return false;
        }
...
{
    {
        ...
        // Decrypt Sapling spending keys if the key manager is encrypted
        if (m_sapling_key_manager.IsCrypto()) {
            if (!m_sapling_key_manager.Unlock(vMasterKeyIn)) {
                return false;
            }
        }
...
void CWallet::SetupDescriptorScriptPubKeyMans(const SecureString& mnemonic_arg, const SecureString mnemonic_passphrase)
{
    ...
    if (!CMnemonic::Check(mnemonic)) {
        throw std::runtime_error(std::string(__func__) + ": invalid mnemonic (checksum failure)");
    }
    ...
    for (auto type : {PathDerivationType::BIP44_External, PathDerivationType::BIP44_Internal}) {
        ...
        for (bool internal : {false, true}) {
            ...
            for (const UniValue& desc_val : descriptor_vals.get_array().getValues()) {
                ...
                // only activates External and Internal types
