...
extern const std::string SAPLING_FVK;
extern const std::string SAPLING_ADDR;
...
public:
    ...
    // Sapling key persistence
    bool WriteSaplingExtendedSpendingKey(const sapling::SaplingIncomingViewingKey& ivk, const sapling::SaplingExtendedSpendingKey& key);
    bool WriteCryptedSaplingSpendingKey(const sapling::SaplingIncomingViewingKey& ivk, const std::vector<unsigned char>& vchCryptedSecret);
    bool WriteSaplingFullViewingKey(const sapling::SaplingIncomingViewingKey& ivk, const sapling::SaplingFullViewingKey& fvk, const std::array<uint8_t, 32>& dk);
    bool WriteSaplingPaymentAddress(const sapling::SaplingPaymentAddress& addr, const sapling::SaplingIncomingViewingKey& ivk);
    bool WriteSaplingWitness(const uint256& nullifier, const std::vector<unsigned char>& witness);
    bool EraseSaplingWitness(const uint256& nullifier);
    bool WriteSaplingDiversifierIndex(const uint256& seed_hash, uint64_t index);
    bool ReadSaplingDiversifierIndex(const uint256& seed_hash, uint64_t& index);
