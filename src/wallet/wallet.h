...
private:
    ...
    mutable bool fAnonymizableTallyCached GUARDED_BY(cs_wallet) = false;
    mutable std::vector<CompactTallyItem> vecAnonymizableTallyCached;
    mutable bool fAnonymizableTallyCachedNonDenom GUARDED_BY(cs_wallet) = false;
    ...
    /** Sapling shielded key manager (ZIP 32 keys, payment addresses) */
...
public:
    ...
    /** Access the Sapling shielded key manager for this wallet */
    SaplingKeyManager& GetSaplingKeyManager() { return m_sapling_key_manager; }
    const SaplingKeyManager& GetSaplingKeyManager() const { return m_sapling_key_manager; }
