...
RPCHelpMan importaddress()
{
    return RPCHelpMan{"importaddress",
            ...
            "Note: This command is only compatible with legacy wallets. Use \"importdescriptors\" with \"addr(X)\" for descriptor wallets.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Kerrigan address (or hex-encoded script)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Kerrigan address or script");
        }
    }
    if (fRescan)
    {
...
RPCHelpMan importwallet()
{
    return RPCHelpMan{"importwallet",
                ...
                "Note: Sapling shielded keys in the dump file (sapling_extsk= lines) are also imported.\n",
                {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool fGood = true;
    {
        ...
        while (file.good()) {
            ...
            // Sapling extended spending key line: sapling_extsk=HEX addr=ADDR
            if (line.substr(0, 15) == "sapling_extsk=") {
                std::vector<std::string> vstr = SplitString(line, ' ');
                std::string hexKey;
                for (const auto& token : vstr) {
                    if (token.substr(0, 15) == "sapling_extsk=") {
                        hexKey = token.substr(15);
                    }
                }
                if (hexKey.empty() || !IsHex(hexKey)) {
                    LogPrintf("importwallet: invalid sapling_extsk on line %d\n", nLine);
                    fGood = false;
                    continue;
                }
                auto keyBytes = ParseHex(hexKey);
                memory_cleanse(hexKey.data(), hexKey.size());
                if (keyBytes.size() != 169) {
                    LogPrintf("importwallet: sapling_extsk wrong length on line %d (got %d, expected 169)\n", nLine, keyBytes.size());
            ...
            if (key.IsValid()) {
                ...
                LogPrintf("importwallet: skipping unrecognized line %d\n", nLine);
            }
        }
        file.close();
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if(strFileExt == "csv") {
        ...
        {
            auto buffer = std::make_unique<char[]>(nFilesize + 1);
    ...
    if (!request.params[1].isNull()) {
        ...
        if (nStartHeight < 0)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Block index must not be negative");
    }
...
RPCHelpMan dumpprivkey()
{
    return RPCHelpMan{"dumpprivkey",
                ...
                "Note: This command is only compatible with legacy wallets.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Kerrigan address for the private key"},
                },
                RPCResult{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Kerrigan address");
    }
...
RPCHelpMan dumpwallet()
{
    return RPCHelpMan{"dumpwallet",
                ...
                "Note: This command is only compatible with legacy wallets.\n"
                "Note: Sapling shielded spending keys are included in the dump as sapling_extsk= lines.\n"
                "importwallet will import them automatically.\n",
                {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    file << "#\n";
    file << "# Sapling shielded spending keys are included below as sapling_extsk= lines.\n";
    file << "# importwallet will import these keys automatically.\n";
    file << "\n";

    UniValue obj(UniValue::VOBJ);
    obj.pushKV("kerriganversion", FormatFullVersion());
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see kerrigand log for details) and could "
