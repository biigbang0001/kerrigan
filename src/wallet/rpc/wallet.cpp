...
static RPCHelpMan listaddressbalances()
{
    return RPCHelpMan{"listaddressbalances",
        ...
        RPCResult{
            ...
            {
                {RPCResult::Type::STR_AMOUNT, "address", "The balance in " + CURRENCY_UNIT},
            }
        },
        RPCExamples{
...
static RPCHelpMan loadwallet()
{
    return RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting kerrigand will be"
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[5].isNull() && request.params[5].get_bool()) {
        ...
        if (request.params[6].isNull()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "The createwallet RPC requires specifying the 'load_on_startup' flag when creating descriptor wallets. Kerrigan introduced this requirement due to breaking changes in the createwallet RPC.");
        }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (request.params[1].isNull()) {
        ...
        // add default account BEFORE persisting so chain is stored with one account
...
Span<const CRPCCommand> GetWalletRPCCommands()
{
    static const CRPCCommand commands[]{
        ...
        // Sapling shielded transaction commands
        {"wallet", &z_getnewaddress},
        {"wallet", &z_getbalance},
        {"wallet", &z_listunspent},
        {"wallet", &z_sendmany},
        {"wallet", &z_listaddresses},
        {"wallet", &z_exportkey},
        {"wallet", &z_importkey},
