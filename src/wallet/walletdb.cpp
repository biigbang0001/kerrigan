...
const std::string SAPLING_KEY{"sapkey"};
const std::string SAPLING_CRYPTED_KEY{"csapkey"};
const std::string SAPLING_FVK{"sapfvk"};
const std::string SAPLING_ADDR{"sapaddr"};
...
bool WalletBatch::WriteSaplingExtendedSpendingKey(const sapling::SaplingIncomingViewingKey& ivk, const sapling::SaplingExtendedSpendingKey& key)
{
    // Key: (SAPLING_KEY, ivk.key)  Value: (key.key)
    std::vector<uint8_t> ivk_vec(ivk.key.begin(), ivk.key.end());
    std::vector<uint8_t> sk_vec(key.key.begin(), key.key.end());
    return WriteIC(std::make_pair(DBKeys::SAPLING_KEY, ivk_vec), sk_vec);
}

bool WalletBatch::WriteCryptedSaplingSpendingKey(const sapling::SaplingIncomingViewingKey& ivk, const std::vector<unsigned char>& vchCryptedSecret)
{
    // Key: (SAPLING_CRYPTED_KEY, ivk.key)  Value: encrypted_sk_bytes
    std::vector<uint8_t> ivk_vec(ivk.key.begin(), ivk.key.end());
    if (!WriteIC(std::make_pair(DBKeys::SAPLING_CRYPTED_KEY, ivk_vec), vchCryptedSecret)) {
        return false;
    }
    // Erase plaintext key if it exists
    EraseIC(std::make_pair(DBKeys::SAPLING_KEY, ivk_vec));
    return true;
}

bool WalletBatch::WriteSaplingDiversifierIndex(const uint256& seed_hash, uint64_t index)
{
    return WriteIC(std::make_pair(DBKeys::SAPLING_DIVERSIFIER_INDEX, seed_hash), index);
}

bool WalletBatch::ReadSaplingDiversifierIndex(const uint256& seed_hash, uint64_t& index)
{
    return m_batch->Read(std::make_pair(DBKeys::SAPLING_DIVERSIFIER_INDEX, seed_hash), index);
}

bool WalletBatch::WriteSaplingFullViewingKey(const sapling::SaplingIncomingViewingKey& ivk, const sapling::SaplingFullViewingKey& fvk, const std::array<uint8_t, 32>& dk)
{
    // Key: (SAPLING_FVK, ivk.key)  Value: (ak || nk || ovk || dk)
    std::vector<uint8_t> ivk_vec(ivk.key.begin(), ivk.key.end());
    std::vector<uint8_t> val;
    val.reserve(128);
    val.insert(val.end(), fvk.ak.begin(), fvk.ak.end());
    val.insert(val.end(), fvk.nk.begin(), fvk.nk.end());
    val.insert(val.end(), fvk.ovk.begin(), fvk.ovk.end());
    val.insert(val.end(), dk.begin(), dk.end());
    return WriteIC(std::make_pair(DBKeys::SAPLING_FVK, ivk_vec), val);
}

bool WalletBatch::WriteSaplingPaymentAddress(const sapling::SaplingPaymentAddress& addr, const sapling::SaplingIncomingViewingKey& ivk)
{
    // Key: (SAPLING_ADDR, d || pk_d)  Value: (ivk.key)
    std::vector<uint8_t> addr_vec;
    addr_vec.reserve(43);
    addr_vec.insert(addr_vec.end(), addr.d.begin(), addr.d.end());
    addr_vec.insert(addr_vec.end(), addr.pk_d.begin(), addr.pk_d.end());
    std::vector<uint8_t> ivk_vec(ivk.key.begin(), ivk.key.end());
    return WriteIC(std::make_pair(DBKeys::SAPLING_ADDR, addr_vec), ivk_vec);
}

bool WalletBatch::WriteSaplingWitness(const uint256& nullifier, const std::vector<unsigned char>& witness)
{
    return WriteIC(std::make_pair(DBKeys::SAPLING_WITNESS, nullifier), witness);
}

bool WalletBatch::EraseSaplingWitness(const uint256& nullifier)
{
    return EraseIC(std::make_pair(DBKeys::SAPLING_WITNESS, nullifier));
}

bool WalletBatch::WriteSaplingNote(const uint256& cmu, const SaplingNoteData& noteData)
{
    return WriteIC(std::make_pair(DBKeys::SAPLING_NOTE, cmu), noteData);
}
             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr, const KeyFilterFn& filter_fn = nullptr) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    try {
        ...
        if (strType == DBKeys::NAME) {
            ...
            // The wallet may be created in an older version of Kerrigan or by importing descriptor
            try
            {
                ssValue >> mnemonic;
            ...
            // Extended spending key: key=(SAPLING_KEY, ivk_bytes), value=sk_bytes
            std::vector<uint8_t> ivk_vec;
            ssKey >> ivk_vec;
            if (ivk_vec.size() != 32) {
                strErr = "Error reading wallet database: Sapling IVK wrong size";
                return false;
            }
            CKeyingMaterial sk_vec;
            ssValue >> sk_vec;
            if (sk_vec.size() != 169) {
                strErr = "Error reading wallet database: Sapling ExtSK wrong size";
                return false;
            }
            sapling::SaplingIncomingViewingKey ivk;
            std::copy(ivk_vec.begin(), ivk_vec.end(), ivk.key.begin());
            sapling::SaplingExtendedSpendingKey sk;
            std::copy(sk_vec.begin(), sk_vec.end(), sk.key.begin());
            if (!pwallet->GetSaplingKeyManager().LoadSpendingKey(ivk, sk)) {
                strErr = "Error reading wallet database: Sapling LoadSpendingKey failed";
                return false;
            }
        } else if (strType == DBKeys::SAPLING_CRYPTED_KEY) {
            // Encrypted spending key: key=(SAPLING_CRYPTED_KEY, ivk_bytes), value=encrypted_sk_bytes
            std::vector<uint8_t> ivk_vec;
            ssKey >> ivk_vec;
            if (ivk_vec.size() != 32) {
                strErr = "Error reading wallet database: Sapling crypted key IVK wrong size";
                return false;
            }
            ...
            sapling::SaplingIncomingViewingKey ivk;
            std::copy(ivk_vec.begin(), ivk_vec.end(), ivk.key.begin());
            pwallet->GetSaplingKeyManager().LoadCryptedSpendingKey(ivk, vchCryptedSecret);
        } else if (strType == DBKeys::SAPLING_DIVERSIFIER_INDEX) {
            // Diversifier index: key=(SAPLING_DIVERSIFIER_INDEX, seed_hash), value=uint64_t
            uint256 seed_hash;
            ssKey >> seed_hash;
            uint64_t index;
            ssValue >> index;
            pwallet->GetSaplingKeyManager().LoadDiversifierIndex(index);
        } else if (strType == DBKeys::SAPLING_FVK) {
            // Full viewing key: key=(SAPLING_FVK, ivk_bytes), value=(ak||nk||ovk||dk)
            std::vector<uint8_t> ivk_vec;
            ssKey >> ivk_vec;
            if (ivk_vec.size() != 32) {
                strErr = "Error reading wallet database: Sapling IVK wrong size (FVK record)";
                return false;
            }
            std::vector<uint8_t> val;
            ssValue >> val;
            if (val.size() != 128) {
                strErr = "Error reading wallet database: Sapling FVK+DK wrong size";
                return false;
            }
            sapling::SaplingIncomingViewingKey ivk;
            std::copy(ivk_vec.begin(), ivk_vec.end(), ivk.key.begin());
            sapling::SaplingFullViewingKey fvk;
            std::copy(val.begin(), val.begin() + 32, fvk.ak.begin());
            std::copy(val.begin() + 32, val.begin() + 64, fvk.nk.begin());
            std::copy(val.begin() + 64, val.begin() + 96, fvk.ovk.begin());
            std::array<uint8_t, 32> dk;
            std::copy(val.begin() + 96, val.end(), dk.begin());
            if (!pwallet->GetSaplingKeyManager().LoadViewingKey(ivk, fvk, dk)) {
                strErr = "Error reading wallet database: Sapling LoadViewingKey failed";
                return false;
            }
        } else if (strType == DBKeys::SAPLING_ADDR) {
            // Payment address: key=(SAPLING_ADDR, d||pk_d), value=ivk_bytes
            std::vector<uint8_t> addr_vec;
            ssKey >> addr_vec;
            if (addr_vec.size() != 43) {
                strErr = "Error reading wallet database: Sapling address wrong size";
                return false;
            }
            std::vector<uint8_t> ivk_vec;
            ssValue >> ivk_vec;
            if (ivk_vec.size() != 32) {
                strErr = "Error reading wallet database: Sapling IVK wrong size (addr record)";
                return false;
            }
            sapling::SaplingPaymentAddress addr;
            std::copy(addr_vec.begin(), addr_vec.begin() + 11, addr.d.begin());
            std::copy(addr_vec.begin() + 11, addr_vec.end(), addr.pk_d.begin());
            sapling::SaplingIncomingViewingKey ivk;
            std::copy(ivk_vec.begin(), ivk_vec.end(), ivk.key.begin());
            pwallet->GetSaplingKeyManager().LoadAddress(addr, ivk);
        } else if (strType == DBKeys::SAPLING_WITNESS) {
            // Witness: key=(SAPLING_WITNESS, nullifier), value=witness_bytes
            // Deprecated: note data (including witnesses) now stored under SAPLING_NOTE.
        } else if (strType == DBKeys::SAPLING_NOTE) {
            ...
            pwallet->GetSaplingKeyManager().LoadNote(cmu, nd);
                   ...
                   strType != DBKeys::SAPLING_DIVERSIFIER_INDEX) {
...
bool WalletBatch::IsKeyType(const std::string& strType)
{
    return (strType == DBKeys::KEY ||
            ...
            strType == DBKeys::SAPLING_FVK ||
            strType == DBKeys::SAPLING_ADDR ||
            strType == DBKeys::SAPLING_DIVERSIFIER_INDEX);
}
...
DBErrors WalletBatch::LoadWallet(CWallet* pwallet)
{
    ...
    // Validate Sapling key consistency: spending keys must have matching FVKs,
    // and FVKs must have at least one address.
    if (!pwallet->GetSaplingKeyManager().IsEmpty()) {
        int nSaplingIssues = pwallet->GetSaplingKeyManager().ValidateConsistency();
        if (nSaplingIssues > 0) {
            pwallet->WalletLogPrintf("WARNING: Sapling key consistency check found %d issue(s). "
                                     "Shielded functionality may be degraded.\n", nSaplingIssues);
