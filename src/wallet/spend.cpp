                                                     ...
                                                     const CoinSelectionParams& coin_selection_params, CoinType nCoinType)
{
    ...
    if (auto knapsack_result{KnapsackSolver(all_groups, target_with_change, coin_selection_params.m_min_change_target,
                                            coin_selection_params.rng_fast, /*fFullyMixedOnly=*/false,
                                 ...
                                 interfaces::Chain& chain, const uint256& block_hash, int block_height)
{
    ...
    if (IsCurrentForAntiFeeSniping(chain, block_hash)) {
        ...
        // e.g. high-latency mix networks, have better privacy.
        ...
        int nExtraPayloadSize) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (coin_control.m_feerate && coin_selection_params.m_effective_feerate > *coin_control.m_feerate) {
        return util::Error{strprintf(_("Fee rate (%s) is lower than the minimum fee rate setting (%s)"), coin_control.m_feerate->ToString(FeeEstimateMode::KSAT_B), coin_selection_params.m_effective_feerate.ToString(FeeEstimateMode::KSAT_B))};
    }
