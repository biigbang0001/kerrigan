...
public:
    ...
    bool ReadTimestampIndex(const uint32_t high, const uint32_t low, std::vector<uint256>& hashes, size_t maxResults = 0);

    bool WriteFlag(const std::string &name, bool fValue);
    bool ReadFlag(const std::string &name, bool &fValue);
    bool ReadChainVersion(int& nVersion);
    bool WriteChainVersion(int nVersion);
