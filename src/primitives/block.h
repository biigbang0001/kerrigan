...
/** Check whether a mining algorithm is an Equihash variant (200,9 or 192,7). */
inline bool IsEquihash(int algo)
{
    return algo == ALGO_EQUIHASH_200 || algo == ALGO_EQUIHASH_192;
}
...
struct CompressibleBlockHeader : CBlockHeader {
    ...
    SERIALIZE_METHODS(CompressibleBlockHeader, obj)
    {
        ...
        if (!obj.bit_field.IsVersionCompressed()) {
            int algo = obj.GetAlgo();
            bool expect_equihash = IsEquihash(algo);
