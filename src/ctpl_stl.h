...
namespace ctpl {
    ...
    namespace detail {
        ...
        public:
            ...
            // Callers that exceed this limit are rejected (push returns false),
            // providing backpressure against task flooding.
            static constexpr size_t MAX_QUEUE_SIZE = 10000;

            bool push(T const & value) {
                std::unique_lock<std::mutex> lock(this->mutex);
                if (this->q.size() >= MAX_QUEUE_SIZE)
                    return false;
                this->q.push(value);
                return true;
            }
    ...
    public:
        ...
        auto push(F && f, Rest&&... rest) ->std::future<decltype(f(0, rest...))> {
            ...
            if (!this->q.push(_f)) {
                // Queue full; return invalid future so callers can detect via !fut.valid()
                delete _f;
