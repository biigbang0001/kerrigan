...
static constexpr std::string_view DB_HASH_BY_NULLIFIER{"is_nf"};
static constexpr std::string_view DB_NULLIFIERS_BY_ISLOCK{"is_nl"};
...
void CInstantSendDb::WriteNewInstantSendLock(const uint256& hash, const InstantSendLockPtr& islock,
                                             const std::vector<uint256>& saplingNullifiers)
{
    LOCK(cs_db);
    CDBBatch batch(*db);
    ...
    for (const auto& nf : saplingNullifiers) {
        batch.Write(std::make_tuple(DB_HASH_BY_NULLIFIER, nf), hash);
    }
    if (!saplingNullifiers.empty()) {
        batch.Write(std::make_tuple(DB_NULLIFIERS_BY_ISLOCK, hash), saplingNullifiers);
    }
    db->WriteBatch(batch);
    ...
    for (const auto& nf : saplingNullifiers) {
        nullifierCache.insert(nf, hash);
    }
}

void CInstantSendDb::RemoveInstantSendLock(CDBBatch& batch, const uint256& hash, const InstantSendLock& islock,
                                           const std::vector<uint256>& saplingNullifiers, bool keep_cache)
{
    AssertLockHeld(cs_db);

    // If the caller didn't supply nullifiers, look them up from the forward index
    // so that DB_HASH_BY_NULLIFIER entries are always cleaned up.
    const std::vector<uint256>* nfPtr = &saplingNullifiers;
    std::vector<uint256> storedNullifiers;
    if (saplingNullifiers.empty()) {
        if (db->Read(std::make_tuple(DB_NULLIFIERS_BY_ISLOCK, hash), storedNullifiers)) {
            nfPtr = &storedNullifiers;
        }
    }
    ...
    for (const auto& nf : *nfPtr) {
        batch.Erase(std::make_tuple(DB_HASH_BY_NULLIFIER, nf));
    }
    // Always erase the forward index entry
    batch.Erase(std::make_tuple(DB_NULLIFIERS_BY_ISLOCK, hash));
                                                ...
                                                gsl::not_null<const CBlockIndex*> pindexConnected)
{
    ...
    for (const auto& tx : pblock->vtx) {
        ...
        // Skip txs that have no transparent inputs AND are not Sapling
        if (tx->vin.empty() && tx->nType != TRANSACTION_SAPLING) {
            continue;
        }
...
InstantSendLockPtr CInstantSendDb::GetInstantSendLockByNullifier(const uint256& nullifier) const
{
    LOCK(cs_db);
    uint256 islockHash;
    if (!nullifierCache.get(nullifier, islockHash)) {
        if (!db->Read(std::make_tuple(DB_HASH_BY_NULLIFIER, nullifier), islockHash)) {
            return nullptr;
        }
        nullifierCache.insert(nullifier, islockHash);
    }
    return GetInstantSendLockByHashInternal(islockHash);
}
                                                                   ...
                                                                   int nHeight)
{
    ...
    while (!stack.empty()) {
        ...
        for (auto& childIslockHash : children) {
            ...
            RemoveInstantSendLock(batch, childIslockHash, *childIsLock, /*saplingNullifiers=*/{}, /*keep_cache=*/false);
    ...
    if (auto islock = GetInstantSendLockByHashInternal(islockHash, /*use_cache=*/false)) {
        RemoveInstantSendLock(batch, islockHash, *islock, /*saplingNullifiers=*/{}, /*keep_cache=*/false);
    }
    WriteInstantSendLockArchived(batch, islockHash, nHeight);
    result.emplace_back(islockHash);
