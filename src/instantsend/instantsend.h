...
/**
 * Extract Sapling nullifiers from a transaction.
 * Returns an empty vector for non-Sapling transactions.
 */
std::vector<uint256> GetSaplingNullifiers(const CTransaction& tx);

/** Returns true if a transaction has IS-lockable inputs (transparent outpoints or Sapling nullifiers). */
...
private:
    ...
    /** Maps Sapling nullifiers from non-locked txs to txid, parallel to nonLockedTxsByOutpoints. */
    Uint256HashMap<uint256> nonLockedTxsByNullifiers GUARDED_BY(cs_nonLocked);
