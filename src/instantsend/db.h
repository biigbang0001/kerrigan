...
private:
    ...
    mutable Uint256LruHashMap<uint256, 10000> nullifierCache GUARDED_BY(cs_db);
     ...
     * @param saplingNullifiers Sapling nullifiers from the locked transaction (empty for non-Sapling txs)
...
public:
     ...
     * @param saplingNullifiers Sapling nullifiers from the locked transaction (empty for non-Sapling txs)
     */
    void WriteNewInstantSendLock(const uint256& hash, const InstantSendLockPtr& islock,
                                 const std::vector<uint256>& saplingNullifiers = {}) EXCLUSIVE_LOCKS_REQUIRED(!cs_db);
    /**
    ...
    /**
     * Gets an IS Lock pointer from a Sapling nullifier.
     * @param nullifier The 32-byte Sapling nullifier being spent
     * @return IS Lock Pointer associated with that nullifier, or nullptr if none.
     */
    InstantSendLockPtr GetInstantSendLockByNullifier(const uint256& nullifier) const EXCLUSIVE_LOCKS_REQUIRED(!cs_db);
    /**
