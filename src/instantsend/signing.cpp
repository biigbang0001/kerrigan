...
bool InstantSendSigner::CheckCanLock(const CTransaction& tx, bool printDebug, const Consensus::Params& params) const
{
    ...
    // Check all transparent inputs can be locked
    if (!ranges::all_of(tx.vin,
                        [&](const auto& in) { return CheckCanLock(in.prevout, printDebug, tx.GetHash(), params); })) {
        return false;
    }

    // Sapling spends reference the commitment tree (not parent UTXOs), so they don't need
    // parent-depth checks. If we reach here, the Sapling tx is lockable.
    return true;
}
...
{
    // Sapling transactions must not enter InstantSend. IS only locks transparent
    // inputs and has no awareness of shielded spends/outputs.
