...
std::vector<uint256> GetSaplingNullifiers(const CTransaction& tx)
{
    std::vector<uint256> nullifiers;
    if (tx.nType != TRANSACTION_SAPLING) return nullifiers;

    auto payload = GetTxPayload<SaplingTxPayload>(tx);
    if (!payload) return nullifiers;

    nullifiers.reserve(payload->vSpendDescriptions.size());
    for (const auto& spend : payload->vSpendDescriptions) {
        uint256 nf;
        static_assert(sizeof(spend.nullifier) == 32);
        std::memcpy(nf.begin(), spend.nullifier.data(), 32);
        nullifiers.emplace_back(nf);
    }
    return nullifiers;
}

bool HasLockableInputs(const CTransaction& tx)
{
    if (!tx.vin.empty()) return true;
    if (tx.nType == TRANSACTION_SAPLING) {
        auto payload = GetTxPayload<SaplingTxPayload>(tx);
        if (payload && !payload->vSpendDescriptions.empty()) return true;
    }
    return false;
}
...
instantsend::PendingState CInstantSendManager::FetchPendingLocks()
{
    ...
    // Evict pendingNoTxInstantSendLocks entries older than 15 minutes.
    // These are IS locks whose transactions never appeared; without this
    // cleanup the map grows unboundedly under sustained load.
    static constexpr int64_t PENDING_NOTX_TIMEOUT = 60 * 15; // 15 minutes
    {
        int64_t now = GetTime();
        for (auto it = pendingNoTxInstantSendLocks.begin(); it != pendingNoTxInstantSendLocks.end();) {
            if (now - it->second.nTimeAdded > PENDING_NOTX_TIMEOUT) {
                it = pendingNoTxInstantSendLocks.erase(it);
            } else {
                ++it;
            }
        }
    ...
    NodeId from, const uint256& hash, const instantsend::InstantSendLockPtr& islock)
{
    ...
    for (const auto& in : islock->inputs) {
        ...
        if (sameOutpointIsLock != nullptr && sameOutpointIsLock->txid != islock->txid) {
            LogPrintf("CInstantSendManager::%s -- txid=%s, islock=%s: conflicting outpoint in islock, rejecting. input=%s, other islock=%s, peer=%d\n", __func__,
                      islock->txid.ToString(), hash.ToString(), in.ToStringShort(), ::SerializeHash(*sameOutpointIsLock).ToString(), from);
            return std::monostate{};
        }
    }

    // We can't check nullifier conflicts here yet because we don't have the tx.
    // Nullifier conflicts are checked later via GetConflictingLock when the tx is known.
    ...
    if (found_transaction) {
        // Extract Sapling nullifiers from the tx so we can index them for conflict detection
        const auto saplingNullifiers = GetSaplingNullifiers(*tx);
        db.WriteNewInstantSendLock(hash, islock, saplingNullifiers);
        if (minedHeight.has_value()) {
...
{
    if (!IsInstantSendEnabled() || !m_mn_sync.IsBlockchainSynced() || !HasLockableInputs(*tx)) {
        return;
    }
...
void CInstantSendManager::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex)
{
    ...
    if (m_mn_sync.IsBlockchainSynced()) {
        ...
        for (const auto& tx : pblock->vtx) {
            if (tx->IsCoinBase() || !HasLockableInputs(*tx)) {
                // coinbase and TXs with no lockable inputs can't be locked
                continue;
            }
...
{
    {
        ...
        if (did_insert) {
            ...
            // Also track Sapling nullifiers for conflict detection
            for (const auto& nf : GetSaplingNullifiers(*tx)) {
                nonLockedTxsByNullifiers.emplace(nf, tx->GetHash());
            }
        }
    }
    {
...
void CInstantSendManager::RemoveNonLockedTx(const uint256& txid, bool retryChildren)
{
    ...
    if (info.tx) {
        ...
        // Also clean up Sapling nullifier tracking
        for (const auto& nf : GetSaplingNullifiers(*info.tx)) {
            nonLockedTxsByNullifiers.erase(nf);
        }
    }
...
void CInstantSendManager::UpdatedBlockTip(const CBlockIndex* pindexNew)
{
    ...
    if (m_chainlocks.IsEnabled() && fDIP0008Active) {
        // Only skip depth-based cleanup if ChainLocks are actually working
        // and reasonably recent. With 0 masternodes or stale locks,
        // nonLockedTxs would grow unboundedly without the fallback.
        int bestClHeight = m_chainlocks.GetBestChainLockHeight();
        if (bestClHeight > 0 &&
            pindexNew->nHeight - bestClHeight < Params().GetConsensus().nInstantSendKeepLock) {
            return;
        }
        // Fall through to depth-based cleanup
    }
...
void CInstantSendManager::RemoveMempoolConflictsForLock(const uint256& hash, const instantsend::InstantSendLock& islock)
{
...

    {
        ...
        // Check transparent outpoint conflicts
        for (const auto& in : islock.inputs) {
        ...
        // Check Sapling nullifier conflicts: find the locked tx's nullifiers and look for
        // other mempool txs that spend the same nullifiers
        if (auto lockedTx = mempool.get(islock.txid)) {
            for (const auto& nf : GetSaplingNullifiers(*lockedTx)) {
                const uint256 conflictTxid = mempool.GetSaplingNullifierTxid(nf);
                if (conflictTxid.IsNull()) {
                    continue;
                }
                if (conflictTxid != islock.txid) {
                    auto conflictTx = mempool.get(conflictTxid);
                    if (conflictTx) {
                        toDelete.emplace(conflictTxid, conflictTx);
                        LogPrintf("CInstantSendManager::%s -- txid=%s, islock=%s: mempool TX %s with nullifier %s conflicts with islock\n", __func__,
                                  islock.txid.ToString(), hash.ToString(), conflictTxid.ToString(), nf.ToString());
                    }
                }
            }
        }
...
void CInstantSendManager::ResolveBlockConflicts(const uint256& islockHash, const instantsend::InstantSendLock& islock)
{
    ...
    std::unordered_map<const CBlockIndex*, Uint256HashMap<CTransactionRef>> conflicts;
    {
        ...
        // Check Sapling nullifier conflicts: we need the locked tx's nullifiers.
        // The locked tx might already have been removed from nonLockedTxs, so try
        // fetching it from there first, then fall back to mempool lookup.
        CTransactionRef lockedTxRef;
        if (auto lockedIt = nonLockedTxs.find(islock.txid); lockedIt != nonLockedTxs.end() && lockedIt->second.tx) {
            lockedTxRef = lockedIt->second.tx;
        }
        if (!lockedTxRef) {
            lockedTxRef = mempool.get(islock.txid);
        }
        if (lockedTxRef) {
            for (const auto& nf : GetSaplingNullifiers(*lockedTxRef)) {
                auto it = nonLockedTxsByNullifiers.find(nf);
                if (it != nonLockedTxsByNullifiers.end()) {
                    auto& conflictTxid = it->second;
                    if (conflictTxid == islock.txid) {
                        continue;
                    }
                    ...
                    LogPrintf("CInstantSendManager::%s -- txid=%s, islock=%s: mined TX %s with nullifier %s and mined in block %s conflicts with islock\n", __func__,
                              islock.txid.ToString(), islockHash.ToString(), conflictTxid.ToString(), nf.ToString(), info.pindexMined->GetBlockHash().ToString());
                    conflicts[info.pindexMined].emplace(conflictTxid, info.tx);
                }
            }
        }
...
instantsend::InstantSendLockPtr CInstantSendManager::GetConflictingLock(const CTransaction& tx) const
{
    ...
    // Check transparent input (outpoint) conflicts
    for (const auto& in : tx.vin) {
    ...
    // Check Sapling nullifier conflicts
    for (const auto& nf : GetSaplingNullifiers(tx)) {
        auto otherIsLock = db.GetInstantSendLockByNullifier(nf);
        if (!otherIsLock) {
            continue;
        }
