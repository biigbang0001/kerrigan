...
public:
    ...
    void Xor(const std::vector<unsigned char>& key)
    {
        ...
        for (size_type i = 0, j = 0; i != size(); i++) {
            vch[i + m_read_pos] ^= std::byte{key[j++]};
...
public:
    ...
    /** CXX Rust FFI bridge methods */
    void read_u8(uint8_t* pch, size_t nSize) { read(MakeWritableByteSpan(Span{pch, nSize})); }
...
private:
    ...
    bool Fill() {
        ...
        size_t nAvail = vchBuf.size() - (nSrcPos - m_read_pos) - nRewind;
        if (nAvail < readNow)
            readNow = nAvail;
        if (readNow == 0)
            return false;
...
/** Wrapper around DataStream exposing read_u8/write_u8 for CXX Rust FFI bridge. */
class RustDataStream : public DataStream {
public:
    using DataStream::DataStream;

    void read_u8(uint8_t* pch, size_t nSize) {
        read(MakeWritableByteSpan(Span{pch, nSize}));
    }
    void write_u8(const uint8_t* pch, size_t nSize) {
        write(MakeByteSpan(Span{pch, nSize}));
    }
};
