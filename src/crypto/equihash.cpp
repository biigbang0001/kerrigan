...
{
    uint32_t le_N = htole32_internal(N);
    uint32_t le_K = htole32_internal(K);
    unsigned char personalization[crypto_generichash_blake2b_PERSONALBYTES] = {};
    // 200,9: "ZcashPoW" for ASIC compatibility (standard Zcash personalization)
    // 192,7: "kerrigan" (custom, no ASICs for this variant)
    if constexpr (N == 200 && K == 9) {
        memcpy(personalization, "ZcashPoW", 8);
    } else {
        memcpy(personalization, "kerrigan", 8);
    }
...
static void GenerateHash(const eh_HashState& base_state, eh_index g,
                  unsigned char* hash, size_t hLen)
{
    eh_HashState state;
    state = base_state;
    eh_index lei = htole32_internal(g);
...
{
    if (bit_len < 8) throw std::length_error("ExpandArray: bit_len < 8");
    if (8*sizeof(uint64_t) < 7+bit_len) throw std::length_error("ExpandArray: bit_len exceeds uint64_t");
    assert(bit_len >= 8);
    assert(8*sizeof(uint64_t) >= 7+bit_len);

    size_t out_width { (bit_len+7)/8 + byte_pad };
    if (out_len != 8*out_width*in_len/bit_len) throw std::length_error("ExpandArray: out_len mismatch");
...
void CompressArray(const unsigned char* in, size_t in_len,
                   ...
                   size_t bit_len, size_t byte_pad)
{
    if (bit_len < 8) throw std::length_error("CompressArray: bit_len < 8");
    if (8*sizeof(uint64_t) < 7+bit_len) throw std::length_error("CompressArray: bit_len exceeds uint64_t");
    assert(bit_len >= 8);
    assert(8*sizeof(uint64_t) >= 7+bit_len);

    size_t in_width { (bit_len+7)/8 + byte_pad };
    if (out_len != bit_len*in_len/(8*in_width)) throw std::length_error("CompressArray: out_len mismatch");
...
void EhIndexToArray(const eh_index i, unsigned char* array)
{
    ...
    eh_index bei = htobe32_internal(i);
...
static eh_index UntruncateIndex(const eh_trunc t, const eh_index r, const unsigned int ilen)
{
    eh_index i{t};
    return (i << (ilen - 8)) | r;
}
...
std::vector<eh_index> GetIndicesFromMinimal(std::vector<unsigned char> minimal,
                                            size_t cBitLen)
{
    if (((cBitLen+1)+7)/8 > sizeof(eh_index)) throw std::length_error("GetIndicesFromMinimal: cBitLen exceeds eh_index size");
