...
// Standard Ravencoin KawPoW constants for wire-compatibility with existing
// RVN mining software (kawpowminer, T-Rex, etc.). Custom constants would
// require custom miner builds with zero hashrate at launch.
    ...
    uint64_t nonce) noexcept
{
...

    {
        ...
        // 3rd apply kerrigan input constraints
        for (int i = 10; i < 25; i++)
...
result hash(const epoch_context_full& context, int block_number, const hash256& header_hash,
    uint64_t nonce) noexcept
{
    /// @note NOT thread-safe. The full dataset is lazily populated without
    /// synchronization. Only call from a single mining thread per epoch context.
