...
void CTxMemPool::addUnchecked(const CTxMemPoolEntry &entry, setEntries &setAncestors, bool validFeeEstimate)
{
    ...
    // Track Sapling nullifiers to prevent in-mempool double-spend.
    AddSaplingNullifiers(tx);
}
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    // Remove Sapling nullifiers so they can be re-spent by future txs.
    RemoveSaplingNullifiers(it->GetTx());
...
bool CTxMemPool::CheckSaplingNullifiers(const CTransaction& tx) const
{
    AssertLockHeld(cs);
    if (tx.nType != TRANSACTION_SAPLING) return true;
    auto payloadOpt = GetTxPayload<SaplingTxPayload>(tx.vExtraPayload);
    if (!payloadOpt) return true;
    for (const auto& spend : payloadOpt->vSpendDescriptions) {
        uint256 nf;
        std::copy(spend.nullifier.begin(), spend.nullifier.end(), nf.begin());
        if (m_sapling_nullifiers.count(nf)) return false;
    }
    return true;
}

void CTxMemPool::AddSaplingNullifiers(const CTransaction& tx)
{
    AssertLockHeld(cs);
    ...
    for (const auto& spend : payloadOpt->vSpendDescriptions) {
        uint256 nf;
        std::copy(spend.nullifier.begin(), spend.nullifier.end(), nf.begin());
        m_sapling_nullifiers.insert(nf);
        m_sapling_nullifier_txids[nf] = txid;
    }
}

void CTxMemPool::RemoveSaplingNullifiers(const CTransaction& tx)
{
    AssertLockHeld(cs);
    if (tx.nType != TRANSACTION_SAPLING) return;
    auto payloadOpt = GetTxPayload<SaplingTxPayload>(tx.vExtraPayload);
    ...
    for (const auto& spend : payloadOpt->vSpendDescriptions) {
        uint256 nf;
        std::copy(spend.nullifier.begin(), spend.nullifier.end(), nf.begin());
        m_sapling_nullifiers.erase(nf);
        m_sapling_nullifier_txids.erase(nf);
    }
}

uint256 CTxMemPool::GetSaplingNullifierTxid(const uint256& nullifier) const
{
    AssertLockHeld(cs);
    auto it = m_sapling_nullifier_txids.find(nullifier);
    if (it != m_sapling_nullifier_txids.end()) {
        return it->second;
    }
    return uint256();
}

void CTxMemPool::removeSaplingNullifierConflicts(const CTransaction& tx)
{
    AssertLockHeld(cs);
    if (tx.nType != TRANSACTION_SAPLING) return;
    auto payloadOpt = GetTxPayload<SaplingTxPayload>(tx.vExtraPayload);
    ...
    for (const auto& spend : payloadOpt->vSpendDescriptions) {
        uint256 nf;
        std::copy(spend.nullifier.begin(), spend.nullifier.end(), nf.begin());
        auto it = m_sapling_nullifier_txids.find(nf);
        if (it != m_sapling_nullifier_txids.end() && it->second != tx.GetHash()) {
            auto txit = mapTx.find(it->second);
            if (txit != mapTx.end()) {
                ClearPrioritisation(it->second);
...
void CTxMemPool::removeForBlock(const std::vector<CTransactionRef>& vtx, unsigned int nBlockHeight)
{
    ...
    for (const auto& tx : vtx)
    {
        ...
        removeConflicts(*tx);
        removeSaplingNullifierConflicts(*tx);
        if (m_dmnman.load(std::memory_order_acquire)) {
...
void CTxMemPool::_clear()
{
    ...
    mapProTxRefs.clear();
    mapAssetUnlockExpiry.clear();
    m_sapling_nullifiers.clear();
    m_sapling_nullifier_txids.clear();
    totalTxSize = 0;
    m_total_fee = 0;
    cachedInnerUsage = 0;
    lastRollingFeeUpdate = GetTime();
...
void CTxMemPool::check(const CCoinsViewCache& active_coins_tip, int64_t spendheight) const
{
    ...
    // Verify Sapling nullifier consistency
    for (const auto& [nf, txid] : m_sapling_nullifier_txids) {
        assert(mapTx.count(txid) > 0);
        assert(m_sapling_nullifiers.count(nf) > 0);
    }
    for (const auto& nf : m_sapling_nullifiers) {
        assert(m_sapling_nullifier_txids.count(nf) > 0);
    }
...
size_t CTxMemPool::DynamicMemoryUsage() const {
    ...
    return memusage::MallocUsage(sizeof(CTxMemPoolEntry) + 12 * sizeof(void*)) * mapTx.size() + memusage::DynamicUsage(mapNextTx) + memusage::DynamicUsage(mapDeltas) + memusage::DynamicUsage(vTxHashes) + memusage::DynamicUsage(m_sapling_nullifiers) + memusage::DynamicUsage(m_sapling_nullifier_txids) + memusage::DynamicUsage(mapProTxRefs) + memusage::DynamicUsage(mapProTxAddresses) + memusage::DynamicUsage(mapProTxPubKeyIDs) + memusage::DynamicUsage(mapProTxBlsPubKeyHashes) + memusage::DynamicUsage(mapProTxCollaterals) + memusage::DynamicUsage(mapAssetUnlockExpiry) + cachedInnerUsage;
}
