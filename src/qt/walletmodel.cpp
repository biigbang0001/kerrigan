...
void WalletModel::lockExistingDustOutputs()
{
    ...
    // When dust protection is disabled, unlock previously locked dust outputs
...
// Helper: invoke a method on a QObject from any thread. Returns false on failure
// instead of aborting. Wallet signal handlers must never crash the app.
static bool SafeInvoke(QObject* obj, const char* method, Qt::ConnectionType type = Qt::QueuedConnection)
{
    bool invoked = QMetaObject::invokeMethod(obj, method, type);
    if (!invoked) {
        qWarning() << "WalletModel: failed to invoke" << method << "(object may be destroyed)";
    }
    return invoked;
}
...
{
    qDebug() << "NotifyUnload";
    SafeInvoke(walletModel, "unload");
}

static void NotifyKeyStoreStatusChanged(WalletModel *walletmodel)
{
    qDebug() << "NotifyKeyStoreStatusChanged";
    SafeInvoke(walletmodel, "updateStatus");
}
        ...
        const std::string &purpose, ChangeType status)
{
    ...
    bool invoked = QMetaObject::invokeMethod(walletmodel, "updateAddressBook",
                              ...
                              Q_ARG(int, status));
    if (!invoked) qWarning() << "WalletModel: failed to invoke updateAddressBook";
}

static void NotifyTransactionChanged(WalletModel *walletmodel, const uint256 &hash, ChangeType status)
{
    SafeInvoke(walletmodel, "updateTransaction");

    // For new transactions, check if dust protection should lock UTXOs
    if (status == CT_NEW) {
        QString hashStr = QString::fromStdString(hash.ToString());
        bool invoked = QMetaObject::invokeMethod(walletmodel, "checkAndLockDustOutputs", Qt::QueuedConnection,
                                            Q_ARG(QString, hashStr));
        if (!invoked) qWarning() << "WalletModel: failed to invoke checkAndLockDustOutputs";
    }
}

static void NotifyISLockReceived(WalletModel *walletmodel)
{
    SafeInvoke(walletmodel, "updateNumISLocks");
}
...
static void NotifyChainLockReceived(WalletModel *walletmodel, int chainLockHeight)
{
    ...
    if (!invoked) qWarning() << "WalletModel: failed to invoke updateChainLockHeight";
}
...
static void ShowProgress(WalletModel *walletmodel, const std::string &title, int nProgress)
{
    bool invoked = QMetaObject::invokeMethod(walletmodel, "showProgress", Qt::QueuedConnection,
                              ...
                              Q_ARG(int, nProgress));
    if (!invoked) qWarning() << "WalletModel: failed to invoke showProgress";
}
...
static void NotifyWatchonlyChanged(WalletModel *walletmodel, bool fHaveWatchonly)
{
    ...
    if (!invoked) qWarning() << "WalletModel: failed to invoke updateWatchOnlyFlag";
}

static void NotifyCanGetAddressesChanged(WalletModel* walletmodel)
{
    SafeInvoke(walletmodel, "canGetAddressesChanged", Qt::AutoConnection);
}
