...
bool loadFonts()
{
    ...
    // Import Kerrigan branding fonts (Orbitron for headings, Inter for body)
    importFont(":fonts/Orbitron-Bold");
    importFont(":fonts/Inter-Regular");
    importFont(":fonts/Inter-Bold");
...
void updateFonts()
{
    ...
    for (QWidget* w : qApp->allWidgets()) {
        ...
        QFont font = w->font();
        if (font.pointSize() <= 0) {
            // Widget uses pixel sizing; convert to points to avoid assertion failure
            int px = font.pixelSize();
            if (px <= 0) px = 10;
            font.setPointSize(std::max(1, px * 72 / 96));
        }
