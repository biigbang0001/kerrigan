...
{
    // Clear seed words from QLabel widgets before destruction
    if (m_gridLayout) {
        for (int i = 0; i < m_gridLayout->count(); ++i) {
            QLayoutItem* item = m_gridLayout->itemAt(i);
            if (item && item->widget()) {
                if (QLabel* lbl = qobject_cast<QLabel*>(item->widget())) {
                    QString t = lbl->text();
                    t.fill(QChar(0));
                    lbl->setText(t);
                }
...
int MnemonicVerificationDialog::getWordCount() const
{
    ...
    if (m_words.empty()) {
        ...
        // Zero before clearing
        mnemonicStr.fill(QChar(0));
        mnemonicStr.clear();
        mnemonicStr.squeeze();
        for (auto& w : words) { w.fill(QChar(0)); }
        words.clear();
        return count;
    }
    return m_words.size();
