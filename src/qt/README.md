This directory contains the source code for the Kerrigan graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Kerrigan UI.
...
- ClientModel is used by the main application and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a kerrigan:... link from a browser).
...
* `bitcoinunits.(h/cpp)`: KRGN / mKRGN / etc. handling
* `callback.h`
...
**Note:** Do not change `local/kerrigan_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `kerrigan-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `kerrigan-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `kerrigan-qt`
9. You're all set! Start developing, building, and debugging the Kerrigan GUI
