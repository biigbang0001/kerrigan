...
const QString AddressTableModel::ShieldedReceive = "ZR";
...
public:
    ...
    void refreshAddressTable(interfaces::Wallet& wallet)
    {
        ...
        // Include Sapling shielded addresses
        try {
            for (const auto& addr : wallet.listSaplingAddresses()) {
                cachedAddressTable.append(AddressTableEntry(
                    AddressTableEntry::ShieldedReceiving,
                    QString("(shielded)"),
                    QString::fromStdString(addr)));
            }
        } catch (...) {
            // Sapling may not be initialized yet
        }
...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        case AddressTableEntry::Receiving:
            return Receive;
        case AddressTableEntry::ShieldedReceiving:
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address)
{
    ...
    else if(type == ShieldedReceive)
    {
        // Generate a new Sapling shielded address
        try {
            strAddress = walletModel->wallet().getNewSaplingAddress();
...
bool AddressTableModel::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    if(count != 1 || !rec || rec->type == AddressTableEntry::Receiving || rec->type == AddressTableEntry::ShieldedReceiving)
    {
