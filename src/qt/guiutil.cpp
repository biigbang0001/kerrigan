...
static const QString defaultTheme = "Kerrigan";
...
// The Kerrigan branded theme name
static const QString kerriganTheme = "Kerrigan";
...
static const std::map<ThemedColor, QColor> themedKerriganColors = {
    { ThemedColor::DEFAULT, QColor(226, 232, 240) },
    { ThemedColor::UNCONFIRMED, QColor(148, 163, 184) },
    { ThemedColor::BLUE, QColor(34, 197, 94) },
    { ThemedColor::ORANGE, QColor(251, 146, 60) },
    { ThemedColor::RED, QColor(239, 68, 68) },
    { ThemedColor::GREEN, QColor(34, 197, 94) },
    { ThemedColor::BAREADDRESS, QColor(148, 163, 184) },
    { ThemedColor::TX_STATUS_OPENUNTILDATE, QColor(100, 116, 139) },
    { ThemedColor::BACKGROUND_WIDGET, QColor(26, 10, 46) },
    { ThemedColor::BORDER_WIDGET, QColor(124, 58, 237) },
    { ThemedColor::BACKGROUND_NETSTATS, QColor(45, 27, 78) },
    { ThemedColor::BORDER_NETSTATS, QColor(124, 58, 237) },
    { ThemedColor::QR_PIXEL, QColor(34, 197, 94) },
    { ThemedColor::ICON_ALTERNATIVE_COLOR, QColor(124, 58, 237) },
};

static const std::map<ThemedStyle, QString> themedKerriganStyles = {
    { ThemedStyle::TS_INVALID, "background:#7C3AED;" },
    { ThemedStyle::TS_ERROR, "color:#EF4444;" },
    { ThemedStyle::TS_WARNING, "color:#FB923C;" },
    { ThemedStyle::TS_SUCCESS, "color:#22C55E;" },
    { ThemedStyle::TS_COMMAND, "color:#22C55E;" },
    { ThemedStyle::TS_PRIMARY, "color:#22C55E;" },
    { ThemedStyle::TS_SECONDARY, "color:#7C3AED;" },
};
...
QColor getThemedQColor(ThemedColor color)
{
    ...
    if (theme == kerriganTheme) return themedKerriganColors.at(color);
...
{
    QString theme = QSettings().value("theme", "").toString();
    if (theme == kerriganTheme) return themedKerriganStyles.at(style);
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool fAllowURI)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Kerrigan address (e.g. %1)").arg(
...
void setupAppearance(QWidget* parent, OptionsModel* model)
{
    if (!QSettings().value("fAppearanceSetupDone", false).toBool()) {
        ...
        dlg.setWindowIcon(QIcon(":icons/kerrigan"));
...
{
    // return if URI is not valid or is no kerrigan: URI
    if(!uri.isValid() || uri.scheme() != QString("kerrigan"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BitcoinUnits::parse(BitcoinUnit::KRGN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    QString ret = QString("kerrigan:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnit::KRGN, info.amount, false, BitcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open kerrigan.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kerrigan.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Kerrigan (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Kerrigan (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for "Kerrigan*.lnk"
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "kerrigan.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("kerrigan-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a kerrigan.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Kerrigan\n";
        else
            optionFile << strprintf("Name=Kerrigan (%s)\n", chain);
...
{
    return mapThemeToStyle.count(strTheme) && strTheme != generalTheme;
}
...
void disableMacFocusRect(const QWidget* w)
{
    ...
    for (const auto& c : w->findChildren<QWidget*>()) {
        if (c->testAttribute(Qt::WA_MacShowFocusRect)) {
            c->setAttribute(Qt::WA_MacShowFocusRect, !darkThemeActive());
...
void updateMacFocusRects()
{
    ...
    while (it != setRectsDisabled.end()) {
        if (allWidgets.contains(*it)) {
            (*it)->setAttribute(Qt::WA_MacShowFocusRect, !darkThemeActive());
            ++it;
        } else {
