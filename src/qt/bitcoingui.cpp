...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Kerrigan address"));

    receiveCoinsAction = new QAction(tr("&Receive"), this);
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kerrigan: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Kerrigan addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Kerrigan addresses"));
    ...
    openAction->setStatusTip(tr("Open a kerrigan: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Kerrigan command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    ...
    if(walletFrame)
    {
        ...
        shieldedButton = new QToolButton(this);
        shieldedButton->setText(tr("&Shielded"));
        shieldedButton->setStatusTip(tr("View shielded transaction history"));
        tabGroup->addButton(shieldedButton);
        ...
        connect(shieldedButton, &QToolButton::clicked, this, &BitcoinGUI::gotoShieldedTransactionsPage);
        ...
        for (auto button : tabGroup->buttons()) {
            ...
            if (button == governanceButton) { m_governance_action = action; }
...
void BitcoinGUI::createIconMenu(QMenu *pmenu)
{
    ...
    connect(
        ...
        [this, show_hide_action, send_action, receive_action, sign_action, verify_action, options_action, node_window_action, quit_action, repair_action, backups_action, info_action, graph_action, peer_action, debuglog_action, conf_action] {
...
void BitcoinGUI::gotoShieldedTransactionsPage()
{
    if (shieldedButton) {
        shieldedButton->setChecked(true);
        if (walletFrame) walletFrame->gotoShieldedTransactionsPage();
    }
}
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        ...
        tooltip = tr("%n active connection(s) to Kerrigan network", "", count);
    } else {
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the logo
...
{
#ifdef Q_OS_MACOS
    // Disabling macOS App Nap on initial sync, disk, reindex operations.
