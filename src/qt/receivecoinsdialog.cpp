...
void ReceiveCoinsDialog::on_generateShieldedButton_clicked()
{
    if (!model)
        return;
    ...
    if (address.isEmpty()) {
        QMessageBox::critical(this, windowTitle(),
            tr("Could not generate shielded address."),
            QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    QMessageBox::information(this, tr("Shielded Address Generated"),
        tr("Your new shielded address has been copied to the clipboard:\n\n%1\n\n"
           "Share this address to receive private payments.").arg(address));
}
