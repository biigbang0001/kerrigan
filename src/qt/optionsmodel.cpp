...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayUnit")) {
        settings.setValue("DisplayUnit", QVariant::fromValue(BitcoinUnit::KRGN));
    }
    QVariant unit = settings.value("DisplayUnit");
    if (unit.canConvert<BitcoinUnit>()) {
        m_display_bitcoin_unit = unit.value<BitcoinUnit>();
    } else {
        m_display_bitcoin_unit = BitcoinUnit::KRGN;
        settings.setValue("DisplayUnit", QVariant::fromValue(m_display_bitcoin_unit));
    }
