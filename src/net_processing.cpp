 ...
 *  Kerrigan has 5x smaller block times (120s vs 600s), so we push more invs per 1MB. */
static constexpr unsigned int INVENTORY_BROADCAST_PER_SECOND = 9;
...
struct Peer {
     ...
     * (Kerrigan) Enables the flag that allows GetTxRelay() to return m_tx_relay */
    ...
    bool HMPMarkSeen(const uint256& hash) EXCLUSIVE_LOCKS_REQUIRED(!m_hmp_seen_mutex) {
        ...
        m_hmp_seen.insert(hash);
        m_hmp_seen_order.push_back(hash);
        return true;
    }
...
private:
     ...
     * (Kerrigan) Always initialized but selectively available through GetTxRelay()
...
struct CNodeState {
    ...
    //! Whether sync gate diagnostic was logged for this peer.
    bool m_sync_gate_logged{false};
 ...
 * Disabled: Kerrigan's multi-algo blocks use extended serialization fields
 * (hashReserved, nNonce256, nSolution for Equihash; nHeight, nNonce64,
 * mix_hash for KawPoW) that differ between CompressibleBlockHeader versions.
 * Standard GETHEADERS/HEADERS avoid the version-mismatch deserialization bug
 * that corrupts the stream when sender and receiver use different field order. */
static bool UsesCompressedHeaders([[maybe_unused]] const Peer& peer)
{
    return false;
}
...
void PeerManagerImpl::FinalizeNode(const CNode& node) {
    ...
    // Clean up per-NodeId HMP rate-limit maps on disconnect
    {
        LOCK(m_hmp_rate_mutex);
        m_sealshare_last_request.erase(nodeid);
        ...
        auto prune = [&](std::map<CNetAddr, int64_t>& m) {
            for (auto it = m.begin(); it != m.end(); ) {
                if (nNow - it->second > IP_RATE_STALE_EXPIRY) it = m.erase(it);
                else ++it;
            }
        };
        prune(m_hmp_ip_sealshare);
        prune(m_hmp_ip_sealasm);
        prune(m_hmp_ip_pubkeycommit);
    }
...
std::unique_ptr<PeerManager> PeerManager::make(const CChainParams& chainparams, CConnman& connman, AddrMan& addrman,
                                               ...
                                               const std::unique_ptr<llmq::ObserverContext>& observer_ctx, bool ignore_incoming_txs)
{
    return std::make_unique<PeerManagerImpl>(chainparams, connman, addrman, banman, chainman, pool, mn_metaman, mn_sync, govman, sporkman, chainlocks, clhandler, active_ctx, dmnman, llmq_ctx, observer_ctx, ignore_incoming_txs);
}
...
bool PeerManagerImpl::AlreadyHave(const CInv& inv)
{
    ...
    case MSG_TX:
        {
            ...
            bool fIgnoreRecentRejects = m_llmq_ctx->isman->IsWaitingForTx(inv.hash) ||
    ...
    /*
        Kerrigan Related Inventory Messages
...
void PeerManagerImpl::ProcessGetBlockData(CNode& pfrom, Peer& peer, const CInv& inv, llmq::CInstantSendManager& isman)
{
    ...
    if (a_recent_block && a_recent_block->GetHash() == pindex->GetBlockHash()) {
        ...
        if (!ReadBlockFromDisk(*pblockRead, pindex, m_chainparams.GetConsensus())) {
            LogPrintf("ERROR: %s: Cannot load block %s from disk\n", __func__, pindex->GetBlockHash().ToString());
            return;
        }
        pblock = pblockRead;
    }
    ...
    const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::QSENDRECSIGS) {
        if (pfrom.GetVerifiedProRegTxHash().IsNull()) {
            LogPrint(BCLog::NET, "Ignoring qsendrecsigs from non-masternode peer=%d\n", pfrom.GetId());
            return;
        }
        bool b;
        vRecv >> b;
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        // Rate-limit getblocks to prevent peers from spamming ActivateBestChain
        const auto now = GetTime<std::chrono::microseconds>();
        if (now - peer->m_last_getblocks_timestamp.load() < 2s) {
            LogPrint(BCLog::NET, "Ignoring repeated getblocks from peer=%d (rate limited)\n", pfrom.GetId());
            return;
        }
        peer->m_last_getblocks_timestamp = now;
        ...
        if (pindex->nHeight <= m_chainman.ActiveChain().Height() + 2) {
            if ((!fAlreadyInFlight && nodestate->nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (status == READ_STATUS_INVALID) {
                    ...
                    return; // READ_STATUS_FAILED: non-malicious (e.g. duplicate txindexes)
                }
    ...
    if (msg_type == NetMsgType::HEADERS || msg_type == NetMsgType::HEADERS2) {
        ...
        if (msg_type == NetMsgType::HEADERS) {
            ...
            for (unsigned int n = 0; n < nCount; n++) {
                ...
                if (!block_header_compressed.Uncompress(headers, last_unique_versions)) {
                    Misbehaving(pfrom.GetId(), 20, strprintf("headers2 invalid version_offset in header %u", n));
                    return;
                }
    ...
    if (msg_type == NetMsgType::GETMNLISTDIFF) {
        ...
        auto& last = m_mnlistdiff_last_request[pfrom.addr];
        if (now - last < MNLISTDIFF_COOLDOWN) {
            Misbehaving(pfrom.GetId(), 10, "repeated getmnlistdiff");
            return;
        }
        last = now;
                                      ...
                                      cmd.baseBlockHash, cmd.blockHash, mnListDiff, strError))
        {
            ...
            Misbehaving(pfrom.GetId(), 10, strError);
        }
        return;
    }
    ...
    if (msg_type == NetMsgType::GETQUORUMROTATIONINFO) {
        ...
        static constexpr size_t MAX_BASE_BLOCK_HASHES = 32;
        if (cmd.baseBlockHashes.size() > MAX_BASE_BLOCK_HASHES) {
            Misbehaving(pfrom.GetId(), 100,
                strprintf("baseBlockHashes size %u exceeds limit %u",
                          cmd.baseBlockHashes.size(), MAX_BASE_BLOCK_HASHES));
            return;
        }
        ...
        // Rate limit: 5s cooldown per peer, matching GETMNLISTDIFF
        {
            ...
            if (now - last < QRINFO_COOLDOWN) {
                Misbehaving(pfrom.GetId(), 10, "repeated getquorumrotationinfo");
                return;
            }
            last = now;
        }
    ...
    if (found)
    {
        ...
        if (msg_type == NetMsgType::CLSIG) {
            if (m_chainlocks.IsEnabled()) {
                ...
                try {
                    ...
                    Misbehaving(pfrom.GetId(), 10, "clsig-malformed");
                    return;
                }
        ...
        if (msg_type == NetMsgType::SEALSHARE) {
            ...
            // Require seal manager and active SPORK_25 to process.
            // No misbehavior; peer may not have seen the spork yet.
            if (g_seal_manager && g_sporkman && g_sporkman->IsSporkActive(SPORK_25_HMP_ENABLED)) {
                // Deserialize before consuming rate limit; malformed messages
                // should not waste legitimate peer's rate-limit slot
                CSealShare share;
                try {
                    vRecv >> share;
                } catch (const std::exception&) {
                    Misbehaving(pfrom.GetId(), 10, "sealshare-malformed");
                    return;
                }
                // Rate limit: per-NodeId 1/s + per-IP per-type
                {
                    LOCK(m_hmp_rate_mutex);
                    ...
                    m_sealshare_last_request[pfrom.GetId()] = nNow;
                    ipLast = nNow;
                }
                auto result = g_seal_manager->AddSealShare(share);
                if (result == HMPAcceptResult::ACCEPTED) {
                    // Cap relay to prevent amplification
                    if (g_seal_manager->GetShareCount(share.blockHash) <= 20) {
                        const uint256 shareHash = share.GetHash();
                        m_connman.ForEachNode([&](CNode* pnode) {
                            if (pnode->GetId() == pfrom.GetId()) return;
                            if (pnode->IsBlockOnlyConn()) return;
                            PeerRef peer_relay = GetPeerRef(pnode->GetId());
                            if (!peer_relay || !peer_relay->HMPMarkSeen(shareHash)) return;
                            CNetMsgMaker msgMaker(pnode->GetCommonVersion());
        ...
        // Hivemind HMP assembled seal (catch-up)
        if (msg_type == NetMsgType::SEALASM) {
            ...
            if (g_seal_manager && g_sporkman && g_sporkman->IsSporkActive(SPORK_25_HMP_ENABLED)) {
                ...
                try {
                    ...
                    Misbehaving(pfrom.GetId(), 10, "sealasm-malformed");
                    return;
                }
                // Rate limit: per-NodeId 2s + per-IP per-type
                {
                    LOCK(m_hmp_rate_mutex);
                    int64_t nNow = GetTime<std::chrono::microseconds>().count();
                    ...
                    m_sealasm_last_request[pfrom.GetId()] = nNow;
                    ipLast = nNow;
                }
                // DoS protection: cap signer count
                if (seal.signers.size() > 200) return;
                // Validate height and blockHash match the active chain
                {
                    LOCK(cs_main);
                    int tipHeight = m_chainman.ActiveChain().Height();
                    if (height < 0 || height > tipHeight) {
                        Misbehaving(pfrom.GetId(), 20, "sealasm-bad-height");
                        return;
                    }
                    const CBlockIndex* pindex = m_chainman.ActiveChain()[height];
                    if (!pindex || pindex->GetBlockHash() != seal.blockHash) {
                        Misbehaving(pfrom.GetId(), 20, "sealasm-hash-mismatch");
                        return;
                    }
                }
                auto asmResult = g_seal_manager->AddAssembledSeal(seal, height);
                if (asmResult == HMPAcceptResult::REJECTED_INVALID) {
                    Misbehaving(pfrom.GetId(), 10, "sealasm-invalid");
                }
            }
            return;
        }
        ...
        if (msg_type == NetMsgType::PUBKEYCOMMIT) {
            ...
            if (g_hmp_commit_pool) {
                ...
                try {
                    ...
                    Misbehaving(pfrom.GetId(), 10, "pubkeycommit-malformed");
                    return;
                }
                // Rate limit: per-NodeId 5s + per-IP per-type
                {
                    LOCK(m_hmp_rate_mutex);
                    int64_t nNow = GetTime<std::chrono::microseconds>().count();
                    ...
                    m_pubkeycommit_last_request[pfrom.GetId()] = nNow;
                    ipLast = nNow;
                }
                auto commitResult = g_hmp_commit_pool->Add(commit);
                if (commitResult == HMPAcceptResult::ACCEPTED) {
                    const uint256 commitHash = ::SerializeHash(commit);
                    m_connman.ForEachNode([&](CNode* pnode) {
                        if (pnode->GetId() == pfrom.GetId()) return;
                        if (pnode->IsBlockOnlyConn()) return;
                        PeerRef peer_relay = GetPeerRef(pnode->GetId());
                        if (!peer_relay || !peer_relay->HMPMarkSeen(commitHash)) return;
                        CNetMsgMaker msgMaker(pnode->GetCommonVersion());
                        m_connman.PushMessage(pnode, msgMaker.Make(NetMsgType::PUBKEYCOMMIT, commit));
                    });
                } else if (commitResult == HMPAcceptResult::REJECTED_INVALID) {
                    Misbehaving(pfrom.GetId(), 10, "pubkeycommit-invalid");
                }
            }
            return;
        }
...
bool PeerManagerImpl::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    try {
        ...
        // Deserialization error: peer sent malformed wire data. Score them.
        LogPrint(BCLog::NET, "%s(%s, %u bytes): Deserialization exception '%s' (%s) caught\n",
                 __func__, SanitizeString(msg.m_type), msg.m_message_size, e.what(), typeid(e).name());
        Misbehaving(pfrom->GetId(), 20, strprintf("deserialization error in %s", SanitizeString(msg.m_type)));
    } catch (const std::exception& e) {
        // Non-deserialization exception: log but don't score (could be our bug)
...
void PeerManagerImpl::CheckForStaleTipAndEvictPeers()
{
    ...
    // Purge stale rate-limit entries
    {
        LOCK(m_hmp_rate_mutex);
        int64_t nNow = GetTime<std::chrono::microseconds>().count();
        static constexpr int64_t RATE_LIMIT_EXPIRY = 60'000'000; // 60s
        auto purge = [&](auto& m) {
            for (auto it = m.begin(); it != m.end(); ) {
                if (nNow - it->second > RATE_LIMIT_EXPIRY)
                    it = m.erase(it);
                else
                    ++it;
            }
        };
        purge(m_sealshare_last_request);
        purge(m_sealasm_last_request);
        purge(m_pubkeycommit_last_request);
        purge(m_hmp_ip_sealshare);
        purge(m_hmp_ip_sealasm);
        purge(m_hmp_ip_pubkeycommit);
    }
    // Purge stale GETMNLISTDIFF/GETQUORUMROTATIONINFO entries (already under cs_main)
    {
        auto purge_chrono = [&](auto& m) {
            for (auto it = m.begin(); it != m.end(); ) {
        ...
        purge_chrono(m_mnlistdiff_last_request);
        purge_chrono(m_qrinfo_last_request);
    }
}
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        // Diagnostic: log sync gate state once per peer until sync starts
        if (!state.fSyncStarted && !state.m_sync_gate_logged) {
            bool canServe = CanServeBlocks(*peer);
            LogPrintf("SyncGate peer=%d: fSyncStarted=%d canServe=%d fImporting=%d fReindex=%d canRelay=%d preferDL=%d nSyncStarted=%d bestHeader=%s\n",
                      pto->GetId(), state.fSyncStarted, canServe, fImporting.load(), fReindex.load(),
                      pto->CanRelay(), state.fPreferredDownload, nSyncStarted,
                      m_chainman.m_best_header ? m_chainman.m_best_header->GetBlockHash().ToString().substr(0, 16) : "null");
            state.m_sync_gate_logged = true;
        }
        ...
        if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
            ...
            if (fSendTrickle && tx_relay->m_send_mempool) {
                ...
                for (const auto& txinfo : vtxinfo) {
                    ...
                    queueAndMaybePushInv(CInv(MSG_TX, hash));
        ...
        // This code also handles non-TXs (Kerrigan-specific messages)
