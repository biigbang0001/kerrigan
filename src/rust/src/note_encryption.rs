...
// Adapted from Zcash's note_encryption.rs
...
pub(crate) fn try_sapling_note_decryption(
    ...
    let ivk = parse_and_prepare_sapling_ivk(raw_ivk)
        .ok_or("Invalid Sapling ivk passed to try_sapling_note_decryption()")?;
...
pub(crate) fn try_sapling_output_recovery(
    ...
    let cv = Option::from(ValueCommitment::from_bytes_not_small_order(&output.cv))
        .ok_or("Invalid output.cv passed to try_sapling_output_recovery()")?;
...
/// Parses and validates a Sapling incoming viewing key.
...
/// A Sapling output that we successfully decrypted.
