...
static bool rest_getutxos(const CoreContext& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    if (uriParts.size() > 0)
    {
        ...
        for (size_t i = (fCheckMemPool) ? 1 : 0; i < uriParts.size(); i++)
        {
            ...
            if (!ParseInt32(strOutput, &nOutput) || nOutput < 0 || !IsHex(strTxid))
                return RESTERR(req, HTTP_BAD_REQUEST, "Invalid output index: " + uriParts[i]);
    ...
    case RESTResponseFormat::HEX: {
        ...
        if (!strRequestMutable.empty() && !IsHex(strRequestMutable))
            return RESTERR(req, HTTP_BAD_REQUEST, "Parse error: invalid hex input");
