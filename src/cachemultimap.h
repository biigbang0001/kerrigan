...
public:
    ...
    SERIALIZE_METHODS(CacheMultiMap, obj)
    {
        ...
        SER_READ(obj, {
            // Reject corrupt cache: nMaxSize must be sane and list must
            // not exceed it.
            static constexpr size_type MAX_DESERIALIZE_SIZE = 2000000;
            if (obj.nMaxSize > MAX_DESERIALIZE_SIZE) {
                throw std::ios_base::failure("CacheMultiMap nMaxSize exceeds safety limit");
            }
            while (obj.listItems.size() > obj.nMaxSize) {
                obj.listItems.pop_back();
            }
            obj.RebuildIndex();
        });
    }
