...
void ChainlockHandler::TransactionAddedToMempool(const CTransactionRef& tx, int64_t nAcceptTime, uint64_t)
{
    ...
    if (tx->vin.empty()) {
        if (tx->nType == TRANSACTION_SAPLING) {
            return;  // Sapling shielded-only, safe by construction, no IS needed
        }
        return;
    }
...
void ChainlockHandler::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex)
{
    ...
    int64_t curTime = GetTime<std::chrono::seconds>().count();
    {
        ...
        for (const auto& tx : pblock->vtx) {
            ...
            if (tx->vin.empty()) {
                if (tx->nType == TRANSACTION_SAPLING) continue;  // shielded-only, no IS needed
                continue;
            }
...
void ChainlockHandler::Cleanup()
{
...

    {
        ...
        // Evict txFirstSeenTime entries older than 24 hours.
        // The per-tx GetTransaction check below handles normal cleanup, but if
        // transactions linger (e.g. unconfirmed, not evicted from mempool) the
        // map can grow without bound. This caps its lifetime.
        constexpr auto TX_FIRST_SEEN_TIMEOUT{24h};
        auto now = GetTime<std::chrono::seconds>();
        for (auto it = txFirstSeenTime.begin(); it != txFirstSeenTime.end();) {
            if (now - it->second >= TX_FIRST_SEEN_TIMEOUT) {
