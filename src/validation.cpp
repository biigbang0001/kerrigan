...
/** Determine the active HMP stage for a given block height.
 *  Stage boundaries are consensus parameters:
 *    Stage 1 (pure PoW):                height < nHMPStage2Height
 *    Stage 2 (commitments open):        height < nHMPStage3Height
 *    Stage 3 (soft seal):               height < nHMPStage4Height
 *    Stage 4 (full seal + neg. proof):  height >= nHMPStage4Height
 */
static int GetHMPStage(int height, const Consensus::Params& params)
{
    if (height < params.nHMPStage2Height) return 1;
    if (height < params.nHMPStage3Height) return 2;
    if (height < params.nHMPStage4Height) return 3;
    return 4;
}

/** Strip uncommitted and unprivileged signers from a seal signer list.
 *  Used by ConnectBlock, RebuildHMPState, and RollforwardBlock to ensure
 *  that nSealWeight / privilege updates are computed from the same filtered
 *  signer set regardless of code path.
 *
 *  @param signers     [in/out] Seal signer public keys; uncommitted/unprivileged entries removed.
 *  @param algos       [in/out] Per-signer algorithm IDs; entries removed in parallel with signers.
 *  @param registry    Commitment registry (may be nullptr; skips commitment check).
 *  @param privilege   Privilege tracker (may be nullptr; skips privilege check).
 *  @param height      Height of the embedding block (used to compute sealed ancestor height).
 *  @param seal_depth  Consensus trailing depth (nHMPSealTrailingDepth).
 *  @param pindex      Block index for privilege tier lookup (passed through to GetTier).
 *  @param caller      Human-readable caller name for log messages.
 */
static void FilterSealSigners(std::vector<CBLSPublicKey>& signers,
                               std::vector<uint8_t>& algos,
                               const CHMPCommitmentRegistry* registry,
                               ...
                               const char* caller)
{
    // Strip uncommitted signers; maturity checked against sealed ancestor height.
    if (registry && registry->IsEnabled() && !signers.empty()) {
        int sealedHeight = height - seal_depth;
        if (sealedHeight < 0) sealedHeight = 0;
        std::vector<CBLSPublicKey> committed;
        std::vector<uint8_t> committedAlgos;
        committed.reserve(signers.size());
        committedAlgos.reserve(algos.size());
        for (size_t i = 0; i < signers.size(); i++) {
            if (registry->IsCommitted(signers[i], sealedHeight)) {
                committed.push_back(signers[i]);
                if (i < algos.size()) {
                    committedAlgos.push_back(algos[i]);
                }
            } else {
                LogPrint(BCLog::NET, "HMP: %s: ignoring uncommitted seal signer %s at sealed height %d\n",
                         caller, signers[i].ToString().substr(0, 16), sealedHeight);
            }
        }
        signers = std::move(committed);
        algos = std::move(committedAlgos);
    }

    // Strip signers with no privilege on their signed algo.
    // Build a new vector to avoid erase-in-place with unsigned index.
    if (privilege && !signers.empty()) {
        std::vector<CBLSPublicKey> privileged;
        std::vector<uint8_t> privilegedAlgos;
        privileged.reserve(signers.size());
        privilegedAlgos.reserve(algos.size());
        for (size_t i = 0; i < signers.size() && i < algos.size(); i++) {
            HMPPrivilegeTier tier = privilege->GetTier(signers[i], algos[i], pindex);
            if (tier != HMPPrivilegeTier::UNKNOWN) {
                privileged.push_back(signers[i]);
                privilegedAlgos.push_back(algos[i]);
            } else {
                LogPrint(BCLog::NET, "HMP: %s: removing signer %s with no privilege on algo %d at height %d\n",
                         caller, signers[i].ToString().substr(0, 16), algos[i], height);
            }
        }
        signers = std::move(privileged);
        algos = std::move(privilegedAlgos);
    }
}
...
static bool ContextualCheckTransaction(const CTransaction& tx, TxValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // Size limits: Sapling transactions may be larger due to zk-SNARK proofs
    if (fDIP0001Active_context) {
        unsigned int nMaxTxSize = (tx.nType == TRANSACTION_SAPLING)
            ? MAX_SAPLING_TX_EXTRA_PAYLOAD
            : MAX_STANDARD_TX_SIZE;
        if (::GetSerializeSize(tx, PROTOCOL_VERSION) > nMaxTxSize)
    ...
    bool fAddToMempool)
{
    ...
    // Checks whether the transaction is still final, if it spends a coinbase output mature,
    // and if it is a Sapling tx that its anchors are still valid on the current chain.
    ...
    const auto filter_final_and_mature = [this](CTxMemPool::txiter it)
        ...
        // Sapling transactions: evict if any spend anchor is no longer valid.
        // After a reorg, UndoBlock removes tree roots from the anchor index,
        // so spends referencing those roots would produce invalid blocks.
        if (tx.nType == TRANSACTION_SAPLING && m_chain_helper) {
            auto payloadOpt = GetTxPayload<SaplingTxPayload>(tx.vExtraPayload);
            if (payloadOpt) {
                for (const auto& spend : payloadOpt->vSpendDescriptions) {
                    uint256 anchor;
                    std::memcpy(anchor.begin(), spend.anchor.data(), 32);
                    if (!m_chain_helper->sapling_state->IsValidAnchor(anchor)) {
                        LogPrintf("Removing Sapling tx %s from mempool: stale anchor %s after reorg\n",
                                  tx.GetHash().ToString(), anchor.ToString());
                        return true;
                    }
                }
            }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (m_chain_helper.IsInstantSendWaitingForTx(hash)) {
        ...
        m_pool.removeSaplingNullifierConflicts(tx);
        ...
        for (const CTxIn &txin : tx.vin)
        {
            ...
            if (ptxConflicting)
            {
                // Transaction conflicts with mempool and RBF doesn't exist in Kerrigan
    ...
    // Sapling transactions: enforce action-based minimum fee (mempool policy).
    // Proof verification is CPU-expensive, so we gate on fee before doing that work.
    if (tx.nType == TRANSACTION_SAPLING && !bypass_limits) {
        auto payloadOpt = GetTxPayload<SaplingTxPayload>(tx.vExtraPayload);
        if (!payloadOpt) {
            return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-sapling-payload",
                "failed to parse Sapling payload for fee check");
        }
        size_t nSpends = payloadOpt->vSpendDescriptions.size();
        size_t nOutputs = payloadOpt->vOutputDescriptions.size();
        CAmount minSaplingFee = CalculateSaplingFee(nSpends, nOutputs);
        if (ws.m_modified_fees < minSaplingFee) {
            return state.Invalid(TxValidationResult::TX_MEMPOOL_POLICY, "sapling-fee-too-low",
                strprintf("%s < %s", FormatMoney(ws.m_modified_fees), FormatMoney(minSaplingFee)));
        }
    }

    // Sapling double-spend check: reject if any nullifier is already spent on-chain
    // or already in the mempool. Check on-chain first (cheap LevelDB read) BEFORE
    // expensive Groth16 proof verification happens downstream.
    if (tx.nType == TRANSACTION_SAPLING) {
        auto payloadOpt = GetTxPayload<SaplingTxPayload>(tx.vExtraPayload);
        if (payloadOpt && m_chain_helper.sapling_state) {
            for (const auto& spend : payloadOpt->vSpendDescriptions) {
                uint256 nf;
                std::memcpy(nf.begin(), spend.nullifier.data(), 32);
                if (m_chain_helper.sapling_state->HasNullifier(nf)) {
                    return state.Invalid(TxValidationResult::TX_CONFLICT, "sapling-nullifier-already-spent",
                        strprintf("nullifier %s already confirmed on-chain", nf.ToString().substr(0, 16)));
                }
            }
        }
        if (!m_pool.CheckSaplingNullifiers(tx)) {
            return state.Invalid(TxValidationResult::TX_CONFLICT, "sapling-nullifier-already-in-mempool");
        }
    }
...
double ConvertBitsToDouble(unsigned int nBits)
{
    ...
    double dDiff = (double)0x0000ffff / (double)nMantissa;
...
double ConvertBitsToDoubleEquihash(unsigned int nBits)
{
    int nShift = (nBits >> 24) & 0xff;
...
/*
 * Kerrigan block subsidy: fixed 25 COIN per block with Bitcoin-style halvings.
 *
 * Halving interval: nSubsidyHalvingInterval (1,051,200 blocks on mainnet, ~4 years at 120s).
 * Max supply: ~52.5M coins.
 *
 * nPrevBits and fV20Active are unused but kept for API compatibility with callers.
 */
...
{
    // Kerrigan has no superblock/governance subsidy split
    return 0;
}
...
CAmount GetBlockSubsidy(const CBlockIndex* const pindex, const Consensus::Params& consensusParams)
{
    ...
    return GetBlockSubsidyInner(pindex->pprev->nBits, pindex->pprev->nHeight, consensusParams, /*fV20Active=*/false);
}

/**
 * Kerrigan masternode payment: 20% of block value goes to the selected masternode.
 *
 * nHeight and fV20Active are unused but kept for API compatibility.
 */
...
void CChainState::CheckForkWarningConditions()
{
    ...
    if (m_chainman.m_best_invalid && m_chainman.m_best_invalid->nChainSealWork > m_chain.Tip()->nChainSealWork + (GetBlockProof(*m_chain.Tip()) * 6)) {
...
void CChainState::InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    if (!m_chainman.m_best_invalid || pindexNew->nChainSealWork > m_chainman.m_best_invalid->nChainSealWork) {
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool fVerifyOnly)
{
    AssertLockHeld(cs_main);
    ...
    // Verify SaplingDB consistency for blocks at or after Sapling activation.
    // Skip during verify-only mode (VerifyDB level 3): SaplingDB has no
    // rollback transaction like EvoDB, so the best-block check fails on
    // the second block walked because fVerifyOnly prevents state updates.
    if (!fVerifyOnly &&
        sapling::IsSaplingActive(m_params.GetConsensus(), pindex->nHeight) &&
        !m_chain_helper->sapling_state->VerifyBestBlock(pindex->GetBlockHash())) {
        // Tolerate SaplingDB being ahead of coin-state: after a crash where
        // SaplingDB committed but CoinsTip did not, the rollback loop may
        // disconnect blocks that SaplingDB already unwound past. UndoBlock
    ...
    if (!m_chain_helper->special_tx->UndoSpecialTxsInBlock(block, pindex, mnlist_updates_opt, fVerifyOnly)) {
    ...
    // Kerrigan is a new chain with no BIP30 exceptions (Bitcoin's duplicate
    // coinbase heights 91722/91812 cannot exist here).
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        // Undo escrow release execution records
        ...
        for (const auto& txout : tx.vout) {
            if (txout.scriptPubKey.size() == 38 &&
                ...
                std::equal(KRGN_MAGIC.begin(), KRGN_MAGIC.end(), txout.scriptPubKey.begin() + 2))
            {
                uint256 proposalHash;
                memcpy(proposalHash.begin(), txout.scriptPubKey.data() + 6, 32);
                UnmarkEscrowReleaseExecuted(m_evoDb, proposalHash);
                LogPrintf("%s: undoing escrow release execution %s at height %d\n",
                          __func__, proposalHash.ToString(), pindex->nHeight);
                break;
            }
        }
    }
...
public:
    ...
    {
        // Bits 8-11 encode the mining algorithm (BLOCK_VERSION_ALGO), not
        // soft-fork signals.  Skip them to avoid false "unknown rules" warnings
        // on every non-X11 block.
        if (m_bit >= 8 && m_bit <= 11) return false;
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const ChainstateManager& chainman)
{
    ...
    // It always active on Kerrigan chains
    ...
    // Enforce NULLFAIL (BIP146) unconditionally. Kerrigan launches fresh
    // with no legacy blocks, so there are no pre-existing non-NULLFAIL sigs.
    flags |= SCRIPT_VERIFY_NULLFAIL;

    // Kerrigan: fresh chain, enforce all standard flags from genesis
...
static int64_t nTimeKerriganSpecific = 0;
    ...
    bool fIBD)
{
    ...
    for (const auto& txout : tx.vout) {
        if (txout.scriptPubKey.size() == 38 &&
            txout.scriptPubKey[0] == OP_RETURN &&
            txout.scriptPubKey[1] == 0x24 /* push 36 bytes */ &&
            std::equal(KRGN_MAGIC.begin(), KRGN_MAGIC.end(), txout.scriptPubKey.begin() + 2))
        {
            memcpy(proposalHash.begin(), txout.scriptPubKey.data() + 6, 32);
            foundMarker = true;
    ...
    if (!foundMarker) {
        LogPrintf("ERROR: %s: tx %s spends escrow without governance reference\n",
                  __func__, tx.GetHash().ToString());
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
            "bad-escrow-no-proposal", "escrow spend missing OP_RETURN governance proposal reference");
    }

    // 2. Anti-replay: check if this proposal has already been executed
    if (IsEscrowReleaseExecuted(evodb, proposalHash)) {
        LogPrintf("ERROR: %s: tx %s replays executed escrow release %s\n",
                  __func__, tx.GetHash().ToString(), proposalHash.ToString());
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
    ...
    if (!pGovMan) {
        LogPrintf("ERROR: %s: governance manager unavailable for escrow spend validation\n", __func__);
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
            "bad-escrow-no-govman", "governance manager not available");
    }
    auto pGovObj = pGovMan->FindFundedEscrowRelease(proposalHash);
    if (!pGovObj) {
        LogPrintf("ERROR: %s: tx %s references unfunded/unknown escrow release %s\n",
                  __func__, tx.GetHash().ToString(), proposalHash.ToString());
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
            "bad-escrow-not-funded", "escrow release proposal not found or not funded");
    }

    // 5. Parse the proposal to get approved amount and destination
    CEscrowReleaseValidator validator(pGovObj->GetDataAsHexString());
    std::string strAmount;
    std::string strAddress;
    if (!validator.GetPaymentAmountStr(strAmount) || !validator.GetPaymentAddress(strAddress)) {
        LogPrintf("ERROR: %s: failed to parse escrow release proposal %s\n",
                  __func__, proposalHash.ToString());
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
            "bad-escrow-parse", "failed to parse escrow release proposal data");
    }
    // Use ParseFixedPoint for deterministic string-to-CAmount conversion
    // (avoids IEEE 754 double rounding that varies across platforms)
    CAmount approvedAmount = 0;
    if (!ParseFixedPoint(strAmount, 8, &approvedAmount) || !MoneyRange(approvedAmount)) {
        LogPrintf("ERROR: %s: invalid escrow payment amount '%s' in proposal %s\n",
                  __func__, strAmount, proposalHash.ToString());
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
            "bad-escrow-amount", "invalid escrow payment amount");
    }
    CTxDestination approvedDest = DecodeDestination(strAddress);
    if (!IsValidDestination(approvedDest)) {
        LogPrintf("ERROR: %s: invalid payment address '%s' in escrow release proposal %s\n",
                  __func__, strAddress, proposalHash.ToString());
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
            "bad-escrow-address", "escrow release proposal contains invalid payment address");
    }
    CScript approvedScript = GetScriptForDestination(approvedDest);

    // 6. Verify a tx output matches the approved payment
    bool foundPayment = false;
    for (const auto& txout : tx.vout) {
        if (txout.scriptPubKey == approvedScript && txout.nValue == approvedAmount) {
            foundPayment = true;
            break;
        }
    }
    if (!foundPayment) {
        LogPrintf("ERROR: %s: tx %s escrow spend outputs don't match proposal %s (expected %s to %s)\n",
                  __func__, tx.GetHash().ToString(), proposalHash.ToString(),
                  FormatMoney(approvedAmount), strAddress);
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
            "bad-escrow-output-mismatch", "escrow spend outputs don't match governance proposal");
    }

    // 7a. Pre-pass: ensure exactly one output pays the approved address for the approved amount.
    //     Without this, a malicious tx could include N duplicate payment outputs,
    //     draining N * approvedAmount from the escrow.
    {
        CAmount totalToApproved = 0;
        int paymentOutputCount = 0;
        for (const auto& txout : tx.vout) {
            if (txout.scriptPubKey == approvedScript) {
                totalToApproved += txout.nValue;
                paymentOutputCount++;
            }
        }
        if (paymentOutputCount != 1 || totalToApproved != approvedAmount) {
            LogPrintf("ERROR: %s: tx %s has %d outputs to approved address totalling %s (expected exactly 1 output of %s)\n",
                      __func__, tx.GetHash().ToString(), paymentOutputCount,
                      FormatMoney(totalToApproved), FormatMoney(approvedAmount));
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                "bad-escrow-duplicate-payment", "escrow spend must have exactly one payment output to the approved address");
        }
    }

    // 7b. Verify remaining outputs go to escrow (change) or are OP_RETURN (marker)
    for (const auto& txout : tx.vout) {
        if (txout.scriptPubKey == approvedScript && txout.nValue == approvedAmount) continue; // payment (verified unique above)
        if (txout.scriptPubKey == escrowScript) continue; // change back to escrow
        if (txout.scriptPubKey.IsUnspendable()) continue; // OP_RETURN marker
        LogPrintf("ERROR: %s: tx %s has unauthorized output in escrow spend\n",
                  __func__, tx.GetHash().ToString());
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
            "bad-escrow-unauthorized-output", "escrow spend has unauthorized output");
    }

    // 8. Mark as executed (anti-replay)
    MarkEscrowReleaseExecuted(evodb, proposalHash, nBlockHeight);
    LogPrintf("%s: escrow release %s executed at height %d: %s to %s\n",
              __func__, proposalHash.ToString(), nBlockHeight,
              FormatMoney(approvedAmount), strAddress);
    return true;
}
...
bool CChainState::ConnectBlock(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex,
                               CCoinsViewCache& view, bool fJustCheck, bool fVerifyOnly)
{
    ...
    /// KERRIGAN: Check superblock start
    ...
    std::optional<MNListUpdates> mnlist_updates_opt{std::nullopt};
    if (!m_chain_helper->special_tx->ProcessSpecialTxsInBlock(block, pindex, view, fJustCheck, fScriptChecks, state, mnlist_updates_opt, fVerifyOnly)) {
        return error("ConnectBlock(KRGN): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // RAII guard: discard Sapling pending batch if ConnectBlock fails after
    // ProcessSpecialTxsInBlock has built one. Set fSaplingBatchCommitted
    // on the success path to prevent the guard from discarding a valid batch.
    bool fSaplingBatchCommitted = false;
    auto* saplingStatePtr = (m_chain_helper && m_chain_helper->sapling_state)
                            ? m_chain_helper->sapling_state.get() : nullptr;
    struct SaplingCleanupGuard {
        bool& committed;
        sapling::CSaplingState* state;
        ~SaplingCleanupGuard() { if (!committed && state) state->DiscardPendingBatch(); }
    } saplingGuard{fSaplingBatchCommitted, saplingStatePtr};

    // Deferred HMP state mutations: collect mutation data during HMP
    // processing but defer actual global state changes until after ALL consensus
    // checks pass (IS lock conflicts, masternode payments, superblock).
    // This prevents stale global state when a later check fails.
    struct DeferredHMPMutation {
        bool fDoCommitmentRegistry{false};
        int nCommitHeight{0};
        CBLSPublicKey commitMinerIdentity;
        std::vector<CBLSPublicKey> commitExplicit;

        bool fDoCommitPoolCleanup{false};
        CBLSPublicKey poolCleanupMinerIdentity;
        std::vector<CBLSPublicKey> poolCleanupKeys;

        bool fDoPrivilege{false};
        int nPrivHeight{0};
        int nPrivAlgo{0};
        CBLSPublicKey privMinerIdentity;
        std::vector<CBLSPublicKey> privSigners;
        std::vector<uint8_t> privSignerAlgos;
    } deferredHMP;

    // Hivemind HMP: 4-stage height-based activation.
    //
    // Stage 1 (< nHMPStage2Height): Pure PoW, no HMP processing.
    // Stage 2 (< nHMPStage3Height): Commitments Open, pubkey commits accepted/stored.
    // Stage 3 (< nHMPStage4Height): Soft Seal, sealing begins, positive-only weight.
    // Stage 4 (>= nHMPStage4Height): Full HMP, complete weight ladder + negative proofs.
    //
    // NOTE: SPORK_25_HMP_ENABLED is intentionally NOT used for validation.
    // Consensus-critical computation (nSealWeight, nChainSealWork) must be derived
    // deterministically from on-chain data alone. Live spork state varies across nodes
    // and time, causing consensus divergence. SPORK_25 gates block production only:
    // miner.cpp (identity/seal/commitment embedding), seal signing (below),
    // and PUBKEYCOMMIT broadcasting (net_processing.cpp, init.cpp).
    if (DeploymentActiveAt(*pindex, m_params.GetConsensus(), Consensus::DEPLOYMENT_HMP)) {
        // VerifyDB (level 4) reconnect: nSealWeight is already persisted in the block index
        // on disk. Skip recomputation entirely to avoid depending on non-deterministic global
        // state (g_hmp_commitments, g_hmp_privilege) which can differ between nodes after
        // crashes, IBD ordering, or VerifyDB runs.
        if (fVerifyOnly) {
            arith_uint256 blockProof = GetBlockProof(*pindex);
            uint64_t clampedWeight = std::min(pindex->nSealWeight, uint64_t{20000});
            arith_uint256 weightedProof = blockProof * arith_uint256(clampedWeight) / arith_uint256(10000);
            pindex->nChainSealWork = (pindex->pprev ? pindex->pprev->nChainSealWork : arith_uint256(0)) + weightedProof;
            LogPrint(BCLog::NET, "HMP: VerifyOnly at height %d, using persisted nSealWeight=%u\n",
                     pindex->nHeight, pindex->nSealWeight);
        } else {
        // Detect HMP activation (height-based) and clear stale privilege data.
        if (!fJustCheck && pindex->pprev &&
            !DeploymentActiveAt(*pindex->pprev, m_params.GetConsensus(), Consensus::DEPLOYMENT_HMP)) {
            if (g_hmp_privilege) {
                LogPrintf("HMP: activated at height %d, clearing stale privilege data\n", pindex->nHeight);
                g_hmp_privilege->Clear();
            }
        }
        ...
        if (nHMPStage == 1) {
            ...
            arith_uint256 blockProof = GetBlockProof(*pindex);
            pindex->nChainSealWork = (pindex->pprev ? pindex->pprev->nChainSealWork : arith_uint256(0)) + blockProof;
            LogPrint(BCLog::NET, "HMP: Stage 1 (pure PoW) at height %d\n", pindex->nHeight);
        } else if (!block.vtx.empty() && block.vtx[0]->nVersion == 3 && block.vtx[0]->nType == TRANSACTION_COINBASE) {
            auto cbTxOpt = GetTxPayload<CCbTx>(*block.vtx[0]);
            if (cbTxOpt && cbTxOpt->nVersion >= CCbTx::Version::HMP_SEAL) {
                const auto& cbTx = *cbTxOpt;

                // Stage 3+: reject blocks without valid minerIdentity.
                // Stage 2 allows empty identity (miners still registering).
                if (nHMPStage >= 3 && !cbTx.minerIdentity.IsValid()) {
                    return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cbtx-no-miner-identity");
                }

                // Store miner identity in block index
                if (cbTx.minerIdentity.IsValid()) {
                    pindex->hmpMinerPubKey = cbTx.minerIdentity;
                }

                // Bounds-check vCommitments from wire data
                if (cbTx.vCommitments.size() > static_cast<size_t>(Consensus::Params::MAX_COMMITMENTS_PER_BLOCK)) {
                    return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cbtx-too-many-commitments");
                }

                // Stage 2+: Accept and store pubkey commitments
                if (nHMPStage >= 2) {
                    // Guard all global HMP state mutations: fJustCheck means we are in
                    // TestBlockValidity (called by getblocktemplate); it should not
                    // mutate global state. fVerifyOnly is already gated at the top
                    // of this HMP block.
                    if (!fJustCheck) {
                        // Defer commitment registry + commit pool + privilege mutations
                        // until after all consensus checks pass.
                        if (g_hmp_commitments) {
                            deferredHMP.fDoCommitmentRegistry = true;
                            deferredHMP.nCommitHeight = pindex->nHeight;
                            deferredHMP.commitMinerIdentity = cbTx.minerIdentity;
                            deferredHMP.commitExplicit = cbTx.vCommitments;
                        }

                        if (g_hmp_commit_pool) {
                            deferredHMP.fDoCommitPoolCleanup = true;
                            deferredHMP.poolCleanupMinerIdentity = cbTx.minerIdentity;
                            deferredHMP.poolCleanupKeys = cbTx.vCommitments;
                        }

                        // Only call for Stage 2 (not Stage 3+) to avoid double-counting.
                        // Stage 3+ defers BlockConnected below with actual seal signers.
                        if (g_hmp_privilege && nHMPStage < 3) {
                            ...
                            // noSigners: leave privSigners/privSignerAlgos empty
                        }
                    }
                }

                // Stage 2 (commitments only): neutral seal weight, no sealing yet
                if (nHMPStage == 2) {
                    pindex->nSealWeight = 10000;
                    arith_uint256 blockProof = GetBlockProof(*pindex);
                    pindex->nChainSealWork = (pindex->pprev ? pindex->pprev->nChainSealWork : arith_uint256(0)) + blockProof;
                    LogPrint(BCLog::NET, "HMP: Stage 2 (commitments open) at height %d\n", pindex->nHeight);
                }

                // Stage 3+: Seal processing
                if (nHMPStage >= 3) {
                    // Extract signer list from assembled seal (via seal manager or P2P)
                    std::vector<CBLSPublicKey> sealSignerPubKeys;
                    std::vector<uint8_t> sealSignerAlgos;

                    if (cbTx.sealForAncestor.IsValid() && !cbTx.sealBlockHash.IsNull()) {
                        // Validate sealBlockHash is the correct ancestor at (height - nHMPSealTrailingDepth)
                        int targetHeight = pindex->nHeight - hmpConsensus.nHMPSealTrailingDepth;
                        if (targetHeight >= 0) {
                            const CBlockIndex* pAncestor = pindex->GetAncestor(targetHeight);
                            if (!pAncestor || pAncestor->GetBlockHash() != cbTx.sealBlockHash) {
                                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-seal-wrong-ancestor",
                                    strprintf("sealBlockHash %s does not match ancestor at height %d",
                                              cbTx.sealBlockHash.ToString().substr(0, 16), targetHeight));
                            }
                        }

                        // Use the signer list committed on-chain (vSealSigners in CCbTx v4) so that
                        // nSealWeight is deterministic from block data alone, regardless of whether the
                        // assembled seal is in the in-memory seal manager cache.
                        // Bounds already enforced by CCbTx deserialization (LIMITED_VECTOR 200).
                        sealSignerPubKeys = cbTx.vSealSigners;
                        sealSignerAlgos = cbTx.vSealSignerAlgos;

                        // Signer/algo vector size must match
                        if (sealSignerPubKeys.size() != sealSignerAlgos.size()) {
                            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS,
                                "bad-seal-signers-algos-mismatch");
                        }

                        // Verify the BLS aggregate signature against the on-chain signer list.
                        // Construct a temporary CAssembledSeal for verification.
                        if (!sealSignerPubKeys.empty()) {
                            CAssembledSeal tempSeal;
                            tempSeal.blockHash = cbTx.sealBlockHash;
                            tempSeal.aggregatedSig = cbTx.sealForAncestor;
                            tempSeal.signers = sealSignerPubKeys;
                            tempSeal.signerAlgos = sealSignerAlgos;
                            if (!tempSeal.Verify()) {
                                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-seal-signature");
                            }

                            // VRF re-verification against in-memory seal shares when available.
                            // VRF proofs are not stored on-chain; only applies to live validation.
                            if (g_seal_manager && g_hmp_privilege) {
                                auto cachedSeal = g_seal_manager->GetSeal(cbTx.sealBlockHash);
                                if (!cachedSeal) {
                                    // No cached seal, can't re-verify VRF (IBD/catch-up). Warn only.
                                    LogPrint(BCLog::NET, "HMP: no cached seal for VRF re-verify at height %d\n",
                                             pindex->nHeight);
                                }
                            }
                        }
                    ...
                    FilterSealSigners(sealSignerPubKeys, sealSignerAlgos,
                                      g_hmp_commitments.get(), g_hmp_privilege.get(),
                    ...
                    if (!fVerifyOnly) {
                        ...
                        if (!sealSignerPubKeys.empty() && sealSignerPubKeys.size() < 2) {
                        ...
                        // In Stage 3 (soft seal), only positive seal weight is used; no
                        // negative proofs, allowing the network to bootstrap seal participation
                        // without penalizing miners during the transition.
                        if (nHMPStage >= 4) {
                            ...
                            pindex->nSealWeight = sw * penalty / 10000;
                        }
                    }

                    // Remove from candidates before mutating sort key to avoid std::set UB.
                    // Skip during fJustCheck (TestBlockValidity) -- pindex is a stack-local
                    // indexDummy that must not be inserted into the global set.
                    if (!fJustCheck) {
                        setBlockIndexCandidates.erase(pindex);
                    }

                    // Compute cumulative seal-weighted work
                    arith_uint256 blockProof = GetBlockProof(*pindex);
                    uint64_t clampedWeight = std::min(pindex->nSealWeight, uint64_t{20000});
                    arith_uint256 weightedProof = blockProof * arith_uint256(clampedWeight) / arith_uint256(10000);
                    pindex->nChainSealWork = (pindex->pprev ? pindex->pprev->nChainSealWork : arith_uint256(0)) + weightedProof;
                    ...
                    // Start seal collection for this block and sign if eligible.
                    if (!fJustCheck && g_seal_manager) {
                        ...
                        // Always track new blocks for seal collection so the
                        // seal manager's height-to-hash index stays consistent.
                        g_seal_manager->OnNewBlock(blockHash, pindex->nHeight, prevSealHash);
                        ...
                        if (g_sporkman && g_sporkman->IsSporkActive(SPORK_25_HMP_ENABLED)) {
                            ...
                            if (share) {
                                ...
                                if (result == HMPAcceptResult::ACCEPTED) {
                                    g_seal_manager->BroadcastShare(*share);
                                }
                            }
                        }
                    }

                    LogPrint(BCLog::NET, "HMP: Stage %d at height %d (seal weight=%u)\n",
                             nHMPStage, pindex->nHeight, pindex->nSealWeight);
                }
            }
        } else {
            // HMP deployed + spork active, but coinbase is not HMP format.
            // Still update privilege tracker and chain seal work so the
            // Elder set remains accurate across old-format blocks.
            if (!fJustCheck && g_hmp_privilege) {
                deferredHMP.fDoPrivilege = true;
                deferredHMP.nPrivHeight = pindex->nHeight;
                deferredHMP.nPrivAlgo = pindex->GetAlgo();
                // empty key + no signers, leave privMinerIdentity/privSigners default
            }
            pindex->nSealWeight = 10000; // neutral, no seal data
            arith_uint256 blockProof = GetBlockProof(*pindex);
            pindex->nChainSealWork = (pindex->pprev ? pindex->pprev->nChainSealWork : arith_uint256(0)) + blockProof;
        }
        } // else (!fVerifyOnly)
    ...
    // Check transactions for InstantSend conflicts

    if (!fJustCheck && m_chain_helper->ShouldInstantSendRejectConflicts()) {
        // Require other nodes to comply, send them some data in case they are missing it.
        const bool has_chainlock = m_chain_helper->HasChainLock(pindex->nHeight, pindex->GetBlockHash());
        for (const auto& tx : block.vtx) {
            // skip txes that have no lockable inputs (transparent or Sapling nullifiers)
            if (!HasLockableInputs(*tx)) continue;
            while (auto conflictLockOpt = m_chain_helper->ConflictingISLockIfAny(*tx)) {
                ...
                if (has_chainlock) {
                    LogPrint(BCLog::ALL, "ConnectBlock(KRGN): chain-locked transaction %s overrides islock %s\n", tx->GetHash().ToString(), conflict_islock_hash.ToString());
                    m_chain_helper->RemoveConflictingISLockByTx(*tx);
                } else {
                    // The node which relayed this should switch to correct chain.
                    // TODO: relay instantsend data/proof.
                    LogPrintf("ERROR: ConnectBlock(KRGN): transaction %s conflicts with transaction lock %s\n", tx->GetHash().ToString(), conflict_txid.ToString());
    ...
    // Check masternode payments and superblocks
    ...
    if (!m_chain_helper->mn_payments->IsBlockValueValid(block, pindex->nHeight, blockSubsidy + feeReward, strError, check_superblock)) {
        ...
        LogPrintf("ERROR: ConnectBlock(KRGN): %s\n", strError);
    ...
    if (!m_chain_helper->mn_payments->IsBlockPayeeValid(*block.vtx[0], pindex->pprev, blockSubsidy, feeReward, check_superblock)) {
        // NOTE: Do not punish, the node might be missing governance data
        LogPrintf("ERROR: ConnectBlock(KRGN): couldn't find masternode or superblock payments\n");
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeKerriganSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Kerrigan specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeKerriganSpecific * MICRO, nTimeKerriganSpecific * MILLI / nBlocksTotal);

    // END KERRIGAN

    // Apply deferred HMP state mutations: all consensus checks have passed.
    // These were collected during HMP processing above but deferred so that
    // IS lock conflicts, masternode payments, and superblock checks can reject
    // the block without leaving stale global state.
    if (!fJustCheck) {
        if (deferredHMP.fDoCommitmentRegistry && g_hmp_commitments) {
            g_hmp_commitments->BlockConnected(deferredHMP.nCommitHeight,
                                               deferredHMP.commitMinerIdentity,
                                               deferredHMP.commitExplicit);
        }
        if (deferredHMP.fDoCommitPoolCleanup && g_hmp_commit_pool) {
            if (deferredHMP.poolCleanupMinerIdentity.IsValid()) {
                g_hmp_commit_pool->Remove(deferredHMP.poolCleanupMinerIdentity);
            }
            for (const auto& pk : deferredHMP.poolCleanupKeys) {
                g_hmp_commit_pool->Remove(pk);
            }
        }
        if (deferredHMP.fDoPrivilege && g_hmp_privilege) {
            g_hmp_privilege->BlockConnected(deferredHMP.nPrivHeight, deferredHMP.nPrivAlgo,
                                            deferredHMP.privMinerIdentity,
    ...
    // Commit deferred Sapling state (nullifiers, tree roots, best block).
    // This must happen after all validation checks pass to avoid persisting
    // state for blocks that fail masternode/superblock checks.
    // Skip during VerifyDB; no pending batch was built (Sapling processing
    // was skipped via fVerifyOnly), but guard against the VerifyDB code path.
    if (!fVerifyOnly) {
        if (!m_chain_helper->sapling_state->CommitBlock()) {
            return AbortNode(state, "Failed to commit Sapling state");
        }
        fSaplingBatchCommitted = true; // prevent RAII guard from discarding
    }
...
bool CChainState::DisconnectTip(BlockValidationState& state, DisconnectedBlockTransactions* disconnectpool)
{
    ...
    int64_t nStart = GetTimeMicros();
    {
        ...
        // HMP disconnect inside EvoDB transaction scope for atomicity.
        // HMP state is in-memory and rebuilt on startup, but keeping it
        // consistent with EvoDB avoids a brief inconsistency window on crash.
        if (g_hmp_commitments) {
            g_hmp_commitments->BlockDisconnected(pindexDelete->nHeight);
        }
        if (g_hmp_privilege) {
            g_hmp_privilege->BlockDisconnected(pindexDelete->nHeight);
        }
        if (g_seal_manager) {
    ...
    // Restore consumed commitments to pending pool after reorg
    if (g_hmp_commit_pool && block.vtx[0]) {
        auto cbTxOpt = GetTxPayload<CCbTx>(*block.vtx[0]);
        if (cbTxOpt) {
            for (const auto& pk : cbTxOpt->vCommitments) {
                CPubKeyCommit fakeCommit;
                fakeCommit.pubKey = pk;
                fakeCommit.nTimestamp = GetTime<std::chrono::seconds>().count();
                g_hmp_commit_pool->AddFromReorg(fakeCommit);
            }
            if (cbTxOpt->minerIdentity.IsValid()) {
                CPubKeyCommit minerCommit;
                minerCommit.pubKey = cbTxOpt->minerIdentity;
                minerCommit.nTimestamp = GetTime<std::chrono::seconds>().count();
                g_hmp_commit_pool->AddFromReorg(minerCommit);
            }
        }
    }
...
bool CChainState::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    if (!pblock) {
        ...
        if (!ReadBlockFromDisk(*pblockNew, pindexNew, m_params.GetConsensus())) {
            return state.Error("Failed to read block from disk (possible corruption)");
        }
    ...
    do {
        ...
        while (pindexTest && !m_chain.Contains(pindexTest)) {
            ...
            if (fFailedChain || fMissingData || fConflictingChain) {
                ...
                if (fFailedChain && (m_chainman.m_best_invalid == nullptr || pindexNew->nChainSealWork > m_chainman.m_best_invalid->nChainSealWork)) {
                    m_chainman.m_best_invalid = pindexNew;
                }
...
bool CChainState::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    // Deep-reorg protection is handled by ChainLocks (SPORK_19) once deployed.
    // A hard depth cap here would cause consensus divergence under partition/delay
    // scenarios since different nodes can disagree on fork-choice.
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        for (CBlockIndex* pindexConnect : vpindexToConnect | std::views::reverse) {
            if (!ConnectTip(state, pindexConnect, pindexConnect == pindexMostWork ? pblock : std::shared_ptr<const CBlock>(), connectTrace, disconnectpool)) {
                ...
                if (!pindexOldTip || m_chain.Tip()->nChainSealWork > pindexOldTip->nChainSealWork) {
...
bool CChainState::PreciousBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    {
        LOCK(cs_main);
        if (pindex->nChainSealWork < m_chain.Tip()->nChainSealWork) {
            // Nothing to do, this block is not at the tip.
            return true;
        }
        if (m_chain.Tip()->nChainSealWork > nLastPreciousChainwork) {
            // The chain has been extended since the last call, reset the counter.
            nBlockReverseSequenceId = -1;
        }
        nLastPreciousChainwork = m_chain.Tip()->nChainSealWork;
        setBlockIndexCandidates.erase(pindex);
...
bool CChainState::InvalidateBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    // Use nChainSealWork (HMP-weighted) for candidate lookup
...

    {
        ...
        for (auto& entry : m_blockman.m_block_index) {
            ...
            if (!m_chain.Contains(candidate) &&
                ...
                candidate_blocks_by_work.insert(std::make_pair(candidate->nChainSealWork, candidate));
            }
        }
    }
    ...
    while (true) {
        ...
        auto candidate_it = candidate_blocks_by_work.lower_bound(invalid_walk_tip->pprev->nChainSealWork);
    ...
    while (pindex_walk && !m_chain.Contains(pindex_walk)) {
        ...
        for (auto jt = itp.first; jt != itp.second; ++jt) {
            ...
            if (!MarkConflictingBlock(state, jt->second)) {
                ...
                // Unconditional halt, not safe to continue with conflicting blocks unmarked
                throw std::runtime_error(strprintf("CChainState::%s -- MarkConflictingBlock failed: %s",
                                                    __func__, state.ToString()));
            }
...
static bool CheckKawPowSolution(const CBlockHeader& block, BlockValidationState& state)
{
    ...
    if (epoch_number < 0 || static_cast<uint32_t>(epoch_number) > MAX_REASONABLE_EPOCH) {
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "kawpow-epoch-too-large",
                             strprintf("KawPoW epoch %d exceeds max %u (nHeight=%u)", epoch_number, MAX_REASONABLE_EPOCH, block.nHeight));
    }
...
static bool CheckBlockHeader(const CBlockHeader& block, const uint256& hash, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    if (fCheckPOW) {
        ...
        // Reject blocks with unknown mining algorithm before any expensive hashing
        if (algo < 0 || algo >= NUM_ALGOS) {
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-algo",
                                 "block uses unknown mining algorithm");
        }

        // Algo-specific solution checks
        if (IsEquihash(algo)) {
            // hashReserved must be zero (reserved for future Sapling commitment root)
            if (!block.hashReserved.IsNull())
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-equihash-hashreserved",
                    "hashReserved must be zero");
            // Quick bounds check before expensive BLAKE2b hashing.
            // Equihash(200,9) solution = 1344 bytes, Equihash(192,7) = 400 bytes.
            const size_t expected = (algo == ALGO_EQUIHASH_200) ? 1344 : 400;
            if (block.nSolution.size() != expected)
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-equihash-size",
                    strprintf("Equihash solution size %u != expected %u", block.nSolution.size(), expected));
            if (!block.CheckEquihashSolution())
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-equihash", "Equihash solution verification failed");
            ...
            if (epoch_number < 0 || static_cast<uint32_t>(epoch_number) > MAX_REASONABLE_EPOCH) {
                return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "kawpow-epoch-too-large",
                    strprintf("KawPoW epoch %d exceeds max %u", epoch_number, MAX_REASONABLE_EPOCH));
            }
        }
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    for (const auto& tx : block.vtx) {
        // Early valueBalance range check
        // NOTE: valueBalance can be legitimately negative for t->z shielding.
        if (tx->nType == TRANSACTION_SAPLING && !tx->vExtraPayload.empty()) {
            auto payload = GetTxPayload<SaplingTxPayload>(*tx);
            if (payload && (payload->valueBalance < -MAX_MONEY || payload->valueBalance > MAX_MONEY)) {
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-sapling-value-balance-range",
                    strprintf("Sapling tx %s has out-of-range valueBalance", tx->GetHash().ToString()));
            }
        }
        TxValidationState tx_state;
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev, int64_t nAdjustedTime) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (pindexPrev == nullptr) {
        return state.Invalid(BlockValidationResult::BLOCK_MISSING_PREV, "bad-prevblk-null",
                             "pindexPrev is null in ContextualCheckBlockHeader");
    }
    const int nHeight = pindexPrev->nHeight + 1;

    // Contextual KawPoW height validation: the header's nHeight field must
    // match the actual chain height.  The context-free check in CheckBlockHeader
    // caps the epoch, but only a contextual check can reject plausible-but-wrong
    // heights (e.g. nHeight=100000 at actual height 5).
    if (block.GetAlgo() == ALGO_KAWPOW) {
        if (block.nHeight != static_cast<uint32_t>(nHeight)) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "kawpow-bad-height",
                                 strprintf("KawPoW nHeight=%u != expected %d", block.nHeight, nHeight));
        }
        // Full KawPoW solution verify with confirmed nHeight
        if (!CheckKawPowSolution(block, state))
            return false;
    }

    // Reject unknown algo encodings in version bits 8-11
    {
        uint32_t algoField = block.nVersion & BLOCK_VERSION_ALGO;
        if (algoField != BLOCK_VERSION_X11 && algoField != BLOCK_VERSION_KAWPOW &&
            algoField != BLOCK_VERSION_EQUIHASH_200 && algoField != BLOCK_VERSION_EQUIHASH_192) {
            return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-algo",
                                 strprintf("unknown algo encoding 0x%04x", algoField));
        }
    }

    // Check proof of work (strict for all blocks, no legacy DGW tolerance)
    //
    // Transition tolerance (#1024): between nDiffFloorHeight and
    ...
    // Strip algo bits (8-11) so version checks work correctly for all algos.
    int32_t baseVersion = block.nVersion & ~BLOCK_VERSION_ALGO;
    if ((baseVersion < 2 && DeploymentActiveAfter(pindexPrev, chainman.GetConsensus(), Consensus::DEPLOYMENT_HEIGHTINCB)) ||
        (baseVersion < 3 && DeploymentActiveAfter(pindexPrev, chainman.GetConsensus(), Consensus::DEPLOYMENT_DERSIG)) ||
        (baseVersion < 4 && DeploymentActiveAfter(pindexPrev, chainman.GetConsensus(), Consensus::DEPLOYMENT_CLTV))) {
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const ChainstateManager& chainman, const CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // Guard: pindexPrev is null for the genesis block during reindex.
    // CSVHeight=0 makes DeploymentActiveAfter(nullptr) return true, but
    // there's no median time past without a parent.
    bool enforce_locktime_median_time_past{false};
    if (pindexPrev != nullptr && DeploymentActiveAfter(pindexPrev, chainman.GetConsensus(), Consensus::DEPLOYMENT_CSV)) {
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    bool fHasMoreOrSameWork = (m_chain.Tip() ? pindex->nChainSealWork >= m_chain.Tip()->nChainSealWork : true);
                       ...
                       bool fCheckMerkleRoot)
{
    ...
    // RAII guard: always restores bls_legacy_scheme on exit.
    // TestBlockValidity must never permanently change the scheme --
    // ConnectBlock (via ProcessSpecialTxsInBlock) may toggle it at the
    // V19 activation boundary, but we're only *testing* the block.
    ...
    // schemeGuard destructor restores original value -- never commit() here
    // because TestBlockValidity must not permanently change the scheme.
    return true;
}
...
void CChainState::RebuildHMPState()
{
    ...
    int maxLookback = std::max({
        consensus.nHMPDominanceCatchMaxLookback + consensus.nHMPPrivilegeWindow,
        consensus.nHMPPrivilegeWindow + consensus.nHMPWarmupBlocks,
    ...
    if (consensus.HMPHeight > startHeight) {
        startHeight = consensus.HMPHeight;
    }

    // Clear any stale data from the fresh-constructed trackers
    ...
    for (int height = startHeight; height <= tip->nHeight; height++) {
        ...
        // Stage 1: pure PoW, no HMP processing
        if (nHMPStage == 1) continue;
        ...
        bool fFoundAnyCoinbase = false;
        for (const auto& tx : block.vtx) {
            if (tx->nVersion == 3 && tx->nType == TRANSACTION_COINBASE) {
                fFoundAnyCoinbase = true;
                auto cbTxOpt = GetTxPayload<CCbTx>(*tx);
                if (cbTxOpt && cbTxOpt->nVersion >= CCbTx::Version::HMP_SEAL) {
                    // Update commitment registry
                    if (g_hmp_commitments && nHMPStage >= 2) {
                        g_hmp_commitments->BlockConnected(height,
                            cbTxOpt->minerIdentity, cbTxOpt->vCommitments);
                    }

                    // Filter signers: strip uncommitted + unprivileged.
                    std::vector<CBLSPublicKey> filteredSigners;
                    std::vector<uint8_t> filteredAlgos;
                    if (cbTxOpt->sealForAncestor.IsValid() && !cbTxOpt->sealBlockHash.IsNull()) {
                        filteredSigners = cbTxOpt->vSealSigners;
                        filteredAlgos = cbTxOpt->vSealSignerAlgos;

                        FilterSealSigners(filteredSigners, filteredAlgos,
                                          g_hmp_commitments.get(), g_hmp_privilege.get(),
                                          height, consensus.nHMPSealTrailingDepth,
                    ...
                    // Rebuild seal-manager sessions (not signing/broadcasting)
                    if (nHMPStage >= 3 && g_seal_manager) {
                        uint256 prevHash = pindex->pprev ? pindex->pprev->GetBlockHash() : uint256();
                        g_seal_manager->OnNewBlock(pindex->GetBlockHash(), height, prevHash);
                    }
                }
                break;
            }
        ...
        // Non-HMP coinbase fallback
        if (!fFoundAnyCoinbase && g_hmp_privilege) {
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (nCheckLevel >= 3) {
            if (curr_coins_usage <= chainstate.m_coinstip_cache_size_bytes) {
                ...
                // Pass fVerifyOnly=true to prevent Sapling LevelDB writes.
                // VerifyDB operates on an in-memory UTXO cache that is never
                // flushed, but Sapling::UndoBlock writes directly to its own
                // LevelDB with fSync=true.  A crash between level-3 disconnects
                // and level-4 reconnects would leave Sapling DB permanently
                // corrupted while UTXO state remains at the chain tip.
                DisconnectResult res = chainstate.DisconnectBlock(block, pindex, coins, /*fVerifyOnly=*/true);
    ...
    if (nCheckLevel >= 4 && !skipped_l3_checks) {
        while (pindex != chainstate.m_chain.Tip()) {
            ...
            if (!ReadBlockFromDisk(block, pindex, consensus_params))
                return error("VerifyDB(): *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
            // Pass fVerifyOnly=true to prevent Sapling LevelDB writes
            // during reconnection (consistent with the level-3 disconnect path).
            if (!chainstate.ConnectBlock(block, state, pindex, coins, /*fJustCheck=*/false, /*fVerifyOnly=*/true))
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    BlockValidationState state;
    std::optional<MNListUpdates> mnlist_updates_opt{std::nullopt};
    // Enable BLS signature checks (fCheckCbTxMerkleRoots=true) during rollforward
    // to detect on-disk corruption of LLMQ commitment signatures during crash recovery.
    if (!m_chain_helper->special_tx->ProcessSpecialTxsInBlock(block, pindex, inputs, false /*fJustCheck*/, true /*fCheckCbTxMerkleRoots*/, state, mnlist_updates_opt)) {
        return error("RollforwardBlock(KRGN): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), state.ToString());
    }
    ...
    if (fAddressIndex) {
        if (!m_blockman.m_block_tree_db->WriteAddressIndex(addressIndex)) {
            return error("RollforwardBlock(KRGN): Failed to write address index");
        }

        if (!m_blockman.m_block_tree_db->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            return error("RollforwardBlock(KRGN): Failed to write address unspent index");
        }
    }

    if (fSpentIndex) {
        if (!m_blockman.m_block_tree_db->UpdateSpentIndex(spentIndex))
            return error("RollforwardBlock(KRGN): Failed to write transaction index");
    }

    if (fTimestampIndex) {
        if (!m_blockman.m_block_tree_db->WriteTimestampIndex(CTimestampIndexKey(pindex->nTime, pindex->GetBlockHash())))
            return error("RollforwardBlock(KRGN): Failed to write timestamp index");
    }

    // Commit deferred Sapling state built by ProcessSpecialTxsInBlock.
    if (!m_chain_helper->sapling_state->CommitBlock()) {
        return error("RollforwardBlock(KRGN): Failed to commit Sapling state at %d", pindex->nHeight);
    }

    // Rebuild HMP privilege + commitment state from coinbase during crash recovery.
    // Apply same commitment stripping + privilege filtering as ConnectBlock.
    // Note: nSealWeight is persisted in the block index and nChainSealWork is reconstructed
    // from it in LoadBlockIndex, no need to recompute either here.
    if (DeploymentActiveAt(*pindex, m_params.GetConsensus(), Consensus::DEPLOYMENT_HMP)) {
        // (c) Detect HMP activation boundary and clear stale privilege data,
        // matching ConnectBlock.
        if (pindex->pprev &&
            !DeploymentActiveAt(*pindex->pprev, m_params.GetConsensus(), Consensus::DEPLOYMENT_HMP)) {
            if (g_hmp_privilege) {
                LogPrintf("HMP: activated at height %d (rollforward), clearing stale privilege data\n", pindex->nHeight);
                g_hmp_privilege->Clear();
            }
        }

        // (d) Determine HMP stage for privilege update gating.
        const auto& hmpConsensus = m_params.GetConsensus();
        int nHMPStage = GetHMPStage(pindex->nHeight, hmpConsensus);

        // Stage 1: pure PoW, skip all HMP processing
        if (nHMPStage == 1) {
            // nSealWeight and nChainSealWork already loaded from disk
        } else {
        ...
        for (const auto& tx : block.vtx) {
            if (tx->nVersion == 3 && tx->nType == TRANSACTION_COINBASE) {
                ...
                if (cbTxOpt && cbTxOpt->nVersion >= CCbTx::Version::HMP_SEAL) {
                    ...
                    // Enforce minerIdentity at Stage 3+.
                    // If a block on disk fails this, it indicates corruption; halt.
                    if (nHMPStage >= 3 && !cbTxOpt->minerIdentity.IsValid()) {
                        return error("RollforwardBlock: consensus-invalid block at h=%d, missing minerIdentity (corruption?)",
                                     pindex->nHeight);
                    }

                    // Enforce vCommitments bounds
                    if (cbTxOpt->vCommitments.size() > Consensus::Params::MAX_COMMITMENTS_PER_BLOCK) {
                        return error("RollforwardBlock: consensus-invalid block at h=%d, too many commitments",
                                     pindex->nHeight);
                    }

                    // (a) Update commitment registry for v4+ CbTx.
                    if (g_hmp_commitments && nHMPStage >= 2) {
                        g_hmp_commitments->BlockConnected(pindex->nHeight,
                            cbTxOpt->minerIdentity, cbTxOpt->vCommitments);
                    }
                    ...
                    std::vector<uint8_t> filteredAlgos;
                    if (cbTxOpt->sealForAncestor.IsValid() && !cbTxOpt->sealBlockHash.IsNull()) {
                        // Validate sealBlockHash matches trailing ancestor (hard error on corruption)
                        int targetHeight = pindex->nHeight - hmpConsensus.nHMPSealTrailingDepth;
                        if (targetHeight >= 0) {
                            const CBlockIndex* pAncestor = pindex->GetAncestor(targetHeight);
                            if (!pAncestor || pAncestor->GetBlockHash() != cbTxOpt->sealBlockHash) {
                                return error("RollforwardBlock: sealBlockHash mismatch at h=%d (corruption?)",
                                             pindex->nHeight);
                            }
                        }
                        // Enforce signer/algo vector parity (hard error on corruption)
                        if (cbTxOpt->vSealSigners.size() != cbTxOpt->vSealSignerAlgos.size()) {
                            return error("RollforwardBlock: signer/algo size mismatch at h=%d (corruption?)",
                                         pindex->nHeight);
                        }
                        filteredSigners = cbTxOpt->vSealSigners;
                        filteredAlgos = cbTxOpt->vSealSignerAlgos;

                        // Verify BLS aggregate seal signature.
                        // Detects on-disk corruption during crash recovery.
                        if (!filteredSigners.empty()) {
                            CAssembledSeal tempSeal;
                            tempSeal.blockHash = cbTxOpt->sealBlockHash;
                            tempSeal.aggregatedSig = cbTxOpt->sealForAncestor;
                            tempSeal.signers = filteredSigners;
                            tempSeal.signerAlgos = filteredAlgos;
                            if (!tempSeal.Verify()) {
                                return error("RollforwardBlock: bad seal signature at h=%d (corruption?)",
                                             pindex->nHeight);
                            }
                        }
                    }
        ...
        // (b) Non-HMP coinbase fallback: HMP deployed but coinbase is not
        // v3/TRANSACTION_COINBASE at all. Still update privilege tracker with
        // empty signers so Elder set remains accurate across old-format blocks.
        // Same as the ConnectBlock fallback for non-HMP coinbases.
        // Note: if a v3/TRANSACTION_COINBASE exists but CbTx < HMP_SEAL,
        // ConnectBlock skips privilege update; match that by gating on
        // !fFoundAnyCoinbase rather than !fFoundHMPCoinbase.
        if (!fFoundAnyCoinbase && g_hmp_privilege) {
            CBLSPublicKey emptyKey;
            std::vector<CBLSPublicKey> noSigners;
            g_hmp_privilege->BlockConnected(pindex->nHeight, pindex->GetAlgo(),
...
bool CChainState::ReplayBlocks()
{
    ...
    if (!hashHeads[1].IsNull()) { // The old tip is allowed to be 0, indicating it's the first flush.
        ...
        if (fDIP0003Active && !m_evoDb.VerifyBestBlock(pindexOld->GetBlockHash())) {
            return error("ReplayBlocks(KRGN): Found EvoDB inconsistency");
        }
        // Verify SaplingDB consistency for blocks at or after Sapling activation.
        // Tolerate Sapling-ahead state: if a prior crash occurred after Sapling commits
        // but before the final coin-state flush, SaplingDB's best block may be ahead
        // of the coin-state. This is safe because ProcessBlock is idempotent and
        // skips blocks already committed to SaplingDB.
        if (sapling::IsSaplingActive(m_params.GetConsensus(), pindexOld->nHeight) &&
            !m_chain_helper->sapling_state->VerifyBestBlock(pindexOld->GetBlockHash())) {
            // Check if SaplingDB is at a block between pindexFork and pindexNew (ahead but valid)
            uint256 saplingBest = m_chain_helper->sapling_state->GetBestBlock();
            bool saplingAhead = false;
            if (!saplingBest.IsNull() && m_blockman.m_block_index.count(saplingBest)) {
                const CBlockIndex* pSaplingBest = &m_blockman.m_block_index[saplingBest];
                if (pSaplingBest->nHeight > pindexOld->nHeight && pSaplingBest->nHeight <= pindexNew->nHeight) {
                    saplingAhead = true;
                    LogPrintf("ReplayBlocks(): SaplingDB ahead at height %d (coin-state at %d), "
                              "will skip already-committed blocks\n",
                              pSaplingBest->nHeight, pindexOld->nHeight);
                }
            }
            if (!saplingAhead) {
                return error("ReplayBlocks(): Found SaplingDB inconsistency, reindex required");
            }
        }
    }
    ...
    while (pindexOld != pindexFork) {
        if (pindexOld->nHeight > 0) { // Never disconnect the genesis block.
            ...
            // Disconnect HMP state to match DisconnectTip behavior
            if (g_hmp_commitments) g_hmp_commitments->BlockDisconnected(pindexOld->nHeight);
            if (g_hmp_privilege) g_hmp_privilege->BlockDisconnected(pindexOld->nHeight);
            if (g_seal_manager) g_seal_manager->BlockDisconnected(pindexOld->GetBlockHash(), pindexOld->nHeight);
...
bool ChainstateManager::LoadBlockIndex()
{
    ...
    if (!fReindex) {
        ...
        for (CBlockIndex* pindex : vSortedByHeight) {
            ...
            if (pindex->nStatus & BLOCK_FAILED_MASK && (!m_best_invalid || pindex->nChainSealWork > m_best_invalid->nChainSealWork)) {
...
// Kerrigan has no BIP30 exceptions: new chain, no duplicate coinbase history.
