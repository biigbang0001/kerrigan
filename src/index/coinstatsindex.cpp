...
struct DBVal {
    ...
    CAmount total_sapling_shielded_amount;
    ...
    SERIALIZE_METHODS(DBVal, obj)
    {
        ...
        READWRITE(obj.total_sapling_shielded_amount);
    }
};
...
bool CoinStatsIndex::WriteBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    // Sapling shielded pool: subtract valueBalance (negative vB = into pool).
    for (const auto& tx : block.vtx) {
        if (tx->nType == TRANSACTION_SAPLING) {
            if (const auto opt_payload = GetTxPayload<SaplingTxPayload>(*tx)) {
                m_total_sapling_shielded_amount -= opt_payload->valueBalance;
            }
        }
    }

    // If spent prevouts + block subsidy are still a higher amount than
    // new outputs + coinbase + current unspendable amount + shielded pool,
    // the miner did not claim the full block reward. Unclaimed block
    // rewards are also unspendable.
    const CAmount unclaimed_rewards{(m_total_prevout_spent_amount + m_total_subsidy) - (m_total_new_outputs_ex_coinbase_amount + m_total_coinbase_amount + m_total_unspendable_amount + m_total_sapling_shielded_amount)};
    ...
    value.second.total_sapling_shielded_amount = m_total_sapling_shielded_amount;
...
bool CoinStatsIndex::Init()
{
    ...
    if (pindex) {
        ...
        m_total_sapling_shielded_amount = entry.total_sapling_shielded_amount;
    }
...
bool CoinStatsIndex::ReverseBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    // Reverse Sapling shielded pool tracking (undo the valueBalance accumulation).
    for (const auto& tx : block.vtx) {
        if (tx->nType == TRANSACTION_SAPLING) {
            if (const auto opt_payload = GetTxPayload<SaplingTxPayload>(*tx)) {
                m_total_sapling_shielded_amount += opt_payload->valueBalance;
    ...
    const CAmount unclaimed_rewards{(m_total_new_outputs_ex_coinbase_amount + m_total_coinbase_amount + m_total_unspendable_amount + m_total_sapling_shielded_amount) - (m_total_prevout_spent_amount + m_total_subsidy)};
    m_total_unspendable_amount -= unclaimed_rewards;
    m_total_unspendables_unclaimed_rewards -= unclaimed_rewards;
    ...
    Assert(m_total_sapling_shielded_amount == read_out.second.total_sapling_shielded_amount);
