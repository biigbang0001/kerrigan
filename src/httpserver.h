...
public:
    ...
    /** Get the local port this request was received on.
     * Used for per-algo RPC port routing.
     */
...
/** Map of local RPC port to mining algorithm index.
 * Populated from -rpcalgoport config. When a getblocktemplate request arrives
 * on one of these ports, the algo is used automatically, no "algo" param needed.
 * This allows standard pool software (s-nomp, etc.) to mine a specific algorithm
 * by simply connecting to the correct RPC port.
 */
extern std::map<uint16_t, int> g_rpc_algo_ports;
