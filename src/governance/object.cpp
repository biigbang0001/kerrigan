                                    ...
                                    const CGovernanceVote& vote, CGovernanceException& exception)
{
    ...
    // Look up existing record without inserting. We only insert after all
    // validation passes to avoid leaving ghost entries on rejection.
    auto it = mapCurrentMNVotes.find(vote.GetMasternodeOutpoint());
    if (it != mapCurrentMNVotes.end()) {
        auto it2 = it->second.mapInstances.find(int(eSignal));
        if (it2 != it->second.mapInstances.end()) {
            const vote_instance_t& existingVote = it2->second;

            // Reject obsolete votes
            if (vote.GetTimestamp() < existingVote.nCreationTime) {
                std::string msg{strprintf("CGovernanceObject::%s -- Obsolete vote", __func__)};
                LogPrint(BCLog::GOBJECT, "%s\n", msg);
                exception = CGovernanceException(msg, GOVERNANCE_EXCEPTION_NONE);
                return false;
            } else if (vote.GetTimestamp() == existingVote.nCreationTime) {
                ...
                if (vote.GetOutcome() < existingVote.eOutcome) {
    ...
    int64_t nVoteTimeUpdate = 0;
    if (it != mapCurrentMNVotes.end()) {
        auto it2 = it->second.mapInstances.find(int(eSignal));
        if (it2 != it->second.mapInstances.end()) {
            nVoteTimeUpdate = it2->second.nTime;
        }
    }
    if (govman.AreRateChecksEnabled()) {
        int64_t nTimeDelta = nNow - nVoteTimeUpdate;
    ...
    bool onlyVotingKeyAllowed = (m_obj.type == GovernanceObject::PROPOSAL ||
                                  m_obj.type == GovernanceObject::ESCROW_RELEASE)
    ...
    // All validation passed. Now safe to insert into the map.
    mn_metaman.AddGovernanceVote(dmn->proTxHash, vote.GetParentHash());

    auto& voteRecordRef = mapCurrentMNVotes[vote.GetMasternodeOutpoint()];
    auto& voteInstanceRef = voteRecordRef.mapInstances[int(eSignal)];
...
bool CGovernanceObject::IsValidLocally(const CDeterministicMNList& tip_mn_list, const ChainstateManager& chainman, std::string& strError, bool& fMissingConfirmations, bool fCheckCollateral) const
{
    ...
    case GovernanceObject::ESCROW_RELEASE: {
        CEscrowReleaseValidator validator(GetDataAsHexString());
        if (!validator.Validate()) {
            strError = strprintf("Invalid escrow release data: %s", validator.GetErrorMessages());
            return false;
        }
        if (fCheckCollateral && !IsCollateralValid(chainman, strError, fMissingConfirmations)) {
            strError = "Invalid escrow release collateral";
            return false;
        }
        return true;
    }
...
CAmount CGovernanceObject::GetMinCollateralFee() const
{
    ...
    switch (m_obj.type) {
        ...
        case GovernanceObject::ESCROW_RELEASE: {
            return GOVERNANCE_PROPOSAL_FEE_TX; // same collateral as regular proposals
        }
        default: {
            return MAX_MONEY;
        }
...
int CGovernanceObject::CountMatchingVotes(const CDeterministicMNList& tip_mn_list, vote_signal_enum_t eVoteSignalIn, vote_outcome_enum_t eVoteOutcomeIn) const
{
    ...
    for (const auto& [outpoint, recVote] : mapCurrentMNVotes) {
        ...
        if (it2 != recVote.mapInstances.end() && it2->second.eOutcome == eVoteOutcomeIn) {
            // 4x times weight vote for BroodNode owners.
            // No need to check if v19 is active since no BroodNode is allowed to register before v19s
