...
private:
    ...
    // Per-peer burst counter for aggregate vote sync requests.
    // After 10 distinct per-object syncs, a fulfilled-request cooldown is triggered.
    // Stores {count, last_activity_epoch_seconds} per peer IP.
    // Capped at MAX_GOVSYNC_BURST_ENTRIES with lazy eviction of stale entries.
    mutable Mutex m_govsync_cs;
    std::map<CNetAddr, std::pair<int, int64_t>> m_govsync_burst_count GUARDED_BY(m_govsync_cs);
    static constexpr size_t MAX_GOVSYNC_BURST_ENTRIES = 1000;
    static constexpr int64_t GOVSYNC_BURST_STALE_SECS = 3600;
};
