    ...
    scheduler.scheduleEvery(
        [this]() -> void {
            ...
            if (!vecOrphanHashes.empty()) {
                ...
                // Limit to 3 peers per orphan hash (was O(hashes * peers) storm)
...
void NetGovernance::ProcessMessage(CNode& peer, const std::string& msg_type, CDataStream& vRecv)
{
    ...
    if (msg_type == NetMsgType::MNGOVERNANCESYNC) {
        ...
        if (nProp == uint256()) {
            ...
            // Send inventory to requesting peer only (was global broadcast via PeerRelayInv)
            ...
            // Aggregate per-peer rate limit -- after 10 distinct per-object vote
            // syncs, require a cooldown before accepting more. This caps the total BLS
            // verification burst from a single peer across all proposal hashes.
            if (m_netfulfilledman.HasFulfilledRequest(peer.addr, "govsync-aggregate")) {
                LogPrint(BCLog::GOBJECT, "MNGOVERNANCESYNC -- peer=%d hit aggregate vote sync cooldown\n",
                         peer.GetId());
                return;
            }
            {
                ...
                // Evict stale entries, then enforce hard cap (mirrors m_orphan_sync_last_sent pattern)
                for (auto it = m_govsync_burst_count.begin(); it != m_govsync_burst_count.end(); ) {
                    if ((nNow - it->second.second) > GOVSYNC_BURST_STALE_SECS)
                        it = m_govsync_burst_count.erase(it);
                    else
                        ++it;
                }
                auto it = m_govsync_burst_count.find(peer.addr);
                if (it == m_govsync_burst_count.end()) {
                    if (m_govsync_burst_count.size() >= MAX_GOVSYNC_BURST_ENTRIES) {
                        // At capacity -- silently drop; fulfilled-request cooldown still protects us
                        LogPrint(BCLog::GOBJECT, "MNGOVERNANCESYNC -- burst map at capacity, ignoring new peer\n");
                    } else {
                        it = m_govsync_burst_count.emplace(peer.addr, std::make_pair(1, nNow)).first;
                    }
                } else {
                    it->second.first++;
                    it->second.second = nNow;
                }
                if (it != m_govsync_burst_count.end() && it->second.first >= 10) {
                    m_netfulfilledman.AddFulfilledRequest(peer.addr, "govsync-aggregate");
                    m_govsync_burst_count.erase(it);
                }
            }

            // Rate limit per-object vote sync requests
            ...
            m_connman.PushMessage(&peer, msgMaker.Make(NetMsgType::SYNCSTATUSCOUNT, MASTERNODE_SYNC_GOVOBJ_VOTE,
                                                       static_cast<int>(invs.size())));
            // Send inventory to requesting peer only (was global broadcast)
            for (const auto& inv : invs) {
                m_connman.PushMessage(&peer, msgMaker.Make(NetMsgType::INV, std::vector<CInv>{inv}));
            }
        }
    ...
    // A NEW GOVERNANCE OBJECT VOTE HAS ARRIVED
        ...
        if (m_gov_manager.ProcessVote(&peer, vote, exception, hashToRequest)) {
            ...
            if (hashToRequest != uint256()) {
                ...
                // Rate-limit to 1 MNGOVERNANCESYNC per peer per 5 seconds
                ...
                bool shouldSend = false;
                {
                    ...
                    // Evict stale entries unconditionally, then enforce hard cap
