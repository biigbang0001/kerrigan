...
bool operator<(const CGovernanceVote& vote1, const CGovernanceVote& vote2)
{
    // Proper lexicographic comparison.
    // The previous implementation always returned false because
    // each stage required both (a < b) AND (a == b) to be true simultaneously.
    if (vote1.masternodeOutpoint != vote2.masternodeOutpoint)
        return vote1.masternodeOutpoint < vote2.masternodeOutpoint;
    if (vote1.nParentHash != vote2.nParentHash)
        return vote1.nParentHash < vote2.nParentHash;
    if (vote1.nVoteSignal != vote2.nVoteSignal)
        return vote1.nVoteSignal < vote2.nVoteSignal;
    return vote1.nVoteOutcome < vote2.nVoteOutcome;
}
