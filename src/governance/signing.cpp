...
std::optional<const CSuperblock> GovernanceSigner::CreateSuperblockCandidate(int nHeight) const
{
    ...
    for (const auto& proposal : approvedProposals) {
        ...
        // Skip proposals that are too expensive (subtraction form avoids signed overflow UB)
        if (payment.nAmount > governanceBudget - budgetAllocated) continue;

        int64_t startEpoch = jproposal["start_epoch"].getInt<int64_t>();
        int64_t endEpoch = jproposal["end_epoch"].getInt<int64_t>();
        constexpr int64_t fudge = count_seconds(GOVERNANCE_FUDGE_WINDOW);
        // Guard against overflow from extreme epoch values
        if (startEpoch < fudge || endEpoch > std::numeric_limits<int64_t>::max() - fudge) continue;
        int64_t windowStart = startEpoch - fudge;
        int64_t windowEnd = endEpoch + fudge;
