...
// No custom operator<; std::pair's default lexicographic comparison
// handles both the vote (first) and timestamp tiebreaker (second).
...
struct vote_rec_t {
    ...
    SERIALIZE_METHODS(vote_rec_t, obj)
    {
        ...
        SER_READ(obj, {
            if (obj.mapInstances.size() > 100) {
                throw std::ios_base::failure("vote_rec_t: mapInstances size exceeds safety limit");
            }
        });
    }
};
...
public:
    ...
    void Unserialize(Stream& s) EXCLUSIVE_LOCKS_REQUIRED(!cs)
    {
        ...
        if (s.GetType() & SER_DISK) {
            ...
            LOCK(cs);
            s >> nDeletionTime >> fExpired >> mapCurrentMNVotes >> fileVotes;
            if (mapCurrentMNVotes.size() > 100000) {
                throw std::ios_base::failure("CGovernanceObject: mapCurrentMNVotes size exceeds safety limit");
            }
        }
