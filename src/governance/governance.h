...
public:
    ...
    {
        READWRITE(LIMITED_VECTOR(obj.vecTimestamps, RATE_BUFFER_SIZE), obj.nDataStart, obj.nDataEnd, obj.fBufferEmpty);
        SER_READ(obj, {
            if (obj.nDataStart >= RATE_BUFFER_SIZE || obj.nDataEnd >= RATE_BUFFER_SIZE) {
                obj.nDataStart = 0;
                obj.nDataEnd = 0;
                obj.fBufferEmpty = true;
            }
        });
    }
};
...
protected:
    ...
    static constexpr int MAX_ORPHAN_VOTES_CACHE_SIZE = 10000;
...
public:
    ...
    void Unserialize(Stream &s) EXCLUSIVE_LOCKS_REQUIRED(!cs_store)
    {
        ...
        // Post-deserialization bounds check.
        // Maps are allocated element-by-element from the stream so OOM risk
        // is bounded by file size, but reject clearly corrupt caches early.
        static constexpr size_t MAX_GOV_CACHE_ENTRIES = 100000;
        if (mapErasedGovernanceObjects.size() > MAX_GOV_CACHE_ENTRIES ||
            mapObjects.size() > MAX_GOV_CACHE_ENTRIES ||
            mapLastMasternodeObject.size() > MAX_GOV_CACHE_ENTRIES) {
            throw std::ios_base::failure("Governance cache container exceeds safety limit");
        }
    }
...
public:
    ...
    // Escrow release: look up a funded ESCROW_RELEASE governance object by hash
    std::shared_ptr<const CGovernanceObject> FindFundedEscrowRelease(const uint256& nHash) const
        EXCLUSIVE_LOCKS_REQUIRED(!cs_store);
    ...
    std::pair<std::vector<uint256>, std::vector<uint256>> FetchGovernanceObjectVotes(
        size_t peers_per_hash_max, int64_t now, std::map<uint256, std::map<CNetAddr, int64_t>>& map_asked_recently) const
        EXCLUSIVE_LOCKS_REQUIRED(!cs_store);
