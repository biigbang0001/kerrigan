...
void CProposalValidator::ParseStrHexData(const std::string& strHexData)
{
    ...
    ParseJSONData(std::string(v.begin(), v.end()), "Proposal");
}
...
bool CProposalValidator::ValidateStartEndEpoch(bool fCheckExpiration)
{
    ...
    // Reject absurd epoch values that could overflow +/-2h arithmetic in signing.cpp
    static constexpr int64_t MIN_EPOCH = 0;
    static constexpr int64_t MAX_EPOCH = 4102444800LL; // 2100-01-01 00:00:00 UTC
    if (nStartEpoch < MIN_EPOCH || nStartEpoch > MAX_EPOCH ||
        nEndEpoch < MIN_EPOCH || nEndEpoch > MAX_EPOCH) {
        strErrorMessages += "epoch value out of range;";
        return false;
    }
    ...
    // Max governance object lifetime: 1 year (365 * 24 * 60 * 60 = 31536000 seconds)
    static constexpr int64_t MAX_GOVERNANCE_OBJECT_LIFETIME = 365 * 24 * 60 * 60;
    if (nEndEpoch - nStartEpoch > MAX_GOVERNANCE_OBJECT_LIFETIME) {
        strErrorMessages += "end_epoch too far from start_epoch (max 1 year);";
        return false;
    }
...
bool CProposalValidator::ValidatePaymentAmount()
{
    ...
    // Bounds-check in float domain before casting to avoid UB
...
// CProposalValidator overrides: log exception details to strErrorMessages
