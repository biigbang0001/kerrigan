...
void CSuperblock::ParsePaymentSchedule(const std::string& strPaymentAddresses, const std::string& strPaymentAmounts, const std::string& strProposalHashes)
{
    ...
    // CAP THE NUMBER OF PAYMENTS TO PREVENT UNBOUNDED ALLOCATION
    static constexpr size_t MAX_SUPERBLOCK_PAYMENTS = 100;
    if (vecPaymentAddresses.size() > MAX_SUPERBLOCK_PAYMENTS) {
        throw std::runtime_error(strprintf("CSuperblock::%s -- Too many payments (%zu)", __func__, vecPaymentAddresses.size()));
    }
    ...
    */
    ...
    for (int i = 0; i < (int)vecPaymentAddresses.size(); i++) {
        ...
        if (!IsValidDestination(dest)) {
            std::string msg{strprintf("CSuperblock::%s -- Invalid Kerrigan address: %s", __func__, vecPaymentAddresses[i])};
            LogPrintf("%s\n", msg);
            throw std::runtime_error(msg);
        }
...
CAmount CSuperblock::GetPaymentsTotalAmount()
{
    ...
    for (int i = 0; i < nPayments; i++) {
        // Check for overflow before adding to avoid signed wrap bypassing the budget cap
        if (!MoneyRange(nPaymentsTotalAmount + vecPayments[i].nAmount)) {
            return MAX_MONEY; // force rejection by limit check
        }
