...
GovernanceStore::GovernanceStore() :
    ...
    cmmapOrphanVotes(MAX_ORPHAN_VOTES_CACHE_SIZE),
...
void CGovernanceManager::AddPostponedObjectInternal(const CGovernanceObject& govobj)
{
    ...
    // Cap mapPostponedObjects to prevent memory exhaustion via pending-collateral flood
    static constexpr size_t MAX_POSTPONED_OBJECTS = 1000;
    if (mapPostponedObjects.size() >= MAX_POSTPONED_OBJECTS) {
        LogPrint(BCLog::GOBJECT, "GOVERNANCE: mapPostponedObjects at capacity (%d), rejecting %s\n",
                 MAX_POSTPONED_OBJECTS, govobj.GetHash().ToString());
        return;
    }
...
void CGovernanceManager::AddGovernanceObjectInternal(CGovernanceObject& insert_obj, const CNode* pfrom)
{
    ...
    // Cap the total number of governance objects to prevent unbounded memory growth
    // (e.g. when nMasternodes=0 and objects are never cleaned up by superblock cycles).
    static constexpr size_t MAX_GOVERNANCE_OBJECTS = 10000;
    if (mapObjects.size() >= MAX_GOVERNANCE_OBJECTS) {
        LogPrintf("GOVERNANCE: mapObjects at capacity (%zu), rejecting new object %s\n",
                  MAX_GOVERNANCE_OBJECTS, nHash.ToString());
        return;
    }
    ...
    for (auto it = mapObjects.begin(); it != mapObjects.end();) {
        ...
        if ((pObj->IsSetCachedDelete() || pObj->IsSetExpired()) &&
            ...
            if (pObj->GetObjectType() == GovernanceObject::PROPOSAL) {
                // Tombstone TTL: expire proposal tombstones after ~70 days (50000 blocks * 120s).
                // This prevents unbounded memory growth while being long enough to prevent
                // tombstone-bypassed re-submission.
                int64_t nTombstoneTTL = 50000 * Params().GetConsensus().nPowTargetSpacing;
                nTimeExpired = (nNow + std::chrono::seconds{nTombstoneTTL}).count();
            } else {
            ...
            // Cap erased objects map to prevent unbounded growth
            static constexpr size_t MAX_ERASED_OBJECTS = 10000;
            if (mapErasedGovernanceObjects.size() >= MAX_ERASED_OBJECTS) {
                // Evict oldest (earliest expiry) entry
                auto oldest = std::min_element(mapErasedGovernanceObjects.begin(),
                    mapErasedGovernanceObjects.end(),
                    [](const auto& a, const auto& b) { return a.second < b.second; });
                if (oldest != mapErasedGovernanceObjects.end())
                    mapErasedGovernanceObjects.erase(oldest);
            }
...
std::shared_ptr<const CGovernanceObject> CGovernanceManager::FindFundedEscrowRelease(const uint256& nHash) const
{
    LOCK(cs_store);
    auto pObj = FindConstGovernanceObjectInternal(nHash);
    if (!pObj) return nullptr;
    if (pObj->GetObjectType() != GovernanceObject::ESCROW_RELEASE) return nullptr;
    if (!pObj->IsSetCachedFunding()) return nullptr;
    return pObj;
}
...
bool CGovernanceManager::ConfirmInventoryRequest(const CInv& inv)
{
    ...
    // Cap inventory request tracking to prevent unbounded growth between cleanup cycles (#R11)
    // Evict begin() entry (deterministic by hash order, O(1)) instead of O(n) scan
    static constexpr size_t MAX_INV_TRACKING{50000};
    if (m_requested_hash_time.size() >= MAX_INV_TRACKING && m_requested_hash_time.find(inv.hash) == m_requested_hash_time.end()) {
        m_requested_hash_time.erase(m_requested_hash_time.begin());
    }
...
std::vector<CInv> CGovernanceManager::GetSyncableVoteInvs(const uint256& nProp, const CBloomFilter& filter) const
{
    ...
    for (const auto& vote : fileVotes.GetVotes()) {
        ...
        bool onlyVotingKeyAllowed = (govobj.GetObjectType() == GovernanceObject::PROPOSAL ||
                                      govobj.GetObjectType() == GovernanceObject::ESCROW_RELEASE)
...
std::pair<std::vector<uint256>, std::vector<uint256>> CGovernanceManager::FetchGovernanceObjectVotes(
    size_t nPeersPerHashMax, int64_t nNow, std::map<uint256, std::map<CNetAddr, int64_t>>& mapAskedRecently) const
{
