...
static constexpr uint8_t DB_CHAIN_VERSION{'V'};
    ...
    if (old_tip.IsNull()) {
        ...
        if (old_heads.size() == 2) {
            if (old_heads[0] != hashBlock) {
                LogPrintLevel(BCLog::COINDB, BCLog::Level::Error, "The coins database detected an inconsistent state, likely due to a previous crash or shutdown. You will need to restart kerrigand with the -reindex-chainstate or -reindex configuration option.\n");
            }
...
bool CBlockTreeDB::ReadTimestampIndex(const uint32_t high, const uint32_t low, std::vector<uint256>& hashes, size_t maxResults) {
    std::unique_ptr<CDBIterator> pcursor(NewIterator());

    pcursor->Seek(std::make_pair(DB_TIMESTAMPINDEX, CTimestampIndexIteratorKey(low)));

    while (pcursor->Valid()) {
        if (maxResults > 0 && hashes.size() >= maxResults) break;
...
bool CBlockTreeDB::ReadChainVersion(int& nVersion) {
    return Read(DB_CHAIN_VERSION, nVersion);
}

bool CBlockTreeDB::WriteChainVersion(int nVersion) {
    return Write(DB_CHAIN_VERSION, nVersion);
}
    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // Algo-specific header fields, must copy to pindexNew
                // so GetBlockHeader() returns correct data after restart.
                pindexNew->hashReserved   = diskindex.hashReserved;
                pindexNew->nNonce256      = diskindex.nNonce256;
                pindexNew->nSolution      = diskindex.nSolution;
                pindexNew->nKawPowHeight  = diskindex.nKawPowHeight;
                pindexNew->nNonce64       = diskindex.nNonce64;
                pindexNew->mix_hash       = diskindex.mix_hash;
                // HMP seal weight (persisted via CDiskBlockIndex serialization)
                // Clamp to valid range to prevent corrupted DB from causing overflow
                pindexNew->nSealWeight    = std::min(diskindex.nSealWeight, uint64_t{20000});

                // Use the per-algo PoW hash, not the identity (X11) hash, so that
                // KawPoW and Equihash blocks pass this sanity check on mainnet.
                // Reconstruct a partial block header from diskindex which holds all
                // algo-specific fields (nNonce256, nSolution, nNonce64, mix_hash).
                {
                    CBlockHeader hdr;
                    hdr.nVersion       = diskindex.nVersion;
                    hdr.hashPrevBlock  = diskindex.hashPrev;
                    hdr.hashMerkleRoot = diskindex.hashMerkleRoot;
                    hdr.hashReserved   = diskindex.hashReserved;
                    hdr.nTime          = diskindex.nTime;
                    hdr.nBits          = diskindex.nBits;
                    hdr.nNonce         = diskindex.nNonce;
                    hdr.nNonce256      = diskindex.nNonce256;
                    hdr.nSolution      = diskindex.nSolution;
