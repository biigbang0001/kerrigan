...
static constexpr size_t MAX_ADDRESS_INDEX_ENTRIES = 100000;
...
static RPCHelpMan debug()
{
    return RPCHelpMan{"debug",
        ...
        " - \"kerrigan\" activates all Kerrigan-specific categories at once;\n"
        ...
        RPCExamples {
            HelpExampleCli("debug", "kerrigan")
    + HelpExampleRpc("debug", "kerrigan+net")
        },
...
static bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, AddressType> > &addresses)
{
    if (params[0].isStr()) {
        ...
        if (addressValues.size() > 100) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Too many addresses (max 100)");
        }
    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (indexes.size() > MAX_ADDRESS_INDEX_ENTRIES) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Result too large, use height-filtered queries");
    }
    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (unspentOutputs.size() > 100000) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Result too large (>100000 entries). Narrow your query.");
    }
    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (addressIndex.size() > MAX_ADDRESS_INDEX_ENTRIES) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Result too large, use height-filtered queries");
    }
...
static RPCHelpMan getaddressbalance()
{
    return RPCHelpMan{"getaddressbalance",
        ...
        RPCResult{
            RPCResult::Type::OBJ, "", "",
                {
                    {RPCResult::Type::NUM, "balance", "The current total balance in satoshis"},
                    {RPCResult::Type::NUM, "balance_immature", "The current immature balance in satoshis"},
                    {RPCResult::Type::NUM, "balance_spendable", "The current spendable balance in satoshis"},
    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (addressIndex.size() > MAX_ADDRESS_INDEX_ENTRIES) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Address has too many entries, use height-filtered queries");
    }
...
static RPCHelpMan logging()
{
    return RPCHelpMan{"logging",
            ...
            + HelpExampleCli("logging", "'[\"kerrigan\"]' '[\"llmq\",\"zmq\"]'")
...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "kerrigan-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                ...
                auto init = ipc->spawnProcess("kerrigand");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a kerrigand
                // process not a kerrigand-node process, so just create a local
