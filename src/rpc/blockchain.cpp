...
double GetDifficulty(const CBlockIndex* blockindex)
{
    ...
    if (IsEquihash(algo)) {
        return ConvertBitsToDoubleEquihash(blockindex->nBits);
    }
...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex, const chainlock::Chainlocks& chainlocks)
{
    ...
    // Algo-specific PoW fields
    if (IsEquihash(algo)) {
        result.pushKV("nonce256", blockindex->nNonce256.GetHex());
        result.pushKV("hashreserved", blockindex->hashReserved.GetHex());
        ...
        result.pushKV("kawpow_height", (int64_t)blockindex->nKawPowHeight);
...
static RPCHelpMan getdifficulty()
{
    return RPCHelpMan{"getdifficulty",
                ...
                "When algo is specified, returns the difficulty of the most recent block mined with that algorithm.\n",
                {
                    {"algo", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "Mining algorithm: x11, kawpow, equihash200, equihash192. Omit for chain tip difficulty."},
                },
                RPCResult{
                    RPCResult::Type::NUM, "", "the proof-of-work difficulty as a multiple of the minimum difficulty."},
                RPCExamples{
                    HelpExampleCli("getdifficulty", "")
            + HelpExampleCli("getdifficulty", "\"kawpow\"")
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull()) {
        int algo = GetAlgoByName(request.params[0].get_str(), -1);
        if (algo < 0) throw JSONRPCError(RPC_INVALID_PARAMETER, "Unknown algorithm: " + request.params[0].get_str());
        const CBlockIndex* pAlgo = GetLastBlockIndexForAlgo(chainman.ActiveChain().Tip(), chainman.GetConsensus(), algo);
        if (!pAlgo) return 0.0;
        return GetDifficulty(pAlgo);
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    const auto [index, tip_height] = WITH_LOCK(cs_main, return std::make_pair(
        chainman.m_blockman.LookupBlockIndex(block_hash),
        chainman.ActiveChain().Tip() ? chainman.ActiveChain().Tip()->nHeight : 0););
    ...
    if (index->nHeight > tip_height && node::fPruneMode) {
...
static RPCHelpMan getblockhashes()
{
    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    int high_signed = request.params[0].getInt<int>();
    int low_signed = request.params[1].getInt<int>();
    if (high_signed < 0 || low_signed < 0) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Timestamps must be non-negative");
    }
    unsigned int high = static_cast<unsigned int>(high_signed);
    unsigned int low = static_cast<unsigned int>(low_signed);
    static constexpr size_t MAX_BLOCKHASHES_RESULT = 2000;
    std::vector<uint256> blockHashes;

    ChainstateManager& chainman = EnsureAnyChainman(request.context);
    if (LOCK(::cs_main); !GetTimestampIndex(*chainman.m_blockman.m_block_tree_db, high, low, blockHashes, MAX_BLOCKHASHES_RESULT)) {
                ...
                },
                {
                    RPCResult{"for verbose = true",
                        RPCResult::Type::OBJ, "", "",
                        {
                            ...
                            {RPCResult::Type::STR, "algo", "The mining algorithm (x11, kawpow, equihash200, equihash192)"},
                            {RPCResult::Type::STR_HEX, "nonce256", /*optional=*/true, "32-byte equihash mining nonce (equihash algos only)"},
                            {RPCResult::Type::STR_HEX, "hashreserved", /*optional=*/true, "32-byte reserved hash (equihash algos only)"},
                            {RPCResult::Type::STR_HEX, "solution", /*optional=*/true, "Equihash solution (equihash algos only)"},
                            {RPCResult::Type::STR_HEX, "nonce64", /*optional=*/true, "64-bit mining nonce as hex (kawpow only)"},
                            {RPCResult::Type::NUM, "kawpow_height", /*optional=*/true, "DAG epoch height (kawpow only)"},
                            {RPCResult::Type::STR_HEX, "mix_hash", /*optional=*/true, "KawPoW mix hash (kawpow only)"},
    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Cap hex input before ParseHex to prevent unbounded allocation
    // (MAX_BLOOM_FILTER_SIZE = 36000 bytes = 72000 hex chars)
    if (strFilter.size() > 80000) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Bloom filter hex too large");
    }
    if (!IsHex(strFilter)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid hex encoding for bloom filter");
    }
    ...
    // Collect block index pointers under cs_main
    std::vector<const CBlockIndex*> vBlockIndex;
    {
        LOCK(cs_main);
        const CBlockIndex* pblockindex = chainman.m_blockman.LookupBlockIndex(hash);
        if (!pblockindex) {
        ...
        for (; pblockindex; pblockindex = chainman.ActiveChain().Next(pblockindex)) {
            if (--nCount < 0) break;
            vBlockIndex.push_back(pblockindex);
        }
    }

    // Read blocks from disk without holding cs_main
    UniValue arrMerkleBlocks(UniValue::VARR);
    CBlock block;

    for (const CBlockIndex* pindex : vBlockIndex) {
        ...
        {RPCResult::Type::OBJ, "", "",
        {
            ...
            {RPCResult::Type::OBJ, "prevout", "(Only if undo information is available)",
            {
                ...
                {RPCResult::Type::OBJ, "scriptPubKey", "",
                {
                    ...
                    {RPCResult::Type::STR, "address", /*optional=*/true, "The Kerrigan address (only if a well-defined address exists)"},
...
static RPCHelpMan gettxoutsetinfo()
{
    return RPCHelpMan{"gettxoutsetinfo",
                ...
                "The total_amount field only reflects transparent (UTXO) value. Value held in the\n"
                "Sapling shielded pool is tracked separately and reported in the sapling_pool field.\n",
                {
                ...
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        ...
                        {RPCResult::Type::STR_AMOUNT, "sapling_pool", /*optional=*/true, "Total value held in the Sapling shielded pool (not included in total_amount)"},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (maybe_stats.has_value()) {
        ...
        if (node.chain_helper && node.chain_helper->sapling_state) {
            ret.pushKV("sapling_pool", ValueFromAmount(node.chain_helper->sapling_state->GetValuePool()));
        }
        if (!stats.index_used) {
        ...
        },
        {
            ...
            RPCResult{"Otherwise", RPCResult::Type::OBJ, "", "", {
                ...
                {RPCResult::Type::OBJ, "scriptPubKey", "", {
                    ...
                    {RPCResult::Type::STR, "address", /*optional=*/ true, "Kerrigan address (only if a well-defined address exists)"},
                }},
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Per-algo difficulties for block explorer consumption
    UniValue algoDiffs(UniValue::VOBJ);
    for (int a = 0; a < NUM_ALGOS; a++) {
        const CBlockIndex* pAlgo = GetLastBlockIndexForAlgo(&tip, chainman.GetConsensus(), a);
        algoDiffs.pushKV(GetAlgoName(a), pAlgo ? GetDifficulty(pAlgo) : 0.0);
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Ensure wallet and other subscribers process all BlockDisconnected events
    // before returning, so that balance queries reflect the rollback.
    SyncWithValidationInterfaceQueue();
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // Ensure wallet and other subscribers process all BlockConnected events
    // before returning, so that balance queries reflect the reconsidered chain.
    SyncWithValidationInterfaceQueue();
...
static RPCHelpMan getblockstats()
{
    return RPCHelpMan{"getblockstats",
            ...
            RPCResult::Type::OBJ, "", "",
            {
                ...
                {RPCResult::Type::NUM, "avgfeerate", /*optional=*/true, "Average feerate (in satoshis per byte)"},
                {RPCResult::Type::NUM, "avgtxsize", /*optional=*/true, "Average transaction size"},
                {RPCResult::Type::STR_HEX, "blockhash", /*optional=*/true, "The block hash (to check for potential reorgs)"},
                {RPCResult::Type::ARR_FIXED, "feerate_percentiles", /*optional=*/true, "Feerates at the 10th, 25th, 50th, 75th, and 90th percentile size unit (in satoshis per byte)",
                {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[2].isNull()) {
        ...
        if (nCount > 1000)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "count must not exceed 1000");
    }
