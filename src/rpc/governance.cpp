...
// Cap governance object hex input size to prevent unbounded allocation from ParseHex
static constexpr size_t MAX_GOBJECT_HEX_SIZE = 32768; // 16KB decoded
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (strHex.size() > MAX_GOBJECT_HEX_SIZE) {
        throw JSONRPCError(RPC_INVALID_PARAMETER,
            strprintf("hex_data too large: %u bytes (max %u)", strHex.size(), MAX_GOBJECT_HEX_SIZE));
    }
    ...
    if (!u.read(s)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Decoded hex is not valid JSON");
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (strDataHex.size() > MAX_GOBJECT_HEX_SIZE) {
        throw JSONRPCError(RPC_INVALID_PARAMETER,
            strprintf("hex_data too large: %u bytes (max %u)", strDataHex.size(), MAX_GOBJECT_HEX_SIZE));
    }
    ...
    } else if (govobj.GetObjectType() == GovernanceObject::ESCROW_RELEASE) {
        CEscrowReleaseValidator validator(strDataHex);
        if (!validator.Validate()) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid escrow release data: " + validator.GetErrorMessages());
        }
    } else {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid object type, only proposals and escrow releases can be validated");
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // "0" = root node (virtual, hash of zero)
    ...
    // This command is dangerous because it consumes the governance collateral irreversibly.
    ...
    if (!request.params[4].isNull() && !request.params[5].isNull()) {
        ...
        int32_t collateralIndex = ParseInt32V(request.params[5], "outputIndex");
    ...
    // Once commit succeeds the collateral is spent; always return the txid.
    // Metadata write failure is logged as a warning, not an RPC error.
    try {
        wallet->CommitTransaction(tx, {}, {});
    } catch (const std::exception& e) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, strprintf("Transaction commit failed: %s", e.what()));
    }
    if (!wallet->WriteGovernanceObject(Governance::Object{hashParent, nRevision, nTime, tx->GetHash(), strDataHex})) {
        LogPrintf("WARNING: gobject prepare: collateral %s committed but WriteGovernanceObject failed, "
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (nCount > 200) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "count must not exceed 200");
    }
    ...
    size_t start = (static_cast<size_t>(nCount) >= vecObjects.size()) ? 0 : (vecObjects.size() - static_cast<size_t>(nCount));
    for (auto it = vecObjects.begin() + start; it != vecObjects.end(); ++it) {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    bool onlyVotingKeyAllowed = (govObjType == GovernanceObject::PROPOSAL ||
                                  govObjType == GovernanceObject::ESCROW_RELEASE)
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    obj.pushKV("fundingthreshold", std::max(Params().GetConsensus().nGovernanceMinQuorum, static_cast<int>(CHECK_NONFATAL(node.dmnman)->GetListAtChainTip().GetCounts().m_valid_weighted / 10)));
...
Span<const CRPCCommand> GetWalletGovernanceRPCCommands()
{
    static const CRPCCommand commands[]{
        {"kerrigan", &gobject_prepare},
        {"kerrigan", &gobject_list_prepared},
        {"kerrigan", &gobject_vote_many},
        {"kerrigan", &gobject_vote_alias},
    };
    return commands;
}
#endif // ENABLE_WALLET
...
{
    static const CRPCCommand commands[]{
        {"kerrigan", &getgovernanceinfo},
        {"kerrigan", &getsuperblockbudget},
        {"kerrigan", &gobject},
        {"kerrigan", &gobject_count},
        {"kerrigan", &gobject_deserialize},
        {"kerrigan", &gobject_check},
        {"kerrigan", &gobject_submit},
        {"kerrigan", &gobject_list},
        {"kerrigan", &gobject_diff},
        {"kerrigan", &gobject_get},
        {"kerrigan", &gobject_getcurrentvotes},
        {"kerrigan", &voteraw},
    };
