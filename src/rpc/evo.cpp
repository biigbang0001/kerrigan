    ...
    static const std::map<std::string, RPCArg> mapParamHelp = {
        {"collateralAddress",
            {"collateralAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The Kerrigan address to send the collateral to."}
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            {"ownerAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The Kerrigan address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            {"payoutAddress", RPCArg::Type::STR, RPCArg::Optional::NO,
                "The Kerrigan address to use for masternode reward payments."}
        },
...
static RPCHelpMan protx_register_fund_wrapper(const bool legacy)
{
    ...
    return RPCHelpMan{rpc_full_name,
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 10000 Kerrigan\n"
...
static RPCHelpMan protx_register_fund_evo()
{
    ...
    return RPCHelpMan{
        command_name,
        "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 40000 Kerrigan\n"
        "to the address specified by collateralAddress and will then function as the collateral of your\n"
        "BroodNode.\n"
        "A few of the limitations you see in the arguments are temporary and might be lifted after DIP3\n"
        "is fully deployed.\n" +
                                              ...
                                              const MnType mnType)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Kerrigan address: ") + request.params[paramIdx + 6].get_str());
    }
...
static RPCHelpMan protx_update_service_evo()
{
    ...
    return RPCHelpMan{
        ...
        "of a BroodNode.\n"
        "If this is done for a BroodNode that got PoSe-banned, the ProUpServTx will also revive this BroodNode.\n" +
            HELP_REQUIRING_PASSPHRASE,
        {
            GetRpcArg("proTxHash"),
            GetRpcArg("coreP2PAddrs_update"),
...
static UniValue protx_update_service_common_wrapper(const JSONRPCRequest& request, const MnType mnType)
{
    ...
    if (!request.params[paramIdx + 1].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Kerrigan address: ") + request.params[paramIdx + 1].get_str());
    } else {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Kerrigan address: ") + request.params[4].get_str());
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[3].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Kerrigan address: ") + request.params[3].get_str());
    ...
    return RPCHelpMan{"protx list",
        "\nLists all ProTxs in your wallet or on-chain, depending on the given type.\n",
        {
            {"type", RPCArg::Type::STR, RPCArg::Default{"registered"},
                ...
                "  evo          - List only ProTx corresponding to BroodNodes at the given chain height.\n"
#ifdef ENABLE_WALLET
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (type == "wallet") {
        ...
        bool onlyBroodNodes = type == "evo";
        mnList.ForEachMN(onlyValid, [&](const auto& dmn) {
            if (onlyBroodNodes && dmn.nType != MnType::Evo) return;
            ret.push_back(BuildDMNListEntry(wallet.get(), dmn, mn_metaman, detailed, chainman));
        });
    } else {
...
// evodb verify/repair implementation
...
static RPCHelpMan protx_help()
{
    return RPCHelpMan{
        ...
        "  register_evo             - Create and send ProTx to network for a BroodNode\n"
        "  register_fund_evo        - Fund, create and send ProTx to network for a BroodNode\n"
        "  register_prepare_evo     - Create an unsigned ProTx for a BroodNode\n"
        ...
        "  update_service_evo       - Create and send ProUpServTx to network for a BroodNode\n"
