...
static void EnsureAddressIndexAvailable()
{
    if (!node::fAddressIndex) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "Address index is disabled. You should run Kerrigan with -addressindex (requires reindex)");
    }
}
                   ...
                   CSpentIndexValue& value)
{
    ...
    if (!node::fSpentIndex) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "Spent index is disabled. You should run Kerrigan with -spentindex (requires reindex)");
    }
...
bool GetTimestampIndex(CBlockTreeDB& block_tree_db, const uint32_t high, const uint32_t low,
                       std::vector<uint256>& hashes, size_t maxResults)
{
    AssertLockHeld(::cs_main);

    if (!node::fTimestampIndex) {
        throw JSONRPCError(RPC_INVALID_REQUEST, "Timestamp index is disabled. You should run Kerrigan with -timestampindex (requires reindex)");
    }

    return block_tree_db.ReadTimestampIndex(high, low, hashes, maxResults);
}
