...
ARG USER_ID=1000 \
    GROUP_ID=1000
RUN set -ex; \
    (getent group ${GROUP_ID} && usermod -g ${GROUP_ID} ubuntu) || groupmod -g ${GROUP_ID} -n kerrigan ubuntu; \
    usermod -u ${USER_ID} -md /home/kerrigan -l kerrigan ubuntu; \
    chown ${USER_ID}:${GROUP_ID} -R /home/kerrigan; \
    mkdir -p /src/kerrigan && \
    chown ${USER_ID}:${GROUP_ID} /src && \
    chown ${USER_ID}:${GROUP_ID} -R /src

WORKDIR /src/kerrigan
