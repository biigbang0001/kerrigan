...
Tracing (USDT) support for Kerrigan Core on Linux using. For more information on
USDT support in Kerrigan Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `kerrigand` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `kerrigand` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Kerrigan Core USDT documentation] on how to list available tracepoints in your
`kerrigand` binary.

[Kerrigan Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/kerrigand
```
...
Kerrigan P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/kerrigand
```
...
In a different terminal, starting Kerrigan Core in DevNet mode and with
re-indexing enabled.

```
$ ./src/kerrigand -devnet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/kerrigand
```

```
Logging utxocache flushes. Ctrl-C to end...
Duration (us)   Mode       Coins Count     Memory Usage    Prune
