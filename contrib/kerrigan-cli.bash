# bash programmable completion for kerrigan-cli(1)
# Copyright (c) 2012-2019 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $kerrigan-cli for RPC
_kerrigan_rpc() {
...
_kerrigan_cli() {
    ...
    # save and use original argument to invoke kerrigan-cli for -help, help and RPC
    # as kerrigan-cli might not be in $PATH
    kerrigan_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($kerrigan_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_kerrigan_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _kerrigan_cli kerrigan-cli
